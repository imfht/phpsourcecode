...
class  jsonRPCServer {
	 	...
	 	if ($this->request['method'] == "listMethods"){
	 		foreach ($this->classes as $ext => $class){
				...
				foreach($reflectionMethods as $reflectionMethod)
				{
					...
					{
						$methods[$ext][] = $reflectionMethod->getName();
					}
				}
	 ...
	 */
	public function error($c,$fmsg=false,$errorData = Array('no data')){
		$this->response = array (
				'jsonrpc'	=> '2.0',
	...
	public function handle() {
		...
		try {
			...
			if ($obj && ($result = call_user_func_array(array($obj,$this->request['method']),$this->request['params'])) !== false) {
