...
function handleInstallAllOp()
{
    ...
    $adminAccount = post('adminAccount');
    $adminPassword = substr(trim(post('adminPassword')),0,32);
...
{
    $discountUrl = 'https://api.ibos.cn/v3/corp/getdiscount';
    $api = new Api();
...
// --------------------------  CONFIG COOKIE  --------------------------- //
    ...
    // 微信管理员可选择是否提供手机号码和邮箱, 所以这里的 $adminAccount
    $admin = array(// 管理员账号替换信息
        'username' => empty($adminName) ? $adminAccount : $adminName,
        'isadministrator' => 1,
        'password' => $password,
        'createtime' => TIMESTAMP,
        'salt' => $salt,
        'realname' => $realName,
        'mobile' => isMobile($adminAccount) ? $adminAccount : '',
        'email' => '',
        'corpcode' => $corpCode,
        'fullname' => $corpFullname,
        'shortname' => $corpShortname,
    ...
    // 微信管理员可选择是否提供手机号码和邮箱, 所以这里的 $adminAccount
    $adminAccountMobile = isMobile($adminAccount) ? $adminAccount : '';
    $query = $pdo->exec("INSERT INTO `config` ("
        . "`corpcode`, "
        . "`config`, "
        . "`super`, "
