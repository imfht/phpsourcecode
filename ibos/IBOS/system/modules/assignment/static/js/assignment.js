...
var Assignment = {
	...
	// 全局弹窗添加任务
    addPopupTask: function () {
        var dialog = Ui.getDialog("d_am_add"),
            form = dialog.DOM.content.find("form")[0];
        var $form =$(form);
        if(Assignment.validateTaskForm(form)) {
			$form.waiting(null, "small", true);
			var param = $(form).serializeArray();
			Assignment.op.addTask(param).done(function(res){
				$form.waiting(false);
				if(res.isSuccess) {
					Assignment.loadAssignPopupList(data);
					dialog.close();
					Ui.tip(U.lang("ASM.ADD_TASK_SUCCESS"));
				} else {
					Ui.tip(res.msg, "danger");
	...
	openTaskEditDialog: function(param){
		var _this = this;
		if(param.ispopup == "undefined") {
            param.ispopup = false;
        }
		Ui.closeDialog("d_am_edit");
        Ui.closeDialog("d_am_add");
		Ui.ajaxDialog(Ibos.app.url("assignment/default/edit", { "id": param.id,"ispopup": param.ispopup}), {
			id: "d_am_edit",
    ...
    openTaskAddDialog: function(param) {
        var _this = this;
        Ui.closeDialog("d_am_edit");
        Ui.closeDialog("d_am_add");
        Ui.ajaxDialog(Ibos.app.url("assignment/default/addview", {"associatedmodule": param.associatedmodule, "associatednode": param.associatednode, "associatedid": param.associatedid}), {
            id: "d_am_add",
            title: U.lang("ASM.ADD_TASK"),
            width: 780,
            padding: "20px"
        });
    },
    ...
    '<i class="om-am-finished"></i>'+
    '<% } %>'+
    '<% if(status == "4") { %>'+
    '<i class="om-am-cancelled"></i>'+
    '<% } %>'+
    '<% if(showbnt) { %>'+
    '<div href="javascript:;" class="" data-action="openTaskEditDialog" data-param=\'{"id":<%= assignmentid %>,"ispopup":true}\' title="修改">' +
    '<%} else {%>'+
    '<div href="javascript:;" data-action="canNotEditTip" title="无修改权限">'+
    '<%}%>'+
    '	<div class="assign-list--wrap">' +
    '   	<div class="assign-list--content clearfix">' +
    '           <p class="assign-p--botton"><%= subject%></p>' +
    '       	<div class=""><i class="o-assign-label"></i><%= et%></div>' +
    '           <div class=""><%= assignshow%></div>' +
    '<div class="assign-btn-position">' +
    '<% if(showFinishBnt) { %>' +
    '<div class="oa-am-btn-finish" data-action="finishPopupTask" data-param=\'{"id": <%= assignmentid %> }\' title="完成"></div>'+
    '<%}%>' +
    '<% if(showbnt) { %>'+
    '<div class="oa-am-btn-delete" data-action="removeTaskPopup" data-param=\'{"id": <%= assignmentid %> }\' title="删除"></div>' +
    '<%}%>'+
    '</div>'+
    ' </div>' +
    '	</div>' +
     ...
     * @param param
     */
	loadAssignPopupList: function (param) {
        param.op = "getdata";
        var url = Ibos.app.url('assignment/unfinished/listpopup');
        $.get(url, param, $.noop, "json").done(function (res) {
            if(res.isSuccess) {
                // 重新加载列表
                var resdata = res.data;
				var html = '';
				var $assign_list = $("#assign_list");
                for( k in resdata){
                    resdata[k].showurl = Ibos.app.url("assignment/default/show",{"assignmentId":resdata[k].assignmentid});
                    if(resdata[k].designeeuid == G.uid) {
                    	// 发起人是自己才能修改
                        resdata[k].showbnt = true;
					} else {
                        resdata[k].showbnt = false;
					}
                    resdata[k].showFinishBnt = false;
                    // 发起人或负责人是自己才能完成任务，参与人不行
					if (resdata[k].status < 2) {
                        if (resdata[k].designeeuid == G.uid || resdata[k].chargeuid == G.uid) {
                            resdata[k].showFinishBnt = true;
                        }
					}

					// 指派文案处理
					if (resdata[k].designee.uid == resdata[k].charge.uid) {
                        resdata[k].assignshow = '<i class="o-assign-persion"></i>' +
													resdata[k].designee.realname;
					} else {
                        resdata[k].assignshow = '<i class="o-assign-persion"></i>' +
													resdata[k].designee.realname +
														'【指派给】' +
													resdata[k].charge.realname ;
					}
                    html += $.template(Assignment.popupListTpl, resdata[k]);
				}
...
$(function(){
	...
	Ibos.evt.add({
		...
		// 打开编辑任务对话框
		"openTaskEditDialog": function(param){
			Assignment.openTaskEditDialog(param);
        },

        // 打开添加任务对话框
        "openTaskAddDialog": function(param){
            Assignment.openTaskAddDialog(param);
        },
        ...
        // 移除任务
        "removeTaskPopup": function(param){
            var $ele = $(this);
            Ui.confirm(U.lang("ASM.REMOVE_TASK_CONFIRM"), function(){
                Assignment.inLoading(true);
                Assignment.op.removeTask(param.id).done(function(res){
        ...
        // 添加任务数据
        "addPopupTask": function(){
			Assignment.addPopupTask();
        },
		
		// 无修改权限提示
        "canNotEditTip": function () {
            Ui.tip("您没有该任务修改权限","warning");
        },

		// 完成任务
		"finishPopupTask": function (param) {
