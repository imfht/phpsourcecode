...
class BaseController extends Controller
{
    ...
    protected function getUnfinishedDataByUid($uid)
    {
        ...
        // 将最终处理好的数据，添加事件关联标题和跳转地址
        $designeeData = AssignmentUtil::handleEventListData($designeeData);
        $chargeData = AssignmentUtil::handleEventListData($chargeData);
        $participantData = AssignmentUtil::handleEventListData($participantData);
        $params = array(
            'user' => User::model()->fetchByUid($uid),
            'designeeData' => $designeeData,
     ...
     * 检查其他
     */
    protected function getEventAssigmentParam()
    {
        $data['associatedmodule'] = CHtml::encode(Env::getRequest('associatedmodule'));
        $data['associatednode'] = CHtml::encode(Env::getRequest('associatednode'));
        $data['associatedid'] =  CHtml::encode(Env::getRequest('associatedid'));
        if(empty($data['associatedmodule'])) {
            throw new Exception(Ibos::lang("Params error"));
        }
        if(empty($data['associatednode'])) {
            throw new Exception(Ibos::lang("Params error"));
        }
        if(empty($data['associatedid'])) {
            throw new Exception(Ibos::lang("Params error"));
        }
        return $data;
    }
