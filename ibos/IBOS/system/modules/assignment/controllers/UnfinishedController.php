...
class UnfinishedController extends BaseController
{
     ...
     * 其他模块调用弹窗入口
     */
    public function actionListpopup()
    {
        $uid = Ibos::app()->user->uid;
        ...
        if($op == 'getdata') {
            try{
                $param = $this->getEventAssigmentParam();
            }catch (Exception $e) {
                return $this->ajaxReturn(array('isSuccess' => false, 'msg' =>$e->getMessage()));
            }
            $list = Assignment::model()->fetchAllAssignmentListByEvent($uid, $param['associatedmodule'], $param['associatednode'], $param['associatedid']);
            $list = AssignmentUtil::handleListData($list, $uid);
            return $this->ajaxReturn(array('isSuccess' => true, 'data' =>$list));
        } else {
            try{
                $param = $this->getEventAssigmentParam();
            }catch (Exception $e) {
            ...
            $data['uid'] = $uid;
            $data['assetUrl'] = Ibos::app()->assetManager->getAssetsUrl('assignment');
            $data['lang'] = Ibos::getLangSource('assignment.default');
            $data['param'] = json_encode($param);
            $listPopup = 'application.modules.assignment.views.unfinished.listpopup';
            $view = $this->renderPartial($listPopup, $data, true);
            echo $view;
        }
    }
