...
class DefaultController extends BaseController
{
    ...
    {
        // 这里给事件关联任务添加弹窗弹窗
        $assignment['chargeuid'] = StringUtil::wrapId(Ibos::app()->user->uid);
        $assignment['lang'] = Ibos::getLangSource('assignment.default');
        $assignment['assetUrl'] = Ibos::app()->assetManager->getAssetsUrl('assignment');
        $assignment['nowtime'] = date('Y-m-d H:i', time());
        $assignment['eventParam']['associatedmodule'] = \CHtml::encode(Env::getRequest('associatedmodule'));
        $assignment['eventParam']['associatednode'] = \CHtml::encode(Env::getRequest('associatednode'));
        $assignment['eventParam']['associatedid'] = \CHtml::encode(Env::getRequest('associatedid'));
        $assignment['eventParam'] = json_encode($assignment['eventParam']);
        $addAlias = 'application.modules.assignment.views.default.add';
        $addView = $this->renderPartial($addAlias, $assignment, true);
        echo $addView;
    }
    /**
     * 编辑任务
    ...
    public function actionEdit()
    {
        ...
        if (!Env::submitCheck('updatesubmit')) {
            ...
            $ispopup = Env::getRequest('ispopup'); // 判断是不是全局弹窗调用
            $ispopup = (string) $ispopup == 'true' ? 'true' : 'false';
    ...
    public function actionShow()
    {
        ...
        if (empty($op)) {
            ...
            // 关联事件任务
            $assignment = AssignmentUtil::heandleEventAssignmentData($assignment);
            $params = array(
