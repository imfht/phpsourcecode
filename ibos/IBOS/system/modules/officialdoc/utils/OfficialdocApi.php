...
class OfficialdocApi extends MessageApi
{
    ...
    private function loadNewDoc($num = 3)
    {
        ...
        //$condition = " ( ((deptid='alldept' OR FIND_IN_SET('{$allDeptId}',deptid) OR FIND_IN_SET('{$allPosId}',positionid) OR FIND_IN_SET('{$uid}',uid)) OR (deptid='' AND positionid='' AND uid='') OR (author='{$uid}') OR (approver='{$uid}')) ) AND `status`='1'";
        $condition = "((";
        $allDeptIds = array_filter(array_unique(explode(',', $allDeptId )));
        if (count($allDeptIds) > 0) {
            ...
            $condition .= "(deptid='alldept' OR ". $deptCondition.")";
        } else {
            $condition .= "(deptid='alldept')";
        }
        $allPosIds = array_filter(array_unique(explode(',', $allPosId )));
        if (count($allPosIds) > 0) {
            foreach ($allPosIds as $posId) {
                $posConditionArray[] = "FIND_IN_SET('{$posId}',positionid)";
            }
            $posCondition = implode(' OR ', $posConditionArray);
            $condition .= " OR (". $posCondition.")";
        }
        $condition .= "OR FIND_IN_SET('{$uid}',uid)) ";
        $condition .= "OR (deptid='' AND positionid='' AND uid='')) AND `status`='1'";
        $criteria = array(
