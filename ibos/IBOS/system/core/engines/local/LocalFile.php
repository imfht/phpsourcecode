...
//		$this->createFile( $bgPath . $bgMiddle, ' ' );
    ...
    {
        $mainConfig = require PATH_ROOT . '/system/config/config.php';
        $isHide = !empty($mainConfig['jsCacheHidden']) && $mainConfig['jsCacheHidden'] == true ? true : false;
        if (!$isHide) {
            $string = '';
            ...
            $string = '';
            foreach ($typeArray as $type) {
                $string .= '<script src = "?r=main/api/js&type=' . $type . '&' . VERHASH . '"></script>';
            }
            return $string;
        }
    }
