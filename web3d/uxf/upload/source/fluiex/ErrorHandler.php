...
use fluiex\DB;
use fluiex\db\Exception as DbException;
...
class ErrorHandler
{
    ...
    public static function system_error($message, $show = true, $save = true, $halt = true)
    {
        ...
        $debug_backtrace = static::debug_backtrace();
        list($showtrace, $logtrace) = $debug_backtrace;

        if ($save) {
            $messagesave = '<b>' . $message . '</b><br><b>PHP:</b>' . $logtrace;
            static::write_error_log($messagesave);
        }

        if ($show) {
            if (!defined('IN_MOBILE')) {
                static::show_error('system', "<li>$message</li>", $showtrace, 0);
            } else {
                static::mobile_show_error('system', "<li>$message</li>", $showtrace, 0);
            }
        }
    ...
    public static function template_error($message, $tplname)
    {
        ...
        static::system_error($message);
    }

    public static function debug_backtrace()
    {
        $skipfunc[] = __CLASS__ . '::debug_backtrace';
        $skipfunc[] = __CLASS__ . '::db_error';
        $skipfunc[] = __CLASS__ . '::template_error';
        $skipfunc[] = __CLASS__ . '::system_error';
        $skipfunc[] = __CLASS__ . '::exception_error';
        $skipfunc[] = __CLASS__ . '::show_error';
        $skipfunc[] = 'fluiex\F::handleError';
        $skipfunc[] = 'fluiex\F::handleException';
        $skipfunc[] = 'fluiex\F::handleShutdown';
        $skipfunc[] = 'fluiex\db\driver\MySQLi->halt';
        $skipfunc[] = 'fluiex\db\driver\MySQLi->query';

        $show = $log = '';
        $debug_backtrace = debug_backtrace();
        krsort($debug_backtrace);
        foreach ($debug_backtrace as $k => $error) {
            $file = str_replace(DISCUZ_ROOT, '', isset($error['file']) ? $error['file'] : '');
            ...
            $error['line'] = sprintf('%04d', isset($error['line']) ? $error['line'] : '');
    ...
    public static function db_error($message, $sql)
    {
        ...
        list($showtrace, $logtrace) = static::debug_backtrace();
        ...
        static::show_error('db', $msg, $showtrace, false);
        unset($msg, $phperror);
        ...
        static::write_error_log($errormsg);
        exit();
    }
    ...
    public static function exception_error($exception)
    {
        ...
        if ($type == 'db') {
            ...
            $errormsg .= static::sql_clear($exception->getMessage());
            if ($exception->getSql()) {
                $errormsg .= '<div class="sql">';
                $errormsg .= static::sql_clear($exception->getSql());
                $errormsg .= '</div>';
            }
        } else {
        ...
        foreach ($trace as $error) {
                ...
                if (!empty($error['args'])) {
                    ...
                    foreach ($error['args'] as $arg) {
                        ...
                        if (is_array($arg)) {
                            ...
                            $fun .= (defined('DISCUZ_DEBUG') && DISCUZ_DEBUG) ? '\'' . dhtmlspecialchars(substr(static::clear($arg), 0, 10)) . (strlen($arg) > 10 ? ' ...' : '') . '\'' : '%s';
                        }
                        $mark = ', ';
                    }
                }
            ...
            $phpmsg[] = array(
                'file' => str_replace(array(DISCUZ_ROOT, '\\'), array('', '/'), isset($error['file']) ? $error['file'] : ''),
                'line' => isset($error['line']) ? $error['line'] : 0,
                'function' => isset($error['function']) ? $error['function'] : '',
            );
        }

        static::show_error($type, $errormsg, $phpmsg);
        exit();
    }
...
EOT;
    ...
    public static function write_error_log($message)
    {
        ...
        $uri = 'Request: ' . dhtmlspecialchars(static::clear($_SERVER['REQUEST_URI']));
