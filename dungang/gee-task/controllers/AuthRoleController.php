...
class AuthRoleController extends AdminController
{
	...
	public function actionPermission($name)
	{
		...
		$tree = [];
		if($items = $query->indexBy('name')->all()){
			$items = array_map(function($val)use ($rights){
				$val['id'] = $val['name'];
				$val['name'] = $val['description'];
				$val['pid'] = empty($val['parent'])?'':$val['parent'];
			...
			$tree = MiscHelper::listToTree($items,'id','pid','children','');
		}
