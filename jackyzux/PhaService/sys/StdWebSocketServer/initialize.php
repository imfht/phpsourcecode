 ...
 * Path
 */
define('APP_NAME', 'PhaService StdWebSocketServer');
...
define('RUN_PID_FILE', BASE_PATH . '/var/pid/std_web_socket_server.pid');// pid保存文件名
...
if (empty($argv['2'])) {
    ...
    $log_file = sprintf(LOG_PATH . '/std_web_socket_server_%s_%s.log', date('Ymd'), $run_pid);
} else {
    $log_file = $argv['2'];
}
...
// 引入进程文件
require __DIR__ . '/StdWebSocketServer.php';

// 启动服务器
StdWebSocketServer::getInstance($isRun, $log_file);
