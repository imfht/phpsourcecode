...
<div class="main_info clearfix">
  ...
  <div class="main_left"> 
   <div class="mod list clearfix" id="recommend_list_wrap"> 
    <h2 class="main_h2"><span class="bd"></span>支教活动 > <?php echo $zhijiao->title;?></h2> 
	<div class="article">
	<?php if($zhijiao->status==0){?>
		<div style="width:100px;height:30px;background-color:green;color:#fff;border-radius:5px;text-align:center;line-height:30px;margin-left:550px;">报名中</div><br><hr>
    <?php }elseif($zhijiao->status==1){?>
		<div style="width:100px;height:30px;background-color:#ff6c00;color:#fff;border-radius:5px;text-align:center;line-height:30px;margin-left:550px;">活动进行中</div><br><hr>
	<?php }elseif($zhijiao->status==2){?>
		<div style="width:100px;height:30px;background-color:#999999;color:#fff;border-radius:5px;text-align:center;line-height:30px;margin-left:550px;">已结束</div><br><hr>
	<?php }?>
	<?php echo $zhijiao->content;?>
	
	</div>
	<?php if($zhijiao->status==0){?>
	<a onclick="return confirm('确定该操作?')" href="
	<?php if(isset($_SESSION['zjb']) && isset($baoming)){
			if($baoming==0){echo site_url('home/cancelBaoming/'.$zhijiao->id);
				}else{
					echo site_url("home/addBaoming/".$zhijiao->id);
				}
			}else{
				echo site_url("home/logindex");
			}?>" class="fa-btn btn-1 btn-1e wybm">
			<?php if(isset($baoming)){
				if($baoming==0){
					echo "取消报名";
					}else{
						echo "我要报名";
