...
function avatar($uid, $size) {
    ...
    if (!is_file(C('ik_attach_path') . 'face/' . $avatar_file)) {
    	...
    	return __ROOT__ . '/Public/Static/images/' . $avatar_file.'?v='.time();
    }else{
...
function attach($attach) {
    if (false === strpos($attach, 'http://')) {
        ...
        $tempatt = SITE_PATH.'/'.C('ik_attach_path').$attach;
        if(!is_file($tempatt)){
        	//不存在 试着创建
        	$arrfile = explode('/', $attach);
        	$arrimg  = explode('_', end($arrfile));
        	$ext = strtolower(trim(substr(strrchr(end($arrfile), '.'), 1)));
        	$imgname = $arrimg[0].'.'.$ext;
        	$imgpath = substr($attach,0,strpos($attach, strrchr($attach, '/')));
        	$filename = SITE_PATH.'/'.C('ik_attach_path').$imgpath.'/'.$imgname;

        	//判断是否存在原图
        	if(is_file($filename)){
        		//存在在试着切图
	    		$ic = new \Common\Util\ImageCrop($filename);
	    		$w = $arrimg[1]; $h = substr($arrimg[2], 0, strpos($arrimg[2],'.')); 
	    		$ic->Crop($w, $h, 1, $tempatt);
				$ic->SaveImage();
				$ic->destory();
        		...
        		return __ROOT__ . '/Public/Static/images/defimg.gif';
        	}
        }
        return __ROOT__ . '/' . C('ik_attach_path') . $attach.'?v='.time();
        //远程附件
...
function hview($text) {
	$text = stripslashes ( $text ); //函数删除由 addslashes() 函数添加的反斜杠。
	$text = nl2br ( $text );
	return $text;
}
/**
...
function ikhtml_img($type,$typeid,$content){
	...
	if(!empty($photos [2])){
		$strPhoto = D('Common/Images')->getImageByseqid($type,$typeid,$photos [2][0]);
		return $strPhoto;
	}else{
		return;
	}	
}
//注意：该方法已弃用 使用 clearText()函数处理文本输出
function ikhtml_text($type,$typeid,$content){
	...
	foreach ($photos [2] as $key=>$item) {
		$strPhoto = D('Common/Images')->getImageByseqid($type,$typeid,$item);
...
function ikhtml_video($type,$typeid,$content){
	...
	preg_match_all ( '/\[(视频)(\d+)\]/is', $strcontent, $videos );
	if(!empty($videos [2])){
		$str = D('Common/Videos')->getVideoByseqid($type,$typeid,$videos [2][0]);
		return $str;
	}else{
		return;
	}
...
function is_admin_login(){
    ...
    if (empty($user)) {
        ...
        return session('admin_auth_sign') == data_auth_sign($user) ? $user['userid'] : 0;
    }
}    
/**
 * 格式化字节大小
