 ...
 * 调用方法：
 *
 * $ic=new ImageCrop.class('./data/upload/old.jpg','./data/upload/new.jpg');
 * $ic->Crop(120,80,2);
 * $ic->SaveImage();
...
class ImageCrop {
	...
	//$ikimg = array(x,y,w,h) 数组:按位置切割 $mode=4 才可以使用
	function Crop($dst_width, $dst_height, $mode = 4, $dst_file = '', $ikimg = '') {
		if ($dst_file)
			$this->dst_file = $dst_file;
		...
		switch ($mode) {
			...
			case 4 : // keep all image but create actually size
				if ($ratio_w > 1 && $ratio_h > 1) {
					...
					//修改人小麦
					if (!empty($ikimg)){ 		
						imagecopyresampled ( $this->dImage, $this->sImage, 0, 0, $ikimg[0], $ikimg[1], $tmp_w, $tmp_h, $ikimg[2], $ikimg[3] );
					}else{ 
