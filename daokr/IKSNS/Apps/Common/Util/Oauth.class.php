<?php
/*
 * IKPHP爱客网 安装程序 @copyright (c) 2012-3000 IKPHP All Rights Reserved 
 * @author 小麦
...
class Oauth {
    ...
    public function callbackLogin($request_args) {
        ...
        if (!empty($bind_user)) {
            ...
            $user_info = M('user')->field('userid,username,email,doname,last_login_time')->where(array('userid' => $bind_user['uid']))->find();
            //登陆
            //$this->_oauth_visitor()->assign_info($user_info);
            //新版本登录
            session('user_auth', $user_info); 
            session('user_auth_sign', data_auth_sign($user_info));
            return U('space/index/index',array('id'=>$user_info['userid']));
        } else {
            //处理用户名
           ...
           //var_dump($user);die;
            ...
            return U('home/user/binding'); //跳转到绑定注册页
        }
    }
    ...
    public function callbackBind($request_args) {
        if (!session('user_auth')) {
            return U('home/user/login');
        }
        $ik_user = session('user_auth');
        $user = $this->_om->getUserInfo($request_args);
        $bind_user = $this->_checkBind($this->_type, $user['keyid']);
