...
// +----------------------------------------------------------------------
// | Author: 小麦 <810578553@qq.com> <http://www.ikcms.cn>  修改时间：2014年3月17日 晚22:39
// +----------------------------------------------------------------------
...
class UcenterUserModel extends Model{
	...
	protected $_validate = array(
		...
		/* 验证个性域名 */
		array('doname', '1,20', -12, self::EXISTS_VALIDATE, 'length'), //个性域名长度不合法
		array('doname', 'checkDenyDoName', -13, self::EXISTS_VALIDATE, 'callback'), //个性域名禁止注册
		array('doname', '', -14, self::EXISTS_VALIDATE, 'unique'), //个性域名被占用		
	);

	/* 用户模型自动完成 */
	protected $_auto = array(
		array('password', 'ik_ucenter_md5', self::MODEL_BOTH, 'function', UC_AUTH_KEY),
	...
	/**
	 * 检测个性域名是不是被禁止注册
	 * @param  string $doname 用户名
	 * @return boolean          ture - 未禁用，false - 禁止注册
	 */
	protected function checkDenyDoName($doname){
		return true; //TODO: 暂不限制，下一个版本完善
	}
	/**
	 * 检测邮箱是不是被禁止注册
	...
	public function login($username, $password, $type = 2){ 
		...
		if(is_array($user) && !$user['status']){ 
			/* 验证用户密码 */
			if(ik_ucenter_md5($password, UC_AUTH_KEY) === $user['password']){
	...
	public function info($uid, $is_doname = false){
		...
		$user = $this->where($map)->field('id,username,email,mobile,status,doname')->find(); 
	 ...
	 * @param  integer $type   用户名类型 1-用户名，2-用户邮箱，3-用户电话，4-个性域名
	 * @return integer         错误编号
	 */
    ...
    public function checkDoname($doname) {
    	if ($this->where(array('doname'=>$doname))->count('id')) {
    		return false;
    	}
    	return true;
    }
	...
	protected function verifyUser($uid, $password_in){
		...
		if(ik_ucenter_md5($password_in, UC_AUTH_KEY) === $password){
			return true;
		}
		return false;
	}
	...
	{
		$where = "username='" . $doname . "' AND id<>'" . $id . "'";
		$result = $this->where($where)->count('id');
		if ($result) {
			return true;
		} else {
