...
class AdminController extends BackendController {
	...
	public function article_list($nameid){
		...
		foreach($arrArticleItem as $key=>$item){
			...
			$arrArticle [$key]['user'] = D('Common/User')->getOneUser($item['userid']);
	...
	public function ajax_setting(){
		...
		$type_val = I('get.type',0,'intval'); //设置
		$itemid = I('post.itemid'); //多条
		$arrid = explode(',', $itemid);
		switch ($ik) {
			case "order" :
					$field = $this->_post('field');
					$fieldval = $this->_post('fieldval');
					$result = $this->item_mod->where(array('itemid'=>$id))->setField($field,$fieldval);
					if($result){
			...
			case "istop" :
					foreach ($arrid as $item){
						$this->item_mod->where(array('itemid'=>$item))->setField('istop',$type_val);
					}	
		    		$arrJson = array('r'=>0, 'html'=> '操作成功');
		    		echo json_encode($arrJson);			
				break;
			case "isdigest" :
					foreach ($arrid as $item){
						$this->item_mod->where(array('itemid'=>$item))->setField('isdigest',$type_val);
					}	
		    		$arrJson = array('r'=>0, 'html'=> '操作成功');
		    		echo json_encode($arrJson);				
				break;
			case "isaudit" :
					foreach ($arrid as $item){
						$this->item_mod->where(array('itemid'=>$item))->setField('isaudit',$type_val);
					}	
		    		$arrJson = array('r'=>0, 'html'=> '操作成功');
		    		echo json_encode($arrJson);			
				break;								
