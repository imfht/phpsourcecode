...
class ArticleRobotsModel extends Model
{
	//更新缓存
	public function updaterobot($robotid) {
		...
		if(!empty($results)) {
			...
			$userquery = D('Common/User')->where($map)->select();		
			...
			if(!false == M('article_item')->create($arrArticleItem)){
				...
				if($newitemid>0){
					...
					if(!empty($msgarr['arrphotoid'])){
						D('Common/Images')->where(array('type'=>'article','typeid'=>'0'))->setField(array('typeid'=>$id));
					}
				}
			}
