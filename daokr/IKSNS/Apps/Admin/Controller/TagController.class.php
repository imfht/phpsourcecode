...
class TagController extends BackendController {
	...
	public function tags() {
		...
		//显示列表
		$pagesize = 20;
		$count = $this->mod->where($map)->order('tagid DESC')->count();
		$pager = $this->_pager($count, $pagesize);
		$query =  $this->mod->where($map)->order('tagid DESC')->limit($pager->firstRow.','.$pager->listRows)->select();
