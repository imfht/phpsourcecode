...
<html lang="zh-CN">
    ...
    <body>
        <div class="contain">
            <div class="left">
                <ul class="pul">
                    <li class="li-header" style='height:30px;'>&nbsp;&nbsp;<?php echo ($current_name); ?>
                        <ul class="hide-meun" >
                            <?php if(is_array($categorylist)): foreach($categorylist as $key=>$categorylist): ?><li onclick='redirenew(<?php echo ($categorylist["id"]); ?>)'><a><?php echo ($categorylist["title"]); ?></a></li><?php endforeach; endif; ?>
                        </ul>   
                        <i class="icon-list"></i>
                    </li>
                    <?php if(is_array($contentlist)): foreach($contentlist as $key=>$list): ?><li id='contentli<?php echo ($list["id"]); ?>' onclick="getcontent(<?php echo ($list["id"]); ?>)" liextra="1"  <?php if($key == 0): ?>class="twoli"<?php endif; ?> ><small><?php echo ($list["title"]); ?></small></li><?php endforeach; endif; ?>
                </ul>
...
<script type="text/javascript">
    ...
    function getcontent(id){
        $.post("<?php echo U("ajaxgetcontent");?>",{id:id},
            function(data){
                if(data){
                    $(".right").html(data);
    ...
    function redirenew(id){
        location.href = "<?php echo U("index");?>?cid="+id;
