...
class Page{
    ...
    public function show() {
        ...
        $up_page = $up_row > 0 ? '<li><a class="previous" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a></li>' : '';
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page > 0 && $this->totalPages != 1){
                    $link_page .= '<li class="active"><a hre="#">' . $page . '</a></li>';
                }
            }
        }
        ...
        $page_str = str_replace(
            array('%HEADER%', '%NOW_PAGE%', '%FIRST%','%UP_PAGE%',  '%LINK_PAGE%', '%DOWN_PAGE%','%END%', '%TOTAL_ROW%', '%TOTAL_PAGE%'),
            array($this->config['header'], $this->nowPage, $up_page , $the_first, $link_page, $the_end, $down_page,$this->totalRows, $this->totalPages),
            $this->config['theme']);
