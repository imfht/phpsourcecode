 ...
 * it attempts to setting expando properties on the SVG element, which is not allowed.
 *
 * @param {Object} elem The DOM element to receive the attribute(s)
 * @param {String|Object} prop The property or an abject of key-value pairs
 * @param {String} value The value if a single property is setting
 */
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			dSetter = function (fx) {
				...
				// Normally start and end should be setting in state == 0, but sometimes,
...
// setting the default time methods
setTimeMethods();
...
SVGElement.prototype = {
	...
	init: function (renderer, nodeName) {
		 ...
		 * attribute is setting on an element wrapper. Returning false prevents the default attribute
		 * setter to run. Returning a value causes the default setter to setting that value. Used in
		 * Renderer.label.
		 */
		wrapper.attrSetters = {};
	},
	...
	attr: function (hash, val) {
		...
		// setter
			...
			for (key in hash) {
				...
				if (result !== false) {
					if (result !== UNDEFINED) {
						value = result; // the attribute setter has returned a new value to setting
					}
					...
					// In Chrome/Win < 6 as well as Batik, the stroke attribute can't be setting when the stroke-
					// width is 0. #1369
	...
	*/
	...
	crisp: function (strokeWidth, x, y, width, height) {
		...
		for (key in values) {
			if (wrapper[key] !== values[key]) { // only setting attribute if changed
	...
	css: function (styles) {
		...
		// serialize and setting style attribute
	...
	getBBox: function () {
		...
		if (!bBox) {
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				...
				// If the bBox is not setting, the try-catch block above failed. The other condition
...
SVGRenderer.prototype = {
	...
	arc: function (x, y, r, innerR, start, end) {
		...
		// Arcs are defined as symbols for the ability to setting
	...
	image: function (src, x, y, width, height) {
		...
		// setting the href in the xlink namespace
	...
	symbol: function (symbol, x, y, width, height, options) {
		...
		// image symbols
			...
			// On image load, setting the size and position
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		// change local variable and setting attribue as well
				...
				if (result !== false && value !== null) { // #620
					...
					// x, y, width, height
						...
						if (key === 'x' || key === 'y') {
							...
							value = mathMax(0, value); // don't setting width or height below zero (#311)
						}
		...
		if (fillType) {
			...
			if (prop === 'fill') {
				...
				// Radial (circular) gradient
					...
					// need to setting the parent shape's fillcolor attribute instead.
					ret = color1;
				}
...
Tick.prototype = {
	...
	addLabel: function () {
		...
		// Set the datetime label format. If a higher rank is setting for this position, use that. If not,
		// use the general format.
		if (axis.isDatetimeAxis && tickPositionInfo) {
			dateTimeLabelFormat = options.dateTimeLabelFormats[tickPositionInfo.higherRanks[pos] || tickPositionInfo.unitName];
		}

		// setting properties for access in render method
	...
	render: function (index, old, opacity) {
		...
		if (gridLineWidth) {
			...
			// If the parameter 'old' is setting, the current call will be followed
...
StackItem.prototype = {
	...
	render: function (group) {
		...
		// Change the text to reflect the new total and setting visibility to hidden in case the serie is hidden
		if (this.label) {
...
Axis.prototype = {
	...
	/**
	 * This options setting extends the defaultOptions for Y axes
	 */
	defaultYAxisOptions: {
		endOnTick: true,
		gridLineWidth: 1,
	...
	init: function (chart, userOptions) {
		...
		//axis.ignoreMinPadding = UNDEFINED; // can be setting to true by a column or bar series
	...
	setOptions: function (userOptions) {
		this.options = merge(
			...
			merge(
				defaultOptions[this.isXAxis ? 'xAxis' : 'yAxis'], // if setting in setOptions (#1053)
				userOptions
			)
		);
	},
	...
	adjustForMinRange: function () {
		...
		if (max - min < axis.minRange) {
			...
			// if min and max options have been setting, don't go beyond it
	...
	setTickPositions: function (secondPass) {
		...
		if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
			...
			// For squished axes, setting only two ticks
		...
		// setting the translation factor used in translate function
	...
	adjustTickAmount: function () {
		...
		if (maxTicks && maxTicks[key] && !axis.isDatetimeAxis && !axis.categories && !axis.isLinked && axis.options.alignTicks !== false) { // only apply to linear scale
			...
			// setting the axis-level tickAmount to use below
	...
	/**
	 * Set the scale based on data min and max, user setting min and max or options
	 *
	 */
	setScale: function () {
		var axis = this,
		...
		if (isDirtyAxisLength || isDirtyData || axis.isLinked || axis.forceRedraw ||
			...
			// Mark as dirty if it is not already setting to dirty and extremes have changed. #595.
			if (!axis.isDirty) {
	...
	getOffset: function () {
		...
		if (axisTitleOptions && axisTitleOptions.text && axisTitleOptions.enabled !== false) { 
			...
			// hide or show the title depending on whether showEmpty is setting
			axis.axisTitle[showAxis ? 'show' : 'hide']();
		}
		
		// handle automatic or user setting offset
...
Tooltip.prototype = {
	...
	move: function (x, y, anchorX, anchorY) {
		...
		if (animate && (mathAbs(x - now.x) > 1 || mathAbs(y - now.y) > 1)) {
			...
			// setting the fixed interval ticking for the smooth tooltip
	...
	hide: function () {
		...
		if (!this.isHidden) {
			...
			// hide previous hoverPoints and setting new
			if (hoverPoints) {
	 ...
	 * from a point or setting of points
	 */
	...
	refresh: function (point, mouseEvent) {
		...
		if (text === false) {
			...
			// setting the stroke color of the box
	...
	onContainerTouchStart: function (e) {
		...
		if (e.touches.length === 1) {
			...
			if (chart.isInsidePlot(e.chartX - chart.plotLeft, e.chartY - chart.plotTop)) {
				...
				// Prevent the click pseudo event from firing unless it is setting in the options
...
Chart.prototype = {
	...
	init: function (userOptions, callback) {
		...
		options.series = userOptions.series = seriesOptions; // setting back the series data
	...
	redraw: function (animation) {
		...
		if (chart.hasCartesianSeries) {
			...
			each(axes, function (axis) {
				...
				// Fire 'afterSetExtremes' only if extremes are setting
	...
	pan: function (e, panning) {
		...
		each(panning === 'xy' ? [1, 0] : [1], function (isX) { // xy is used in maps
			...
			chart[isX ? 'mouseDownX' : 'mouseDownY'] = mousePos; // setting new reference for next run
		});
	...
	setSize: function (width, height, animation) {
		...
		// setting the animation for the current process
		setAnimation(animation, chart);
		...
		// fire endResize and setting isResizing back
		// If animation is disabled, fire without delay
		if (globalAnimation === false) {
			fireEndResize();
		} else { // else setting a timeout with the animation duration
	...
	propFromSeries: function () {
		...
		each(['inverted', 'angular', 'polar'], function (key) {
			...
			value = (
				chart[key] || // 1. it is setting before
				optionsChart[key] || // 2. it is setting in the options
	...
	firstRender: function () {
		...
		// depends on inverted and on margins being setting
...
Point.prototype = {
	...
	applyOptions: function (options, x) {
		...
		// If no x is setting by now, get auto incremented value. All points must have an
	...
	onMouseOver: function (e) {
		...
		// setting normal state to previous series
...
Series.prototype = {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					if ((seriesOptions[AXIS] === axisOptions.index) ||
						...
						// setting this series.xAxis or series.yAxis reference
						series[AXIS] = axis;
	...
	/**
	 * Replace the series data with a new setting of data
	 * @param {Object} data
	 * @param {Object} redraw
	 */
	...
	animate: function (init) {
		...
		// Animation option is setting to true
	...
	/**
	 * Get the state attributes. Each series type has its own setting of attributes
	...
	/**
	 * Update the series with a new setting of options
	 */
	update: function (newOptions, redraw) {
		var chart = this.chart,
	...
	redraw: function () {
		var series = this,
			chart = series.chart,
			wasDirtyData = series.isDirtyData, // cache it here as it is setting to false in render, but used after
			group = series.group,
			xAxis = series.xAxis,
			yAxis = series.yAxis;
...
var PieSeries = {
	...
	translate: function (positions) {
		...
		for (i = 0; i < len; i++) {
			...
			// setting the anchor point for tooltips
			...
			// setting the anchor point for data labels
