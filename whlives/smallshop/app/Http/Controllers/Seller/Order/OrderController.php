...
class OrderController extends BaseController
{
    ...
    public function detail(Request $request)
    {
        ...
        $data = array();
        if ($id) {
            $order = Order::where(['id' => $id, 'seller_id' => $seller_id])->first();
            if (!$order) {
    ...
    public function getPrice(Request $request)
    {
        ...
        $id = (int)$request->input('id');
        if ($id) {
            $data = Order::select('id', 'sell_price_total', 'promotion_price', 'discount_price', 'delivery_price_real', 'subtotal')->where(['id' => $id, 'seller_id' => $seller_id])->first();
        }
        if (!$data) {
            api_error(__('admin.content_is_empty'));
        }
    ...
    public function updatePrice(Request $request)
    {
        ...
        $res = OrderService::updatePrice($order, $discount_price, $delivery_price_real, $admin_user, OrderLog::USER_TYPE_SELLER);
        if ($res === true) {
    ...
    public function getAddress(Request $request)
    {
        ...
        $id = (int)$request->input('id');
        if ($id) {
            $data = Order::select('id', 'full_name', 'tel', 'prov', 'city', 'area', 'address')->where(['id' => $id, 'seller_id' => $seller_id])->first();
        }
        if (!$data) {
            api_error(__('admin.content_is_empty'));
        }
    ...
    public function cancel(Request $request)
    {
        ...
        $res = OrderService::cancel($order, $admin_user, OrderLog::USER_TYPE_SELLER, $note);
        if ($res === true) {
            return $this->success(true);
        } else {
            api_error($res);
    ...
    public function delivery(Request $request)
    {
        ...
        $res = OrderService::delivery($order, $order_goods_id, $company_id, $code, $admin_user, OrderLog::USER_TYPE_SELLER, $note);
        if ($res === true) {
            return $this->success(true);
        } else {
            api_error($res);
    ...
    public function batchDeliverySubmit(Request $request)
    {
        ...
        $address = SellerAddress::where(['id' => $address_id, 'seller_id' => $seller_id])->first();
        if (!$address) {
        ...
        foreach ($res_list->toArray() as $value) {
            ...
            if ($kdniao_delivery['status']) {
                //电子面单获取成功
                OrderService::apiDelivery($value, $express_company, $kdniao_delivery, $admin_user, OrderLog::USER_TYPE_SELLER);
            } else {
