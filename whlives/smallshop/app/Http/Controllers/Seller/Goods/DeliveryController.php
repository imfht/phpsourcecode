...
class DeliveryController extends BaseController
{
    ...
    public function detail(Request $request)
    {
        ...
        if ($id) {
            $data = Delivery::where(['id' => $id, 'seller_id' => $seller_id])->first();
            //分组地区信息
    ...
    public function save(Request $request)
    {
        ...
        $id = (int)$request->input('id');
        if ($id) {
            $res = Delivery::where(['id' => $id, 'seller_id' => $seller_id])->update($save_data);
        } else {
    ...
    public function status(Request $request)
    {
        ...
        if ($ids && isset($status)) {
            $res = Delivery::where('seller_id', $seller_id)->whereIn('id', $ids)->update(['status' => $status]);
            if ($res) {
                return $this->success();
            } else {
    ...
    public function delete(Request $request)
    {
        $seller_id = $this->getUserId();
        $ids = $this->checkBatchId();
        $res = Delivery::where('seller_id', $seller_id)->whereIn('id', $ids)->delete();
        if ($res) {
            return $this->success();
        } else {
    ...
    public function fieldUpdate(Request $request)
    {
        ...
        if (in_array($field, $field_arr)) {
            ...
            if ($id && $field && $field_value) {
                $res = Delivery::where(['id' => $id, 'seller_id', $seller_id])->update([$field => $field_value]);
                if ($res) {
                    return $this->success();
                } else {
                    api_error(__('admin.fail'));
