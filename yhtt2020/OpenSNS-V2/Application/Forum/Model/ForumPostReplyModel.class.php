...
class ForumPostReplyModel extends Model
{
     ...
     * @auth qhy
     */
    public function getMyForum($uid){
        $forum_id=$this->where(array('uid'=>$uid))->getField('post_id');
        $forum=D('forum_post')->where(array('id'=>$forum_id))->select();
        foreach ($forum as &$val){
            $val['create_time']=friendlyDate($val['create_time']);
            $val['forum_title']=D('forum')->where(array('id'=>$val['forum_id']))->getField('title');
            $val['user']=query_user(array('uid', 'avatar32', 'avatar64', 'avatar128', 'space_url', 'nickname'),$val['uid']);
        }
        unset($val);
        return $forum;
    }
