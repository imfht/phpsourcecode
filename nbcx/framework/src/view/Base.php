...
class Base extends Driver {
    ...
    // 引擎配置
    protected $config = [
        'view_current' => '', //设置当前控制器直接使用的模版的路径，不对模版里的include，layout起作用
        'view_path' => '', // 模板路径
        'view_base' => '',
    ...
    public function __construct(array $config = []) {
        ...
        //设置默认模版路径
        //$module = Router::ins()->module;
        //$root = $module?Config::$o->module.'/'.$module:Config::$o->app;
        //$this->config['view_path']  = __APP__.$root.DS.'view'.DS;

        //默认模版缓存路径
        $this->config['cache_path'] = Config::$o->path_temp . 'view/';
    ...
    public function display($template='', $vars = [], $config = []) {
        ...
        $template = $this->parseTemplateFile($template,$this->config['view_current']);

        if ($template) {
            $cacheName = ($this->config['layout_on']?$this->config['layout_name'].'_':'').md5($template);

            $cacheFile = $this->config['cache_path'] . $this->config['cache_prefix'] . $cacheName . '.' . ltrim($this->config['cache_suffix'], '.');
            if (!$this->checkCache($cacheFile)) {
                // 缓存无效 重新模板编译
                $content = file_get_contents($template);
                $this->compiler($content, $cacheFile);
     ...
     * @return void
     */
    public function display_bak($content='', $vars = [], $config = []) {
        if ($vars) {
            $this->data = $vars;
        }
        if ($config) {
     ...
     * @return void|array
     */
    protected function write($cacheFile, $content) {
        // 检测模板目录
     ...
     * @return void
     */
    protected function read($cacheFile, $vars = []) {
     ...
     * @return boolean
     */
    protected function check($cacheFile, $cacheTime) {
        // 缓存文件不存在, 直接返回false
    ...
    private function compiler(&$content, $cacheFile) {
        ...
        // 添加安全代码及模板引用记录
        $content = '<?php if (!defined(\'__APP__\')) exit(); /*' . serialize($this->includeFile) . '*/ ?>' . "\n" . $content;
        // 编译存储
    ...
    public function parseTagLib($tagLib, &$content, $hide = false) {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = 'nb\\view\\tag\\' . ucwords($tagLib);
        }
        /** @var Taglib $tLib */
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        elseif ('$Request' == $first) {
                            ...
                            $parseStr = '\nb\Request::driver()->' . $method;// . '(' . $params . ')';
                        }
                        else {
    ...
    /**
     * 特殊模板变量解析
     * 格式 以 $NB. 打头的变量属于特殊模板变量
     * @access public
     * @param  array $vars 变量数组
     * @return string
     */
    public function parseNBVar($vars) {
        $type = strtoupper(trim(array_shift($vars)));
        $param = implode('.', $vars);
        if ($vars) {
            switch ($type) {
                case 'SERVER':
                    $parseStr = '\\nb\\Request::driver()->server[\'' . $param . '\']';
                    break;
                case 'GET':
                    $parseStr = '\\nb\\Request::driver()->get[\'' . $param . '\']';
                    break;
                case 'POST':
                    $parseStr = '\\nb\\Request::driver()->post[\'' . $param . '\']';
                    break;
                case 'COOKIE':
                    $parseStr = '\\nb\\Cookie::get(\'' . $param . '\')';
                    break;
                case 'SESSION':
                    ...
                    break;
                case 'REQUEST':
                    $parseStr = '\\nb\\Request::driver()->request[\'' . $param . '\']';
                    break;
                case 'CONST':
     ...
     * @return string|false
     */
    private function parseTemplateFile($template,$current_path=null) {
        ...
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            if (strpos($template, '@') !== false) {
                ...
                $module || $module = $conf->folder_app;
            }

            $router = Router::driver();
            $module = isset($module) ? $module : $router->module;

            if($module && $module !== $conf->folder_app) {
                $module = $conf->folder_module . DS . $module;
            }

            if($current_path) {
                switch ($current_path) {
                    case '..':
                        $path = $module ? __APP__ . $module . DS : __APP__ . $conf->folder_app . DS;
                        break;
                    default :
                        $path = $current_path;
                        break;
                ...
                if ($this->config['view_base']) {
                    ...
                    //如果没有设置模板base目录，则使用view_path值，
                    //且如果包含模块，则在模板路径view_path前加上$module名字
                    $path = $module ? __APP__ . $module . DS . 'view' . DS : $this->config['view_path'];
                }

                if(!$path) {
                    $path = __APP__ . $conf->folder_app . DS . 'view' . DS;
                }
            }

            if ('' == $template) {
                // 如果模板文件名为空 按照默认规则定位
                $template = strtolower($router->controller) . DS . strtolower($router->function);// . '.' . $this->config['view_suffix'];
            }
