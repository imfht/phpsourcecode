 ...
 */
namespace nb\view\tag;

/**
 * 核心标签库解析类
 */
class Cx extends Driver {
    ...
    public function tagVolist($tag, $content) {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \Traversable): $' . $key . ' = 0;';
        // 设置了输出数组长度
    ...
    public function tagForeach($tag, $content) {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \Traversable): ';
        // 设置了输出数组长度
        if (0 != $offset || 'null' != $length) {
            if (!isset($var)) {
                $var = '$_' . uniqid();
    ...
    public function tagEmpty($tag, $content) {
        $name = $tag['name'];
        $name = $this->autoBuildVar($name);
        $parseStr = '<?php if(empty(' . $name . ') || (' . $name . ' instanceof Traversable && ' . $name . '->isEmpty())): ?>' . $content . '<?php endif; ?>';
        return $parseStr;
    }
