...
class content_form {
    ...
    function __construct($modelid,$catid = 0,$categorys = array()) {
        ...
        //取出fields
        $this->fields = D('ModelField')->getFieldsByModelid($modelid);
    ...
    function get($data = array()) {
        ...
        foreach($this->fields as $field=>$v) {
            ...
            $func = $v['formtype'];
            //还未完成的功能， 暂时不显示
            // if(in_array($field, array('posids', 'pages', 'voteid', 'islink', 'copyfrom', 'allow_comment', 'relation', 'readpoint', 'groupids_view', 'template')))
            //     continue;
            if(in_array($func, array('pages', 'omnipotent', 'islink', 'copyfrom', 'box', 'readpoint', 'groupid', 'template')))
                continue;
    ...
    function editor($field, $value, $fieldinfo) {
        ...
        $value = htmlspecialchars_decode($value);
        if($minlength || $pattern) $allow_empty = '';
        if($minlength) $data_rule = 'data-rule="required"';
        // return "<div id='{$field}_tip'></div>".'<textarea name="info['.$field.']" id="'.$field.'" boxid="'.$field.'">'.$value.'</textarea>';
        return "<div id='{$field}_tip'></div>".'<textarea name="info['.$field.']" id="'.$field.'" style="" data-toggle="kindeditor" '.$data_rule.' data-minheight="200" data-uploadjson="'.U('Attachment/uploadJson').'" data-filemanagerjson="'.U('Attachment/fileManagerJson').'">'.$value.'</textarea>';
    }
    ...
    function image($field, $value, $fieldinfo) {
        $setting = string2array($fieldinfo['setting']);
        extract($setting);
        $preview_img = $value ? $value : '/Public/images/upload-pic.png';
        return "<div class='upload-pic img-wrap'><input type='hidden' name='info[$field]' id='$field' value='$value'/>
        <a href='javascript:void(0);' onclick=\"flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',thumb_images,'1,{$upload_allowext},$isselectimage,$images_width,$images_height,$watermark','content','$this->catid','$authkey');return false;\"><img src='$preview_img' id='{$field}_preview' width='135' height='113' style='cursor:hand'/></a>
        <input type='button' id='a_thumb_clear' value='".L('cancel_the_picture')."' /></div>";
    }
    function images($field, $value, $fieldinfo) {
        ...
        $string .= $str."<div class='btn btn-default'><a herf='javascript:void(0);' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',change_images,'{$upload_number},{$upload_allowext},{$isselectimage}','content','$this->catid','{$authkey}')\"> ".L('select_pic')." </a></div>";
        return $string;
    }   
    ...
    function posid($field, $value, $fieldinfo) {
        $setting = string2array($fieldinfo['setting']);
        $position = D('Position')->getAllPosition();
        ...
        if( ACTION_NAME == 'edit') {
            $map['id'] = $this->id;
            $map['modelid'] = $this->modelid;
            $result = M('position_data')->where($map)->field('posid')->index('posid')->select();
