...
class image {
    ...
    function __construct($thumb_enable = 0 ,$siteid = 1) {
    	...
    	/*$this->site_setting = array (
			  'upload_maxsize' => '2048',
			  'upload_allowext' => 'jpg|jpeg|gif|bmp|png|doc|docx|xls|xlsx|ppt|pptx|pdf|txt|rar|zip|swf',
			  'watermark_enable' => '1',
			  'watermark_minwidth' => '200',
			  'watermark_minheight' => '200',
			  'watermark_img' => './Public/images/water/bango.png',
			  'watermark_pct' => '32',
			  'watermark_quality' => '80',
			  'watermark_pos' => '5',
			);*/
    	$siteinfo = D('Site')->getSetting();
    	$this->site_setting = $siteinfo['setting'];
    ...
    function thumb($image, $filename = '', $maxwidth = 200, $maxheight = 200, $suffix='', $autocut = 0, $ftp = 0, $webp = true) {
		...
		if($webp){
			//生成webp
			imagewebp($thumbimg, str_replace($type, 'webp', $filename));
		}else{
	...
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'phpcms',$w_font = 8, $w_color = '#ff0000') {
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, './Public/fonts/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
