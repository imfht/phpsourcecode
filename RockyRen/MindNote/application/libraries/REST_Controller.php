...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    protected function _detect_output_format()
    {
        ...
        //@mine:改为用$_GET也可获取format
        else if(isset($_GET['format']) && array_key_exists($_GET['format'], $this->_supported_formats)){
          return $_GET['format'];
        }
    ...
    public function get($key = NULL, $xss_clean = TRUE)
    {
        ...
        $val = array_key_exists($key, $this->_get_args) ? $this->_xss_clean($this->_get_args[$key], $xss_clean) : FALSE;


        //如果为$val,尝试在$_GET上拿
        if($val == FAlSE){
          return array_key_exists($key, $_GET) ? $this->_xss_clean($_GET[$key], $xss_clean) : FALSE;
        }else{
          return $val;
        }
    }
    ...
    public function put($key = NULL, $xss_clean = TRUE)
    {
        ...
        $val =  array_key_exists($key, $this->_put_args) ? $this->_xss_clean($this->_put_args[$key], $xss_clean) : FALSE;

        //如果在_put_args上找不到，尝试在$_POST上找
        if($val == FALSE){
          return array_key_exists($key, $_POST) ? $this->_xss_clean($_POST[$key], $xss_clean) : FALSE;
        }else{
          return $val;
        }
    }
    ...
    public function delete($key = NULL, $xss_clean = TRUE)
    {
        ...
        $val = array_key_exists($key, $this->_delete_args) ? $this->_xss_clean($this->_delete_args[$key], $xss_clean) : FALSE;

        //如果在_delete_args上找不到，就在$_POST上找
        if($val == FALSE){
          return array_key_exists($key, $_POST) ? $this->_xss_clean($_POST[$key], $xss_clean): FALSE;
        }else{
          return $val;
