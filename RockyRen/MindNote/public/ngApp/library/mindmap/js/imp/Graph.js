 ...
 */
define(['mindmapJS/imp/otherModule/DataHelper', 'mindmapJS/imp/Renderer'], function(DataHelper){
    var Node, Edge;
    ...
    Graph.prototype = {
        ...
        /**
         * 根据parentIdInfoGroup对象渲染创建Graph
         * @param parentIdInfoGroup , {'root': [ {id,name} ],  'parentId2': [ {id,name} ]}
         */
        fromJson: function(parentIdInfoGroup){
          if(!parentIdInfoGroup['root']){
            parentIdInfoGroup['root'] = [];
          }
          this.batchSetParent(this.root, parentIdInfoGroup['root'], parentIdInfoGroup);

        },

        /**
         * 逐层创建节点：根据子节点的信息列表，逐个创建子节点,然后与连接父节点,递归上述操作
         * @param parentNode
         * @param childrenInfoList
         */
        batchSetParent: function(parentNode, childrenInfoList, parentIdInfoGroup){
          var currentInfo, node, newChildrenInfoList;
          for(var i=0; i<childrenInfoList.length; i++){
            currentInfo = childrenInfoList[i];
            node = this.addNode(parentNode, { id: currentInfo.id });
            this.setLabel(node, currentInfo.name);

            newChildrenInfoList = parentIdInfoGroup[currentInfo.id];
            if(newChildrenInfoList){
              this.batchSetParent(node, newChildrenInfoList, parentIdInfoGroup);
            }
          }
        },

        /**
         * 返回parentIdInfoGroup格式的对象
         * @returns  返回格式为{'root': [ {id,name} ],  'parentId2': [ {id,name} ]}
         */
        toJson: function(){
          var parentIdInfoGroup = {};

          this.setGroupByNode('root', this.root, parentIdInfoGroup);


          return parentIdInfoGroup;

        },

        setGroupByNode: function(parentId , parentNode, parentIdInfoGroup){
          var self = this;
          if(parentNode.childrenCount() > 0){
            parentIdInfoGroup[parentId] = [];

            DataHelper.forEach(parentNode.children, function(curChild){
              parentIdInfoGroup[parentId].push({
                id: curChild.id,
                name: curChild.label
              });
              self.setGroupByNode(curChild.id, curChild, parentIdInfoGroup);
            });
          }
        },
        ...
        getMaxNodeId: function(){
          var maxId = 0,  curNodeId;
          for(var nodeIdStr in this.nodes){
            curNodeId = parseInt(nodeIdStr);
            if(maxId < curNodeId){
              maxId = curNodeId;
            }
          }
    ...
    Node = function(g, attr){
        ...
        if(attr.hasOwnProperty('id')){
            ...
            var maxNodeId = this.graph.getMaxNodeId();
            this.id = attr.id || ++maxNodeId;
        }
        ...
        //节点的文本
        this.label = attr.label || "主题" + this.id;
