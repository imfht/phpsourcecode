  ...
  <div class="box">
    ...
    <div class="ny-right">
      <?php foreach ( $category as $item ) { ?>
        <div class="newlist">
        <div class="newlist_top"><span><?= $item->title ?></span><a href="<?= Url::to('category/' . $item->slug, true) ?>" class="more">更多&gt;&gt;</a></div>
        <div class="news">
          <?php foreach ( $category_post[$item->slug] as $post ) { ?>
            <li>
              <div class="title"><a href="<?= Url::to('post/' . $post->id, true) ?>"><?= $post->title ?></a></div>
              <dl>
                <dd>发布时间: <?= date('Y-m-d', $post->published_at) ?></dd>
                <dt><a class="share-a" data-url="<?= Url::to('post/' . $post->id, true) ?>" data-title="<?= $post->title ?>">分享</a></dt>
              </dl>
            </li>
          <?php } ?>
        </div>
