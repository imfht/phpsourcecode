...
class Epub {
     ...
     * @return bool false if the image does not exists
     */
    public function AddPage($content = null, $file = null, $title = 'Untitled', $download_images = false) {
        ...
        if ($download_images) {
            $found_images = preg_match_all('/(\<img.*?src=[\'|"])(.*?)([\'|"].*?\>)/mis', $this->pages[$this->key]['content'], $image_matches);
    ...
    public function CreateEPUB() {
        ...
        foreach ($this->pages as $key => $value) {
            ...
            $value['content'] = preg_replace('/\<(script|iframe)[^>]*\>.*?\<\/(script|iframe)\>/mis', '', $value['content']);
            ...
            foreach ($this->images as $check_image_key => $check_images) {
                $value['content'] = str_replace($check_images['path'], 'images/' . basename($this->new_images[$check_image_key]['path']), $value['content']);
            }
