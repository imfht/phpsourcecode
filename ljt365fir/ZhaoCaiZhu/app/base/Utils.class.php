...
class base_Utils {
	...
	public static function getStr($str, $type = 'string', $default = '') {
		...
		switch ($type) {
			case 'string' : //字符处理
				...
				$_str = str_replace ( array("\\",'='), array("\\\\",''), $_str );
				$_str = str_replace ( "\/", '', $_str );
				break;
			case 'int' : //获取整形数据
				$_str = ( int ) $str;
	...
	public static function ssetcookie($array, $life = 0, $path = '/', $domain = '') {
		...
		if(is_array($array)||is_object($array)){
					foreach($array as $key=>$value){
						setcookie ( $key, $value, $life ? ($_SERVER['REQUEST_TIME'] + $life) : 0, $path, $domain, isset($_SERVER ['HTTPS']));
					}
		}
	}
	 ...
	 * 返回值:是正确的手机号码返回手机号码,不是返回false <br>
	 *感谢@通用 大力支持
	 */
	public static function IsMobile($Argv) {
			$RegExp = '/^1([38]|(4[01234689])|(54))\d{9}$/';
	...
	function isPhone($val) {
		if (preg_match ( "/^(0\d{2,3})-[1-9]\d{6,7}(-\d{3,})?$/", $val ))
			return true;
		return false;
	}
	function isPostcode($val) {
		if (preg_match ( "/^\d{4,6}$/", $val ))
			return true;
		return false;
	}
