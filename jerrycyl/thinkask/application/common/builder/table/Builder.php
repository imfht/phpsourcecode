            ...
            case 'enable':
                ...
                $btn_attribute = [
                    ...
                    'href'  => url(
                        $this->_module.'/'.$this->_controller.'/enable',
                        [
                            'field'   => !empty($attribute['field'])?$attribute['field']:"ids",
                            !empty($attribute['field'])?$attribute['field']:"ids"   => '__id__',
                            'table' => $table
                        ]
                    ),
                ];
                    ...
                    switch ($column['type']) {
                        ...
                        case 'switch': // 开关
                            switch ($row[$column['name']]) {
                                case '0': // 关闭
                                    $row[$column['name']] = '<label class="css-input switch switch-sm switch-primary" title="开启/关闭"><input type="checkbox" data-table="'.$this->_table_name.'" data-id="'.$row['_primary_key_value'].'" data-primarykey="'.$this->_vars['primary_key'].'" data-field="'.$column['name'].'"><span></span></label>';
                                    break;
                                case '1': // 开启
                                    $row[$column['name']] = '<label class="css-input switch switch-sm switch-primary" title="开启/关闭"><input type="checkbox" data-table="'.$this->_table_name.'" data-id="'.$row['_primary_key_value'].'" data-primarykey="'.$this->_vars['primary_key'].'" data-field="'.$column['name'].'" checked=""><span></span></label>';
                                    break;
                            }
                            break;
                        case 'status': // 状态
