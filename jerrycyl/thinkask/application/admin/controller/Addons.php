...
class Addons extends AdminBase {
	...
	public function index($refresh = 0) {
		...
		$this->assign($data);
		return $this->fetch('admin/addons/list');
	}
	...
	public function hooks() {
		...
		$this->assign($data);
		// return $this->fetch('admin/addons/index');
		return $this->fetch('admin/addons/hooks');
	}

	public function edithook() {
		$id = (int) tostr($this->request->only('id'));
		if($id){
			$hook = $this->getbase->getone('hooks',['where'=>['id'=>$id]]);
			$hook['addonss'] = explode(",", $hook['addons']);
			// show($hook);
			$this->assign('hook',$hook);
		}
		// if()
			//插件列表
		$addonsList = model('base')->getall('addons',['where'=>['status'=>1]]);
	   $this->assign('addonsList',$addonsList);
	return $this->fetch('admin/addons/edithook');
