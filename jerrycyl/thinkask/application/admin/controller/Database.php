...
class Database extends AdminBase {
	...
	public function index($type = null) {
		...
		return $this->fetch('admin/database/'.$type);
	}
	/**
	 * 优化表
	...
	public function export($tables = null, $id = null, $start = null) {
		if (IS_POST && !empty($tables) && is_array($tables)) {
			...
			//读取备份配置
			$config = array('path' => realpath($path) . DIRECTORY_SEPARATOR, 'part' => config('data_backup_part_size'), 'compress' => config('data_backup_compress'), 'level' => config('db_level'));
			//检查是否有正在执行的任务
	...
	public function import($time = 0, $part = null, $start = null) {
		if (is_numeric($time) && is_null($part) && is_null($start)) {
			...
			//获取备份文件信息
			$name  = date('Ymd-His', $time) . '-*.sql*';
			$path  = realpath(config('db_path')) . DIRECTORY_SEPARATOR . $name;
			$files = glob($path);
			$list  = array();
			...
			$db = new \Database($list[$part], array('path' => realpath(config('db_path')) . DIRECTORY_SEPARATOR, 'compress' => $list[$part][2]));
