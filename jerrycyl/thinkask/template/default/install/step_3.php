<?php require APP_PATH.'../template/default/install/header.php';?>
	<div class="main">
		<div class="step">
			<ul>
		...
		<table class="table1">
			...
			<tr>
				...
				<td><input type="text" class="text" value="thinkask" name="DB_NAME" /></td>
				<td>不存在则自动创建</td>
			</tr>
			<tr>
				<td><font class="red">*</font>&nbsp;数据库表前缀：</td>
				<td><input type="text" class="text" value="thinkask_" name="DB_PREFIX" /></td>
				<td>不推荐使用默认表前缀</td>
			</tr>
			<tr>
				<th>网站配置</th>
			...
			<tr>
				<td>网站名称：</td>
				<td><input type="text" class="text" value="thinkask问答系统" name="WEB_NAME" /></td>
				<td>&nbsp;</td>
			</tr>
			<tr>
...
function postData() {
	...
	$.post('/install/index/index?step=3',_postForm,function(data){
		if(data.status == 'error') {
			alert(data.info);
			return false;
		} else {

			window.location.href = '/install/index/index?step=4';
		}
	},'json');
}
</script>
