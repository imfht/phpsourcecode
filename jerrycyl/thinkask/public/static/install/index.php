...
switch ($step ) {
	...
	case 3:
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		    ...
		    $weburl = rtrim("http://".$_SERVER['SERVER_NAME'],'/');
	...
	case 4:
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {			
			...
			$datafile = $setting['add_test'] == 1 ? './inc/thinkask.sql' : './inc/thinkask.sql';
			...
			$content=preg_replace('/\/\*[\w\W]*?\*\//', '', $content);
       		$content=preg_replace('/-- ----------------------------[\w\W]*?-- ----------------------------/', '', $content);
			$content = str_replace(
					array('#thinkask#_','#web_url#','#web_name#','#web_style#','#web_cookie_code#',"\r"),
