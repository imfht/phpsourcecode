...
jQuery(document).ready(function() {
    // 快速编辑的url提交地址
    $.fn.editable.defaults.url = thinkask.quick_edit_url;
    // 值为空时显示的信息
    ...
    $.fn.editable.defaults.params = function (params) {
        ...
        params.validate = thinkask.validate;
        params.validate_fields = thinkask.validate_fields;
        return params;
    };
    // 提交成功时的回调函数
    ...
    // 初始化搜索
    var search_field = thinkask.search_field;
    if (search_field != '') {
    ...
    $('.table-builder .field-filter').click(function () {
        var self             = $(this),
            ...
            $_filter         = thinkask._filter,
            $_filter_content = thinkask._filter_content,
            $_field_display  = thinkask._field_display,
            $data  = {
        ...
        layer.open({
            ...
            success: function () {
                ...
                // 获取数据
                $.post(thinkask.get_filter_list, $data).success(function(res) {
                    if (1 != res.code) {
            ...
            yes: function () {
                ...
                location.href = thinkask.curr_url + '?_filter='+ $filter + '&_filter_content=' + $fields + '&_field_display=' + $field_display;
            }
        });
        return false;
    });
    ...
    $('.table-builder .switch input:checkbox').on('click', function () {
        ...
        // 发送ajax请求
        Tthinkask.loading();
        $.post(thinkask.quick_edit_url, $data).success(function(res) {
