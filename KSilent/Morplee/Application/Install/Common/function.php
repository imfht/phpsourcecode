...
function write_config($config, $auth){
    if(is_array($config)){
        ...
        $db = str_replace('[AUTH_KEY]', $auth, $db);
        ...
        if(!IS_WRITE){
            ...
            <p>'.realpath(APP_PATH).'/Db/Conf/config.php</p>
            ...
            if(file_put_contents(APP_PATH . 'Common/Conf/config.php', $conf) &&
               ...
               file_put_contents(APP_PATH . 'Db/Conf/config.php', $db)){
                show_msg('配置文件写入成功');
            } else {
...
function register_administrator($db, $prefix, $admin, $auth){
    ...
    $sql = str_replace(
        array('[NAME]', '[PASS]', '[EMAIL]', '[TIME]'),
        array($admin['username'], $password, $admin['email'], NOW_TIME),
        $sql);
    //执行sql
    if($db->execute($sql)){
        show_msg('创始人帐号注册完成！');
...
function update_tables($db, $prefix = ''){
    ...
    //替换表前缀
    $sql = str_replace(" `morplee_", " `{$prefix}", $sql);
