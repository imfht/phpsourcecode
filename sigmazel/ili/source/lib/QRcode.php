...
class QRtools{
    ...
    public static function log($outfile, $err)
    {
        if (QR_LOG_DIR !== false) {
            if ($err != '') {
                if ($outfile !== false) {
                    file_put_contents(ROOTPATH . "/_cache/qr-{$outfile}-error.log", date('Y-m-d H:i:s') . ': ' . $err, FILE_APPEND);
                } else {
                    file_put_contents(ROOTPATH . "/_cache/qr-error.log", date('Y-m-d H:i:s') . ': ' . $err, FILE_APPEND);
                }
            }
        }
    }
...
class QRimage{
    ...
    public static function png($frame, $filename = false, $pixelPerPoint = 4, $outerFrame = 4, $saveandprint = false, $color = array(0, 0, 0))
    {
        ...
        if ($filename === false) {
            ...
            if ($saveandprint === true) {
                ...
                imagepng($image, ! $filename ? null : $filename);
            }
        }
    ...
    public static function jpg($frame, $filename = false, $pixelPerPoint = 8, $outerFrame = 4, $q = 85, $color = array(0, 0, 0))
    {
        ...
        if ($filename === false) {
            ...
            imagejpeg($image, ! $filename ? null : $filename, $q);
        }
...
class QRcode{
    ...
    public static function png($text, $outfile = false, $level = QR_ECLEVEL_L, $size = 3, $margin = 4, $saveandprint = false, $color = array(0, 0, 0))
    {
        $enc = QRencode::factory($level, $size, $margin);
        return $enc->encodePNG($text, $outfile, $saveandprint, $color);
    }
...
class QRencode{
 ...
 // not supported yet
    ...
    public function encodePNG($intext, $outfile = false, $saveandprint = false, $color = array(0, 0, 0))
    {
        global $config;
        ...
        try {
            ...
            if ($config['qrcode'] == 'jpg')
                QRimage::jpg($tab, $outfile, min(max(1, $this->size), $maxSize), $this->margin, 85, $color);
            else
