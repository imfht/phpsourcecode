...
class Theme extends Model
{
    ...
    public function setTheme($name,$item='all')
    {
        ...
        if($item == 'pc'){
            if (Db::name('Config')->where(array('name' => '_THEME_NOW_THEME'))->count()) {
                $res = Db::name('Config')->where(array('name' => '_THEME_NOW_THEME'))->setField('value', $name);
            } else {
                $config['name'] = '_THEME_NOW_THEME';
                $res = Db::name('Config')->add($config);
            }
        }
