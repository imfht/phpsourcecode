...
class AllMyExpressionModel extends Model
{
    ...
    public  function _initialize()
    {
        ...
        $this->ROOT_PATH = str_replace('/Application/Common/Model/AllMyExpressionModel.class.php', '', str_replace('\\', '/', __FILE__));
    }
    ...
    public function getExpression($pkg){
        ...
        if($pkg == 'miniblog'){
            $filepath =  "/Application/Common/Static/images/expression/" . $pkg;
    ...
    public function getPkgList($checkStatus = 1){
        ...
        if(!$checkStatus || in_array('miniblog',$config)){
            ...
            $pkg['miniblog']['count'] = $this->getCount($this->ROOT_PATH . '/Application/Common/Static/images/Expression/miniblog');
        }
        $exppkg=M('expression_pkg');
    ...
    public function getPkgInfo($pkg ='miniblog'){
        ...
        if($pkg =='miniblog'){
            ...
            $result['count'] = $this->getCount($this->ROOT_PATH . '/Application/Common/Static/images/Expression/miniblog');
        }else{
            if($pkg=='mycollection'){
                $result['title'] ='我的收藏';
                $result['name'] = 'face';
