...
class Wechat
{
    ...
    const CONVERT_TO_OPENID_URL 	= '/user/convert_to_openid?';
    const CONVERT_TO_USERID_URL 	= '/user/convert_to_userid?';
    const SHAKEAROUND_URL               = '/shakearound/getshakeinfo?';
    ...
    public function __construct($options)
    {
        ...
        $this->access_token = isset($options['access_token'])?$options['access_token']:false;
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function getDepartment($id){
        if (!$this->access_token) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::DEPARTMENT_LIST_URL.'access_token='.$this->access_token.($id?('&id='.$id):''));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    #获取和应用相对的openid和appid用于红包发送
    public function getOpenidAndAppidByUserid($userid){
        if (!$this->access_token) return false;
        $data=array(
            'userid'=>$userid,
            'agentid'=>$this->agentid
        );
        $result = $this->http_post(self::API_URL_PREFIX.self::CONVERT_TO_OPENID_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function getUseridByOpenid($openid){
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::CONVERT_TO_USERID_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    /*摇一摇接口*/
    public function getShakeData($ticket){
        if (!$this->access_token) return false;
        $data=array(
            'ticket'=>$ticket
        );
        $result = $this->http_post(self::API_URL_PREFIX.self::SHAKEAROUND_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function getLoginurl($data){
        if (!$this->access_token) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::GET_LOGIN_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode']) || $json['errcode']!=0) {
    ...
    public function setAgent($data){
        if (!$this->access_token) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::AGENT_SET.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
