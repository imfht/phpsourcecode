 ...
 *  @author  dodge <dodgepudding@gmail.com>,UCT <admin@uctoo.com>
...
class WechatOpen
{
	...
	private $appid;                            //公众号或小程序的
    private $appsecret;                        //公众号或小程序的，授权到第三方平台后，一般此值为空
    private $authorizer_refresh_token;         //公众号或小程序的刷新令牌，只会在授权时刻提供，请妥善保存。
    private $component_appid;                  //第三方平台通过自己的，初始化时必填
    private $component_appsecret;              //第三方平台通过自己的，初始化时必填
	private $access_token;             //即authorizer_access_token，公众号或小程序的接口调用凭据,应用维护各授权公众号的access_token以方法参数传入SDK
    private $component_access_token;           //第三方平台自己的接口调用凭据,第三方最好自行保存，初始化时必填，并在收到component_verify_ticket消息时及时更新
    private $pre_auth_code;                    //第三方平台预授权码
    ...
    /**
     * 设置sdk的appid,sdk自动初始化后一般appid还未知，需要开发人员在业务逻辑代码中设置到sdk
     */
	...
	*/
	...
	public function reply($msg=array(),$return = false)
	{
		...
		if ($this->encrypt_type == 'aes') { //如果来源消息为加密方式
		    $pc = new Prpcrypt($this->encodingAesKey);
		    $array = $pc->encrypt($xmldata, $this->component_appid);//加密用开放平台appid
		    $ret = $array[0];
		    if ($ret != 0) {
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	/**
	 * 获取access_token，5、获取（刷新）授权公众号或小程序的接口调用凭据（令牌）
	...
	public function getMenu(){
		...
		//https://api.weixin.qq.com/cgi-bin/menu/get?access_token=o3hMi34QSCEIIfpFyedCTYAXPogVy67ZyMKyNCEgtBgsJE7QSaRVE2k0JNKRfLGuGD46F3p6egVaDL0qMMAgTsKDSSvt2eooI1G4ZCZf7M52c8s2ALdzIOFMlzFGzLznDTCgAFALBK
...
//		$result = $this->http_get(self::API_BASE_URL_PREFIX.self::OAUTH_TOKEN_URL.'appid='.$this->appid.'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');
     ...
     * }
     */
    public function getWxappSession($js_code,$appid=''){
        if (!$appid ) {
            $appid = $this->appid;
        }
     ...
     * 微信小程序修改服务器地址
     *
     * @param string $access_token      //请使用第三方平台获取到的该小程序授权的authorizer_access_token
     * @param string $action            //add添加, delete删除, set覆盖, get获取。当参数是get时不需要填四个域名字段。
     * @param string $requestdomain     //合法域名，当action参数是get时不需要此字段。最多包含三个元素的数组
     * @param string $wsrequestdomain   //合法域名，当action参数是get时不需要此字段。注意是wss协议
     * @return boolean|array
     * 成功返回结果
     * {
     *   "errcode": 0,
     ...
     * }
     */
    public function wxaModifyDomain($appid ,$authorizer_refresh_token,$action,$requestdomain='',$wsrequestdomain='',$uploaddomain='',$downloaddomain=''){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
     ...
     * }
     */
    public function wxaBindTester($appid ,$authorizer_refresh_token, $wechatid){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_BIND_TESTER.'access_token='.$this->access_token, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function wxaUnbindTester($appid ,$authorizer_refresh_token, $wechatid){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        if (!$wechatid) return false;
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_UNBIND_TESTER.'access_token='.$this->access_token, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function wxaCommit($appid ,$authorizer_refresh_token, $template_id, $ext_json, $user_version,$user_desc ){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = array(
    ...
    /**
     * 2、获取体验小程序的体验二维码（仅供第三方开发者代小程序调用）
     *
     * @param string $access_token      //请使用第三方平台获取到的该小程序授权的authorizer_access_token
     * @return boolean|array
     * 成功返回结果
     ...
     * }
     */
    public function wxaGetQrcode($appid ,$authorizer_refresh_token){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;

        $result = $this->http_get(self::API_BASE_URL_PREFIX.self::WXAPP_GET_QRCODE.'access_token='.$this->access_token);
        if ($result)
        {
            return $result;
        }
     ...
     * }
     */
    public function wxaGetCategory($appid ,$authorizer_refresh_token){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX.self::WXAPP_GET_CATEGORY.'access_token='.$this->access_token);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function wxaGetPage($appid ,$authorizer_refresh_token){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;

        $result = $this->http_get(self::API_BASE_URL_PREFIX.self::WXAPP_GET_PAGE.'access_token='.$this->access_token);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     *
     * @param string $access_token      //请使用第三方平台获取到的该小程序授权的authorizer_access_token
     * @param string $data       //提交审核项的一个列表（至少填写1项，至多填写5项），具体参考文档https://open.weixin.qq.com/cgi-bin/showdocument?action=dir_list&t=resource/res_list&verify=1&id=open1489140610_Uavc4&token=&lang=zh_CN
     * @return boolean|array
     * 成功返回结果
     * {
     *   "errcode": 0,
     ...
     * }
     */
    public function wxaSubmitAudit($appid ,$authorizer_refresh_token, $item_list ){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        $data = $item_list;
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_SUBMIT_AUDIT.'access_token='.$this->access_token, $data);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function wxaGetAuditstatus($appid ,$authorizer_refresh_token, $auditid ){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_GET_AUDITSTATUS.'access_token='.$this->access_token, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function wxaGetLatestAuditstatus($appid ,$authorizer_refresh_token){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;

        $result = $this->http_get(self::API_BASE_URL_PREFIX.self::WXAPP_GET_LATESTAUDITSTATUS.'access_token='.$this->access_token);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function wxaRelease($appid ,$authorizer_refresh_token){
        if (!$this->access_token && !$this->checkAuth($appid ,$authorizer_refresh_token)) return false;

        $data = array();

        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_RELEASE.'access_token='.$this->access_token, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 获取第三方平台component_access_token,SDK用缓存维护了component_access_token，应用系统也可以保存在数据库
     *
     * @param string $component_verify_ticket    //在第三方平台创建审核通过后，微信服务器会向其“授权事件接收URL”每隔10分钟定时推送component_verify_ticket。
     * @return boolean|array
     * 成功返回结果
     * {
    ...
    public function getComponentAccessToken($component_verify_ticket){
        ...
        $componentAT = 'component_access_token'.$this->component_appid;
        if ($rs = $this->getCache($componentAT)) {
            $this->access_token = $rs;
            return $rs;
        }
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::WXOPEN_COMPONENT_ACCESS_TOKEN, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
            ...
            $this->setCache($componentAT,$this->component_access_token,$expire);
    ...
    public function getPreAuthCode(){
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::WXOPEN_COMPONENT_PREAUTHCODE.'component_access_token='.$this->component_access_token, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (isset($json['errcode']) && $json['errcode']) {
    ...
    public function clearQuota($appid=''){
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::WXAPP_SESSION_URL.'access_token='.$this->access_token, self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
 ...
 * */
    public function decrypt($timeStamp,$nonce,$msg_sign,$text){
        ...
        $encodingAesKey = config('ENCODINGAES_KEY');
        $token = APP_TOKEN;
