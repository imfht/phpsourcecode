...
class Title
{
    ...
    public function getCurrentTitleInfo($uid)
    {
        ...
        if (array_key_exists("before_level_need", $data)) {
            $data['before_level_need'] = $data['before_level_need'];
        }else{
            $data['before_level_need'] = 0;
        }
        if($data['upgrade_require']==0 && $data['before_level_need'] == 0){
            $data['upgrade_require']=1;
        }
