...
class File
{
    ...
    public function uploadPictureBase64($data = '')
    {
        if(empty($data)){
            $aData = $_POST['data'];
        }else{
        ...
        if ($check) {
            ...
            return ['status'=>1,'id'=>$return['id'],'path'=> $return['path']];
        } else {
            //不存在则上传并返回信息
            $driver = config('PICTURE_UPLOAD_DRIVER','local','config');
            $driver = check_driver_is_exist($driver);
            ...
            if ($rs) {
               ...
               return ['status'=>1,'id' => $id, 'path' => render_picture_path($path)];
            } else {
                return ['status'=>0,'图片上传失败。'];
            }
        }
    }
