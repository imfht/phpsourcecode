...
class Count extends Model
{
    ...
    {
        parent::initialize();
        $this->lostModel=db('CountLost');
        $this->remainModel=db('CountRemain');
        $this->consumptionModel=db('CountConsumption');
        $this->activeModel=db('CountActive');
    }
    ...
    private function _doRemainCount($date,$day=1)
    {
        ...
        if(count($regUids)){
            ...
            if($login_action_id===false){
                $login_action_id=db('Action')->where(array('name'=>'user_login','status'=>1))->getField('id');
            ...
            $loginUids=db('ActionLog')->where($map_login)->field('user_id')->select();
    ...
    public function activeCount()
    {
        ...
        $total_user=db('Member')->where(array('status'=>1))->count();
    ...
    private function _dayActiveCount($action,$today)
    {
        ...
        $users=db('ActionLog')->where($map)->field('user_id')->select();
    ...
    {
        $data['date']=$startTime;
        $pingxxOrderModel=db('PingxxCharge');//ping++模块消费用户统计
        $rechargeOrderModel=db('RechargeRecordAlipay');//Recharge模块消费用户统计
        $orderLinkModel=db('order_link');//订单关联记录表
