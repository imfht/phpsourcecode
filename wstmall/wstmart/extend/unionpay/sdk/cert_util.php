...
//         $cert->certId = $certId;
    ...
    {
    	if( $certPath == null ) {
    		$certPath = SDKConfig::$signCertPath;
    		$certPwd = SDKConfig::$signCertPwd;
    	}
...
// 	    $cert->certId = $certId;
    ...
    public static function verifyAndGetVerifyCert($certBase64String){
		...
		if (SDKConfig::$middleCertPath === null || SDKConfig::$rootCertPath === null){
		...
		if(strtolower(SDKConfig::$ifValidateCNName) == "true"){
			if (COMPANY != $cn){
		...
		$result = openssl_x509_checkpurpose($certBase64String, X509_PURPOSE_ANY, array(SDKConfig::$rootCertPath, SDKConfig::$middleCertPath));
		if($result === FALSE){
    ...
    private static function initVerifyCerts($cert_dir=null) {
    	...
    	if( $cert_dir == null ) {
    		$cert_dir = SDKConfig::$validateCertDir;
    	}
