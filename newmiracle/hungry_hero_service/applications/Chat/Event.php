       ...
       // 判断是不是websocket的关闭连接的包
        ...
        switch($message_data['type'])
        {
            ...
            var_dump('显示原生客户端');
                ...
                $new_message_zhuji = array('iszhuji'=>'true','type'=>$message_data['type'], 'client_id'=>$client_id, 'client_name'=>htmlspecialchars($client_name), 'client_list'=>$client_list, 'time'=>date('Y-m-d H:i:s'));
                $new_message_otherplayer = array('iszhuji'=>'false','type'=>$message_data['type'], 'client_id'=>$client_id, 'client_name'=>htmlspecialchars($client_name), 'client_list'=>$client_list, 'time'=>date('Y-m-d H:i:s'));
            ...
            var_dump('fageizhuji');
            var_dump($client_id_array[0]);

            var_dump('fasongsuoyou');
                Gateway::sendToClient($client_id_array[0], WebSocket::encode(json_encode($new_message_zhuji)));
               ...
               $client_otherplayer_array1 = array_keys($all_clients);
               unset($client_otherplayer_array1[0]);
                    var_dump('fageiqitawanjia');
                    var_dump($client_otherplayer_array1);
                Gateway::sendToAll(WebSocket::encode(json_encode($new_message_otherplayer)), $client_otherplayer_array1);
                }
                return;

            //开始游戏
            case 'getownid':
                $own_client_id = array('type'=>$message_data['type'],'ownid'=>$client_id);
                var_dump('fasongzijiid');
                return Gateway::sendToCurrentClient(WebSocket::encode(json_encode($own_client_id)));
            case 'startgame':
                ...
                var_dump('tongzhisuoyoukuhuduankaishiyouxi');
            ...
            case 'position':
                ...
                $all_clients = self::getClientListFromRoom($_SESSION['room_id']);
                $filerown_client=array();
                $client_id_array = array_keys($all_clients);
                foreach($client_id_array as $clientss){
                    if($clientss!=$client_id)
                    {
                        array_push($filerown_client,$clientss);
                    }
                }
                ...
                $new_message = array(
                    ...
                    'positionXY'=>array(
                      $client_id=>array(
                      'positionX'=>$message_data['positonX'],
                      'positionY'=>$message_data['positonY']
                       )
                    )
                );
              //如果是单机 $filerown_client为空 就代表不发送给客户端
                //如果多人就发给其他玩家不发给自己
                var_dump($new_message);
                return Gateway::sendToAll(WebSocket::encode(json_encode($new_message)), $filerown_client);
