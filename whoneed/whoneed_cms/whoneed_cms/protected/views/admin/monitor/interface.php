...
<html>
	...
	<body>
		<div class="container">
		  	<div class="row">
				<div class="col-md-3">
					...
					?>
						<li <?php echo $addClass; ?> role="presentation">
							<a href="/admin/monitor/interfaceCallStatistics?server_name=<?php echo $server_name; ?>&module_name=<?php echo $k; ?>&ip=<?php echo $_GET['ip']; ?>"><?php echo $k; ?></a>
							...
							<?php if($v){ foreach($v as $v1) { $addClass = ''; if($interface_name == $v1) $addClass = 'class="active"'; ?>
								<li <?php echo $addClass; ?> role="presentation"><a href='/admin/monitor/interfaceCallStatistics?server_name=<?php echo $server_name;?>&module_name=<?php echo $k; ?>&interface_name=<?php echo $v1; ?>&ip=<?php echo $_GET['ip']; ?>'><?php echo $v1; ?></a></li>
							<?php } } ?>
							</ul>
						</li>
					<?php } } ?>
				...
				<div class="col-md-9">
						...
						<?php
							if($code == 0 && $interface_name)
							{
								...
								for($i=13;$i>=0;$i--)
								{
									...
									$date_html	= "<a href='/admin/monitor/interfaceCallStatistics?server_name={$server_name}&module_name={$module_name}&interface_name={$interface_name}&date_time={$the_date}&ip={$ip}' class='btn' type='button'>{$date_str}</a>";
									if($i == 7) $date_html .= "<br/>";
									echo $date_html;
								}
							}
					...
					<?php 
						if($code == 0 && $monitor_data['monitor_interface'])
						{
					?>

					<div class="row clearfix">
						<div class="col-md-12 column height-400" id="main">
						</div>
					</div>
					<div class="row clearfix">
						<div class="col-md-12 column height-400" id="id_time">
						</div>
					</div>
					...
					<div class="row clearfix">
						<div class="col-md-12 column text-center" id="id_no_data" style="padding-top:100px;">
							<center>
								<h1>暂无数据</h1>
							</center>
						</div>
					...
					<?php }else{ ?>
					<div class="row clearfix">
						<div class="col-md-12 column text-center" id="id_left" style="padding-top:100px;">
							<center>
		...
		?>
			var tdata = {
				'timename'	:'2019-03-29',
				'succ'		: <?php  echo json_encode($monitor_data['monitor_interface']['succ']); ?>,
				'fail'		: <?php  echo json_encode($monitor_data['monitor_interface']['fail']); ?>,
				'succTime'	: <?php  echo json_encode($monitor_data['monitor_interface']['succTime']); ?>,
				'failTime'	: <?php  echo json_encode($monitor_data['monitor_interface']['failTime']); ?>,
			};
			...
			var time = Highcharts.chart('id_time',{
				chart: {
					type: 'spline'
				},
				title: {
