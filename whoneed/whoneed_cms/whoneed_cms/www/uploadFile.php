...
$attachDir= '../../project.cdn/upload';//上传文件保存路径，结尾不要带/
...
//            $cdn_domain = Yii::app()->params['cdn_domain'];
			rename($tempPath,$targetPath);
			@chmod($targetPath,0755);
			$targetPath=jsonString('/'.$targetPath);
			if($immediate=='1')$targetPath='!'.$targetPath;
			if($msgType==1)$msg="'$targetPath'";
			else $msg="{'url':'".'http://test.cdn.patabom.com/upload/'.$attachSubDir.'/'.$newFilename."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
		}
	}
