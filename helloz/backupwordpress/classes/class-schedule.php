	...
	protected function do_action( $action ) {
		...
		switch ( $action ) :
			...
			case 'hmbkp_mysqldump_started' :
				...
				$this->set_status( sprintf( __( '清空数据库 %s', 'backupwordpress' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );
				break;

			case 'hmbkp_mysqldump_verify_started' :

				$this->set_status( sprintf( __( '验证数据库转储 %s', 'backupwordpress' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );
				break;

			case 'hmbkp_archive_started' :

				$this->set_status( sprintf( __( '创建.zip文件 %s', 'backupwordpress' ), '(<code>' . $this->get_archive_method() . '</code>)' ) );
				break;
			...
			case 'hmbkp_backup_complete' :
				...
				$this->set_status( __( '完成备份', 'backupwordpress' ) );
				break;
