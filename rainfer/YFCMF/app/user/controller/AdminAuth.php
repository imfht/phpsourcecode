...
class AdminAuth
{
    ...
    protected $config = [
        ...
        'auth_group_access' => 'role_access', // 用户-用户组关系表
    ...
    {
        $config = config('yfcmf.user_auth.auth_config');
        if ($config) {
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups  = db()->name($this->config['auth_group_access'] . ' a')
            ->where('a.uid', $uid)
            ->where('g.status', 1)
            ->join(config('database.prefix') . "{$this->config['auth_group']} g", " a.role_id = g.id")
            ->field('uid,role_id,title,score_min,score_max,rules')->select();
    ...
    public function getAuthList($uid, $type, $mode = 'url')
    {
        ...
        if ($this->config['auth_type'] == 2 && Session::has('user_auth_list_' . $uid . '_' . $t . '_' . $mode)) {
            return Session::get('user_auth_list_' . $uid . '_' . $t . '_' . $mode);
        }
        ...
        if ($this->config['auth_type'] == 2) {
            //规则列表结果保存到session
            Session::set('user_auth_list_' . $uid . '_' . $t . '_' . $mode, $authList);
        }
