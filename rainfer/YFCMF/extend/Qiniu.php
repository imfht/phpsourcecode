...
class Qiniu
{
    ...
    public function config($config = [])
    {
        /* 获取配置 */
        $conf = [];
        if (config('upload_validate.size')) {
            $conf['max_size'] = config('upload_validate.size');
        }
        if (config('upload_validate.ext')) {
            $conf['exts'] = config('upload_validate.ext');
        }
    ...
    public function uploadOne($file_path, $prefix = "", $name = null, $ext = "", $token = null, $params = null, $mime = 'application/octet-stream', $checkCrc = false)
    {
        ...
        if (empty($name)) {
            $name = $prefix . base_convert(time() * 1000, 10, 36) . "_" . base_convert(microtime(), 10, 36) . uniqid() . ($ext ? '.' . $ext : '');
        }
        if (empty($token)) {
            $token = self::token();
        }
