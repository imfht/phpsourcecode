...
class Address extends BaseModel {
    ...
    public function getAddressInfo($condition, $order = '') {
        ...
        if (config('ds_config.delivery_isuse') && $addr_info['dlyp_id']) {
            $deliverypoint_model = model('deliverypoint');
            $dlyp_info = $deliverypoint_model->getDeliverypointOpenInfo(array(array('dlyp_id' ,'=', $addr_info['dlyp_id'])));
            if (!empty($dlyp_info)) {
    ...
    public function getAddressList($condition, $order = 'address_id desc') {
        ...
        if (empty($address_list))
            return array();
        if (config('ds_config.delivery_isuse')) {
            $dlyp_ids = array();
            $dlyp_new_list = array();
            ...
            if (!empty($dlyp_ids)) {
                $deliverypoint_model = model('deliverypoint');
                $condition = array();
                $condition[] = array('dlyp_id','in', $dlyp_ids);
