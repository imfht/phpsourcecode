...
class Sellersetting extends BaseSeller {
    ...
    public function store_mb_sliders() {
        try {
            ...
            $upload_file = BASE_UPLOAD_PATH . DIRECTORY_SEPARATOR . ATTACH_STORE . DIRECTORY_SEPARATOR . 'mobileslide';
            $file_name = session('store_id') . '_' . $file_id.'.png';
            ...
            echo json_encode(array(
                'uploadedUrl' => UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . ATTACH_STORE . DIRECTORY_SEPARATOR . 'mobileslide' . DIRECTORY_SEPARATOR . $newImg,
            ));
            die;
        } catch (\Exception $ex) {
            echo json_encode(array(
    ...
    public function store_mobile() {
        ...
        if (request()->isPost()) {
            ...
            if (!empty($_FILES['mb_title_img']['name'])) {
                $upload_file = BASE_UPLOAD_PATH . DIRECTORY_SEPARATOR . ATTACH_STORE;
                $file_name = session('store_id') . '_' . date('YmdHis') . rand(10000, 99999).'.png';
        ...
        foreach ($mbSliders as $v) {
            if ($v['img']) {
                $mbSliderUrls[] = UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . ATTACH_STORE . DIRECTORY_SEPARATOR . 'mobileslide' . DIRECTORY_SEPARATOR . $v['img'];
            }
        }
