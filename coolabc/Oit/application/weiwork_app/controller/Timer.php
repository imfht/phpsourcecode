 ...
 */
class Timer extends WeiWorkBase {
    public $config_name = 'work_timer';
    public $temp_path = TEMP_PATH . 'file' . DS;
    public $web_card_path = ROOT_PATH . 'public' . DS . 'static' . DS . 'images' . DS . 'weiwork' . DS . 'timer' . DS;
    ...
    public function vr_save_data() {
        ...
        // 检查单据明细中是否有照片资源，有的话，从企业微信中获取到照片，并保存,再清空照片资源列
        $media = $this->agent->media;
        foreach ($ebs_vr_item as $k => $v) {
            if (!empty($v['a_img_1'])) {

                $obj_id = md5(time() + rand());
                $temp_name = $obj_id . '.jpg';
                ...
                $save_file = move_file_to_oit($temp_name, $this->temp_path, $this->web_card_path, [
                    'file_type' => 'wei_timer',
                    'obj_id' => $obj_id,
                    'r_type' => 'web_name'
                ]);

                $ebs_vr_item[$k]['a_img_1'] = '';
                $ebs_vr_item[$k]['a_img_path'] = $save_file['web_path'];
                $ebs_vr_item[$k]['a_img_oit'] = $save_file['oit_obj'];
            }
        }
     ...
     * @return \think\response\Json
     */
    public function vr_get_default_row_data() {
        // 服务器时间
        $js_data['a_time'] = date('H:i:s');
    ...
    public function api() {
        $work = WeChat::agent('Timer')->server;
        echo $work->reply();
