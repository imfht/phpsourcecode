...
class Http {
	...
	static public function curlDownload($remote,$local) {
		...
		curl_setopt($cp, CURLOPT_REFERER, 'http://' . $host . '/');  
		$result = curl_exec($cp);
		curl_close($cp);
		fclose($fp);
		return $result;
	...
	static public function curl($url, $param = array(), $method = 'get', $header = false) {
		$cp = curl_init($url);
		curl_setopt($cp, CURLOPT_FILE, $fp);
		curl_setopt($cp, CURLOPT_CUSTOMREQUEST, strtoupper($method));
		curl_setopt($cp, CURLOPT_RETURNTRANSFER,1);
		curl_setopt($cp, CURLINFO_HEADER_OUT, 1);
		if ($param) {
			curl_setopt($cp, CURLOPT_POSTFIELDS, json_encode($param));
		}
