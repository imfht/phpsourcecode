...
class integrateAction extends backendAction {
    ...
    public function index() {
        $path = LIB_PATH . 'Wkcmslib/passport/';
        $opdir  = dir($path);
        $list = array();
    ...
    public function install() {
        if (IS_POST) {
            ...
            //保存到配置表
            D('global')->where(array('name'=>'integrate_code'))->setField('data', $code);
            D('global')->where(array('name'=>'integrate_config'))->setField('data', $config);
            $this->success(L('operation_success'), U('integrate/index'));
        } else {
            $code = $this->_get('code', 'trim');
            if ($code == 'default') {
                D('global')->where(array('name'=>'integrate_code'))->setField('data', $code);
                D('global')->where(array('name'=>'integrate_config'))->setField('data', '');
                $this->success(L('operation_success'), U('integrate/index'));
            } else {
                $mod = passport::uc($code);
                //安装之前环境检测
    ...
    public function setting() {
        if (IS_POST) {
            ...
            $this->assign('info', $info);
            //获取配置值
            $this->assign('icv', C('wkcms_integrate_config'));
            $this->display();
        }
    }
}