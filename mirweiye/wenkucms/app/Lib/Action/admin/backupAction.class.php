...
class backupAction extends backendAction
{
        ...
        if (IS_POST || isset($_GET['dosubmit']))
        {
            ...
            if (isset($_GET['type']) && $_GET['type'] == 'url')
            {
                $sizelimit = isset($_GET['sizelimit']) && abs(intval($_GET['sizelimit'])) ? abs(intval
                    ($_GET['sizelimit'])) : $this->ajaxReturn(0,L('input') . L('sizelimit'));
                $this->backup_name = isset($_GET['backup_name']) && trim($_GET['backup_name']) ?
                    trim($_GET['backup_name']) : $this->ajaxReturn(0,L('input') . L('backup_name'));
                $vol = $this->_get_vol();
                $vol++;
            } else
            {
                $sizelimit = isset($_POST['sizelimit']) && abs(intval($_POST['sizelimit'])) ?
                    abs(intval($_POST['sizelimit'])) : $this->ajaxReturn(0,L('input') . L('sizelimit'));

                $this->backup_name = isset($_POST['backup_name']) && trim($_POST['backup_name']) ?
                    trim($_POST['backup_name']) : $this->ajaxReturn(0,L('input') . L('backup_name'));

                $backup_tables = isset($_POST['backup_tables']) && $_POST['backup_tables'] ? $_POST['backup_tables'] :
                    $this->ajaxReturn(0,L('plsease_select') . L('backup_tables'));

                if (is_dir(MT_DATA_PATH . $this->backup_path . $this->backup_name))
                {
                    $this->ajaxReturn(0,L('backup_name') . L('exists'));
                }
                mkdir(MT_DATA_PATH . $this->backup_path . $this->backup_name);

                if (!is_file(MT_DATA_PATH . $this->backup_path . $this->backup_name .
                    '/tbl_queue.log'))
                {
                    //写入队列
            ...
            if ($tables === false)
            {
                $this->ajaxReturn(0,L('invalid_queue_file'));
            }
    ...
    public function import()
    {
        $backup_name = isset($_GET['backup']) && trim($_GET['backup']) ? trim($_GET['backup']) :
            $this->ajaxReturn(0,L('plsease_select') . L('backup_name'));
        ...
        if ($this->_import_vol($backup['file']))
        {
            if ($vol < count($backups))
            {
                ...
                $this->ajaxReturn(1,sprintf(L('bakup_import_succ'), $vol - 1), $link);
            } else
            {
                $this->ajaxReturn(1,L('restore_success'), 'index.php?g=admin&m=' . MODULE_NAME .
                    '&a=restore');
            }
        }
    }
    ...
    {
        $sql_file = MT_DATA_PATH . $this->backup_path . $this->backup_name . '/' . $sql_file_name;
    ...
    public function del_backup()
    {
        ...
        import('@.ORG.Dir'); 
        $dir=new Dir();
        $dir->delDir(MT_DATA_PATH . $this->backup_path . $_GET['backup'].'/');             
    ...
    public function download()
    {
        ...
        $sql_file = MT_DATA_PATH . $this->backup_path . $backup_name . '/' . $file;
        ...
        {
            $this->ajaxReturn(0,L('no_such_file'));
        }
    }
    ...
    private function _get_vols($backup_name)
    {
        $vols = array(); //所有的卷
        $bytes = 0;
        $vol_path = MT_DATA_PATH . $this->backup_path . $backup_name . '/';
        if (is_dir($vol_path))
        {
    ...
    {
        $backups = array(); //所有的备份
        if (is_dir(MT_DATA_PATH . $this->backup_path))
        {
            if ($handle = opendir(MT_DATA_PATH . $this->backup_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    if ($file{0} != '.' && filetype(MT_DATA_PATH . $this->backup_path . $file) == 'dir')
                    {
                        $backup['name'] = $file;
                        $backup['date'] = filemtime(MT_DATA_PATH . $this->backup_path . $file) - date('Z');
                        $backup['date_str'] = date('Y-m-d H:i:s', $backup['date']);
                        $backup['vols'] = $this->_get_vols($file);
                        $end_vol = end($backup['vols']);
                        $backup['total_size'] =round($this->_get_dir_size(MT_DATA_PATH . $this->backup_path . $file)/1024,2);
                        $backups[] = $backup;
                    }
                }
            }
    ...
    private function _deal_result($tables, $vol, $sizelimit)
    {
        ...
        if (empty($tables))
        {
            ...
            $this->ajaxReturn(1,L('bakup_succ'), 'index.php?g=admin&m=' . MODULE_NAME .
                '&a=restore');
        } else
        {
            //开始下一卷
            ...
            $link = 'index.php?g=admin&m=' . MODULE_NAME .
                ...
                $sizelimit;
            $this->ajaxReturn(1,sprintf(L('bakup_write_succ'), $vol), $link);
        }
    }
    ...
    {
        $backup_path = MT_DATA_PATH . '/data/backup/';
    ...
    {
        return file_put_contents(MT_DATA_PATH . $this->backup_path . $this->backup_name .
    ...
    {
        $tbl_queue_file = MT_DATA_PATH . $this->backup_path . $this->backup_name .
            '/tbl_queue.log';
    ...
    {
        $log_file = MT_DATA_PATH . $this->backup_path . $this->backup_name . '/vol.log';
