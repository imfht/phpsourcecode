...
class baseAction extends Action
{
    ...
    protected function _upload_init($upload) {
        $allow_max = C('wkcms_attr_allow_size'); //读取配置
        
        
        $doctype=C('wkcms_doctype');
        $doctype=str_replace('*.', '', $doctype);
        
        
        
        $arrayext1= explode(';', $doctype);
        $arrayext2=  explode(',', C('wkcms_attr_allow_exts'));
    ...
    protected function _upload($file, $dir = '', $thumb = array(), $save_rule='uniqid') {
        ...
        if ($dir) {
            $upload_path = C('wkcms_attach_path') . $dir . '/';
