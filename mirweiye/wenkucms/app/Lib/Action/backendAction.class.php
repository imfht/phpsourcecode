...
class backendAction extends baseAction {
    ...
    public function _initialize() {
        ...
        $img_dir = attach($attach, $this->_name);
    ...
    public function ajax_edit() {
        ...
        if ($field == 'score') {
            if (!is_numeric($val)) {
                $this->ajaxReturn(0, L('isnotnum'));
            }
        }
        if ($field == 'username' || $field == 'email' || $field == 'name' || $field == 'title' || $field == 'badword') {
            $map[$pk] = array('neq', $id);
            $map[$field] = $val;
            $status = $mod->where($map)->count($pk);
            if ($status) {
                $this->ajaxReturn(0, L('has_sameexits'));
            }
        }
    ...
    public function delete() {
        ...
        if ($ids) {
            if (method_exists($this, '_before_delete')) {
                $this->_before_delete($ids); //目前仅有文档删除使用了
                
            }
            if (false !== $mod->delete($ids)) {
                if ($this->_name == 'user_role') { //删除角色时删除授权，减少数据库冗余
                    $map['role_id'] = array('in', $ids);
                    D('admin_auth')->where($map)->delete();
                }
    ...
    protected function write_var($var) {
        $config_file = './public/js/var.js';
        if (is_writable($config_file)) {
            file_put_contents($config_file, $var, LOCK_EX);
            @unlink(RUNTIME_FILE);
            return true;
        } else {
            return false;
