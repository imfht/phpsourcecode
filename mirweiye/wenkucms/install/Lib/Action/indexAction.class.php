...
class indexAction extends Action {
	...
	public function index(){
		...
		$jianjie_html = file_get_contents(TMPL_PATH.'/public/jianjie.html');
	...
	public function setconf() {
		...
		if ($_POST) {
			...
			$this->assign('db_name', 'wkcms');
			$this->assign('db_prefix', 'wk_');
			$this->assign('web_md5', 'RdwsWAadWscLi');
	...
	public function finish_done() {
		...
		//添加管理员帐号
		$admin_pass = md5($temp_info['admin_pass'].$temp_info['web_md5']);
		$ctime=time();
		$sqls[] = "INSERT INTO `" . $temp_info['db_prefix'] . "user` (`uid`, `username`, `password`, `email`, `roleid`,`add_time`) VALUES " .
                "(1, '" . $temp_info['admin_user'] . "', '" . $admin_pass . "', '" . $temp_info['admin_email'] . "', 1,'". $ctime ."');";
		...
		$config_data['WEB_MD5'] = $temp_info['web_md5'];
