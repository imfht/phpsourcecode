...
class ControllerEventActivity extends Controller {
	...
	public function addCustomer(&$route, &$args, &$output) {
		if ($this->config->get('config_customer_activity')) {
			...
			$activity_data = array(
				'customer_id' => $output,
				'name'        => get_name($args[0]['firstname'], $args[0]['lastname'])
			);
			...
			if ($this->customer->isLogged()) {
				...
				if ($customer_info) {
					$activity_data = array(
						...
						'name'        => get_name($customer_info['firstname'], $customer_info['lastname'])
					);
	...
	public function addAffiliate(&$route, &$args, &$output) {
		if ($this->config->get('config_customer_activity')) {
			...
			$activity_data = array(
				'customer_id' => $output,
				'name'        => get_name($args[1]['firstname'], $args[1]['lastname'])
			);
			...
			if ($order_info && !$order_info['order_status_id'] && $args[1]) {
				...
				if ($order_info['customer_id']) {
					$activity_data = array(
						...
						'name'        => get_name($order_info['firstname'], $order_info['lastname']),
						'order_id'    => $args[0]
					);
