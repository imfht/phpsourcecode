...
class ModelExtensionPaymentPPExpress extends Model {
	...
	public function paymentRequestInfo() {
		...
		$data['PAYMENTREQUEST_0_CURRENCYCODE'] = config('payment_pp_express_currency');
		...
		foreach ($this->cart->getProducts() as $item) {
			...
			$item_price = $this->currency->format($item['price'], config('payment_pp_express_currency'), false, false);
		...
		if (!empty($this->session->data['vouchers'])) {
			foreach ($this->session->data['vouchers'] as $voucher) {
				$item_total += $this->currency->format($voucher['amount'], config('payment_pp_express_currency'), false, false);
				...
				$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($voucher['amount'], config('payment_pp_express_currency'), false, false);
				$i++;
			}
		}
		...
		foreach ($total_data['totals'] as $total_row) {
			if (!in_array($total_row['code'], array('total', 'sub_total'))) {
				if ($total_row['value'] != 0) {
					$item_price = $this->currency->format($total_row['value'], config('payment_pp_express_currency'), false, false);

					$data['L_PAYMENTREQUEST_0_NUMBER' . $i] = $total_row['code'];
					$data['L_PAYMENTREQUEST_0_NAME' . $i] = $total_row['title'];
					$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($total_row['value'], config('payment_pp_express_currency'), false, false);
					$data['L_PAYMENTREQUEST_0_QTY' . $i] = 1;
		...
		if ($recurring_products) {
			...
			foreach ($recurring_products as $item) {
				...
				if ($item['recurring']['trial']) {
					$trial_amt = $this->currency->format($this->tax->calculate($item['recurring']['trial_price'], $item['tax_class_id'], $this->config->get('config_tax')), config('payment_pp_express_currency'), false, false) * $item['quantity'] . ' ' . config('payment_pp_express_currency');
				...
				$recurring_amt = $this->currency->format($this->tax->calculate($item['recurring']['price'], $item['tax_class_id'], $this->config->get('config_tax')), config('payment_pp_express_currency'), false, false)  * $item['quantity'] . ' ' . config('payment_pp_express_currency');
