...
<div class="col col-md-12 col-sm-12 col-xs-12 col-lg-12">
                    <div class="block">
                        ...
                        <div class="caption"><?php echo ($menu["title"]); ?></div></div><div id="modelObj-action" class="block-controls"><a class="btn btn-primary btn-sm" href="<?php echo ($addUrl); ?>">新增</a>
    <a grid-btn class="btn btn-primary disabled btn-sm" href="<?php echo ($editUrl); ?>">修改</a>
    <a grid-btn grid-delete class="btn disabled btn-danger btn-sm" href="<?php echo ($delUrl); ?>">删除</a></div>
                    </div>
...
<div class="table-responsive">
    <table id="modelObj" class="table table-bordered table-striped table-hover">
        <thead>
        <?php if(is_array($menu[field])): $i = 0; $__LIST__ = $menu[field];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$f): $mod = ($i % 2 );++$i; if(($f[showList]) == "1"): ?><th w_index="<?php echo ($f["name"]); ?>"><?php echo ($f["title"]); ?></th><?php endif; endforeach; endif; else: echo "" ;endif; ?>
        </thead>
    </table>
</div>
