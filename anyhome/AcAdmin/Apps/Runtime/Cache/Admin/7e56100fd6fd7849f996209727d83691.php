...
<div class="col col-md-12 col-sm-12 col-xs-12 col-lg-12">
                    <div class="block">
                        ...
                        <div class="caption">模型</div></div><div id="modelObj-action" class="block-controls"><a grid-btn grid-delete class="btn disabled btn-danger btn-sm" href="<?php echo ($delUrl); ?>">删除</a>
...
<div class="table-responsive">
    <table id="modelObj" class="table table-bordered table-striped table-hover">
        <thead>
            <th w_index="name">模型名称</th>
            <th w_index="title">显示标题</th>
            <th w_render="type">类型</th>
            <th w_render="is_nav">导航显示</th>
            <th w_index="intro">备注</th>
        </thead>
    </table>
</div>
    ...
    modelObj = $.fn.bsgrid.init('modelObj', {
        ...
        event: {
            selectRowEvent: function (record, rowIndex, trObj, options) {
                ...
                var url = "<?php echo U('AcField/index');?>";
                url = U('table',name,url);
                fieldObj.options.settings.url = url;
                fieldObj.page(1);             
            },
...
<div class="col col-md-12 col-sm-12 col-xs-12 col-lg-12">
                    <div class="block">
                        <div  class="block-heading">
                        <div class="main-text h2">
                        <div class="caption">字段</div></div><div id="fieldObj-action" class="block-controls"><a grid-btn grid-dialog class="btn btn-primary disabled btn-sm" href="<?php echo U('AcField/edit');?>">修改</a></div>
                    </div>
                        <div class="block-content-outer">
                        <div class="block-content-inner">
                            
<div class="table-responsive">
    <table id="fieldObj" class="table table-bordered table-striped table-hover">
        <thead>
...
<div id="fieldObj-pager" class="widget-footer"></div>
                        </div>
                        </div>
                    </div>
                </div>
...
$(function(){
    ...
    if (!pageSize) pageSize = 10
    var url = 0;
    if (!url) url = "<?php echo U('AcField/index');?>";
    fieldObj = $.fn.bsgrid.init('fieldObj', {
        url: url,
        pageSize: pageSize,
        // autoLoad:false,
        displayBlankRows:false,
        event: {
            selectRowEvent: function (record, rowIndex, trObj, options) {
                ...
                var table = fieldObj.getRecordIndexValue(record, 'table');
                var field = fieldObj.getRecordIndexValue(record, 'name');
                $('#fieldObj-action').find('a[grid-btn]').each(function(idx,it){
                    $(this).removeClass('disabled');
                    var u = $(this).attr('href');
                    u = U('field',field,u);
                    u = U('table',table,u);
            ...
            unselectRowEvent: function (record, rowIndex, trObj, options) {
                var name = fieldObj.getRecordIndexValue(record, 'name');
                $('#fieldObj-action').find('a[grid-btn]').each(function(idx,it){
                    $(this).addClass('disabled');
                    var u = $(this).attr('href');
                    u = U('name','',u);
    ...
    $('#fieldObj-action').find('[grid-dialog]').on('click',function(event){
        event.preventDefault();
        var box = $('<div style="max-height:500px;overflow-y: auto;" class="row"></div>');
        var size = $(this).attr('dialog-size');
        if (!size) size = 'size-wide';
    ...
    $('#fieldObj-action').find('[grid-delete]').confirmation({
        container: 'body',
        href:false,
        onConfirm:function(event, element){
            event.preventDefault();
...
function is_nav(record, rowIndex, colIndex, options) {
    var is_nav = modelObj.getRecordIndexValue(record, 'is_nav');
    if (is_nav) {
        return "显示";
    }else{
        return "不显示";
...
function type(record, rowIndex, colIndex, options) {
    var type = modelObj.getRecordIndexValue(record, 'type');
    if (type == 'dic') {
        return "字典";
    }else if (type == 'artice'){
        return "文章";
