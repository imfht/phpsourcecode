...
class Auth
{
    ...
    private static function getInstance()
    {
        if (!self::$instance) {
            ...
            //设置session.nam
            ini_set('session.name', 'AUTH_MEMBER_ID');

            //设置session.save_path
            ini_set('session.save_path', base_path('storage') . '/framework/sessions');
    ...
    {
        $token = Input::get('app_access_token', null);
        if (!empty($token)) {
    ...
    public function updateLoginInfo($loginType = null)
    {
        ...
        if (isset($_SESSION[self::MEMBER_INFO]) && (empty($loginType) || $loginType = self::MEMBER_INFO)) {
            ...
            $data     = UserMember::model()->info($memberId);
