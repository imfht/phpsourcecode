...
class BaseController extends \Illuminate\Routing\Controller
{
    ...
    protected function auth()
    {
        ...
        if (!empty($userInfo) && !empty($userInfo['role']) && !empty($userInfo['permission'])) {
            foreach ($userInfo['permission'] as $item) {
        ...
        if (!empty($noAuth[$actionID])) {
            $auth = [];
            if (!empty($userInfo) && !empty($userInfo['permission'])) {
                $auth = $userInfo['permission'];
            }
     ...
     * @return mixed
     */
    protected function showMessage($data = [], $code = null, $message = null, $param = null)
    {
        //指定默认code
        if (empty($code)) {
            $code = Code::SYSTEM_OK;
