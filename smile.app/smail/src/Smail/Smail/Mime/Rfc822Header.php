...
class Rfc822Header
{
     ...
     *
     * @param mixed $hdr
     *            string or array with message headers
     */
    ...
    function parseField($field, $value)
    {
        ...
        switch ($field) {
            case 'date':
                ...
                $d = explode(' ', $d);
                $this->date = Com_date::getTimeStamp($d);
    ...
    function createAddressObject(&$aStack, &$aComment, &$sEmail, $sGroup = '')
    {
        ...
        if ($sPersonal && substr($sPersonal, 0, 2) == '=?') {
            $oAddr->personal = Com_mime::encodeHeader($sPersonal);
        } else {
     ...
     * @param string $address            
     * @param boolean $ar
     *            return array instead of only the first element
     * @param array $addr_ar
     *            (obsolete) array with parsed addresses
     * @param string $group
     *            (obsolete)
     * @param string $host
     *            default domainname in case of addresses without a domainname
     * @param string $lookup
     *            (since) callback function for lookup of address strings which are probably nicks (without @)
    ...
    function parseAddress($address, $ar = false, $aAddress = array(), $sGroup = '', $sHost = '', $lookup = false)
    {
        ...
        foreach ($aAddress as $oAddr) {
            ...
            if (! $oAddr->host) {
                $grouplookup = false;
                if ($lookup) {
                    $aAddr = call_user_func_array($lookup, array(
                        $oAddr->mailbox
                    ));
     ...
     * namely that of the X-Priority header (1, 3, 5).
     * Supports:
    ...
    function parseSpamStatus($value)
    {
        ...
        if (preg_match('/^(No|Yes),\s+score=(-?\d+\.\d+)\s+required=(-?\d+\.\d+)\s+tests=(.*?)\s+autolearn=(.*?)\s+version=(.+?)$/', $value, $matches)) {
            ...
            // is_spam
            if (isset($matches[1]) && strtolower($matches[1]) == 'yes') {
     ...
     *            (since 1.4.0) return encoded or plain text addresses
     * @param boolean $unconditionally_quote
     *            (since 1.4.21/1.5.2) When TRUE, always
     ...
     * @param mixed $arg
     *            string or array of strings
     * @param array $excl_arr
     *            array of excluded email addresses
     * @param array $arr
     *            array of added email addresses
     * @return array
     */
