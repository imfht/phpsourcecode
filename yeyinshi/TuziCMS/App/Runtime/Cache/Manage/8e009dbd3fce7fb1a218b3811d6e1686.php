...
<script src="/tuzicms/App/Manage/View/Default/js/common.js" type="text/javascript"></script>
 <script type="text/javascript">
  function clear_del()
  {
   if(confirm("确定要删除数据吗？"))
   {
	subform("<?php echo U('Link/delall');?>");
   }
  }
 </script>
...
<div class="main">
    ...
    <div class="operate">
        <div class="left">
            <input type="button" onclick="window.open('/tuzicms/index.php/manage/link/add','main')" target="main" class="btn_blue" value="添加">
				<input class="btn_blue" type="button" onClick="return clear_del();"  value="删除" />      
        </div>
        <?php if(ACTION_NAME == "index"): ?><div class="left_pad">
            <form method="get" action="/tuzicms/index.php/manage/link/search">
    ...
    <form action="{:U(GROUP_NAME.'/Link/delAll')}" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo ($v["id"]); ?></td>
                <td><a href="<?php echo ($v["link_url"]); ?>" target="_blank"><?php echo ($v["link_name"]); ?></a></td>
                <td><?php echo ($v["link_url"]); ?></td>
				
                
                <td><?php echo (date('Y-m-d H:i:s', $v["link_addtime"])); ?></td>
				 <td>
				<?php if($v['link_show']==0): ?>否
				<?php else: ?>
				<span style="color:#FF0000">是</span><?php endif; ?>
				 </td>
                <td><?php echo ($v["link_sort"]); ?></td>
                <td>
                    <a href="/tuzicms/index.php/<?php echo ($module); ?>/Link/edit/id/<?php echo ($v["id"]); ?>">编辑</a>
                    <a href="/tuzicms/index.php/manage/link/del/id/<?php echo ($v["id"]); ?>" onclick="return confirm('是否确定删除?')">删除</a>
				</td>
