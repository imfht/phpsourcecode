...
<script src="/tuzicms/App/Manage/View/Default/js/common.js" type="text/javascript"></script>
 <script type="text/javascript">
  function clear_del()
  {
   if(confirm("确定要删除数据吗？"))
   {
	subform("<?php echo U('Attribute/delall');?>");
   }
  }
 </script>
...
<table width="100%" height="31px" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2">
      <tr>
        <td height="31"><div class="titlebt">主题鉴定</div></td>
      </tr>
    </table>
...
<div class="main">
    <div class="operate">
        <div class="left">
            <input type="button" onclick="window.open('/tuzicms/index.php/manage/attribute/add','main')" target="main" class="btn_blue" value="添加">
				<input class="btn_blue" type="button" onClick="return clear_del();"  value="删除" />   
        </div>
        <?php if(ACTION_NAME == "index"): ?><div class="left_pad">
            <form method="get" action="/tuzicms/index.php/manage/attribute/search">
    ...
    <form action="{:U(GROUP_NAME.'/Article/del')}" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<td colspan="5">本区域暂无数据显示...</td>
			</tr>
			<?php else: ?> 
			<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                <td><input type="checkbox" name="id[]" value="<?php echo ($v["id"]); ?>" ></td>
                <td><?php echo ($v["id"]); ?></td>
                <td><a href="/tuzicms/index.php/manage/attribute/news/id/<?php echo ($v["id"]); ?>"><?php echo ($v["attr_name"]); ?></a></td>
                <td>
				<div style="background:<?php echo ($v["attr_color"]); ?>; width:50px; height:18px; margin:0 auto"></div>
				</td>
                <td><a href="/tuzicms/index.php/manage/attribute/edit/id/<?php echo ($v["id"]); ?>">修改</a> <a href="/tuzicms/index.php/manage/attribute/del/id/<?php echo ($v["id"]); ?>" onclick="return confirm('是否确定删除?')">删除</a></td>
