...
<div class="left_pad">
		...
		<!--@end 全局导航 -->
	<div class="column cont intro-page">	
		<!-- 二级导航不显示 -->
		﻿<div class="nav_list">
...
<div style="clear:both"></div>
		...
		<?php else: ?>
		<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="product_volist">
		<div class="product_volist_left">
			<a href="<?php echo ($v["url"]); ?>" title="<?php echo ($v["news_title"]); ?>" >
			<img width="186" height="150" alt="<?php echo ($v["news_title"]); ?>" src="<?php echo ($v["news_pic"]); ?>" class="img">
			</a>
		</div>
		<div class="product_volist_right">
		<h3 class="product_title"><a href="<?php echo ($v["url"]); ?>" title="<?php echo ($v["news_title"]); ?>" ><?php echo ($v["news_title"]); ?></a>
				<?php if($v['attr_name']): ?><span class="news_attr" style="color:<?php echo ($v["attr_color"]); ?>">[<?php echo ($v["attr_name"]); ?>]</span>
				<?php else: endif; ?>
				</h3>
		<span style="color:rgb(68, 68, 68); margin-top:10px; font-size:14px; position:absolute; width:650px;word-break: break-all; word-wrap:break-word;"><?php echo ($v["news_content"]); ?></span>
		</div>
		</div>
		<div style="height:35px;"></div><?php endforeach; endif; else: echo "" ;endif; endif; ?>
  	</div>
</div>
