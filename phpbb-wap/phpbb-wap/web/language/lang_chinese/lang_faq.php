...
$faq[] = array("怎样使我的用户名不出现在在线用户列表上?", "在您的个人资料里您会找到一个选项[隐藏您的在线状态],如果设定为是,您的在线状态将只有您自己和管理员看得到,您会被认为是一位隐身用户.");
...
$faq[] = array("时间不正确!", "时间应该是正确的,然而,您可能会看到一个不同于您所在的时区的时间.如果是这样的话,您应该改变您的个人档案内的时区设定到适合时区,例如:伦敦,巴黎,纽约,悉尼等等.只有注册用户才可以做这些设定,假如您尚未注册,那么请您原谅,您不可以做上述的设定");
...
$faq[] = array("怎样才可以在我的用户名下面显示我的图像?", "当观看帖子时，在一个用户名下面，可能有两幅图像。 第一幅图像与您的等级有关，通常会以星星或者小方格的型式表示您已在这个论坛发了多少帖子以及您在论坛里的地位。下面可能是一幅更大图像的称为“个性头像”，对每个用户来说这是独特的和个性化的，选择和收集头像是论坛管理员的责任,他们也会制作相当的可用的头像供用户选择,如果您不能使用头像是因为管理员的决定,您可以询问他们原因.");
...
$faq[] = array("当我点击一位用户的电子邮件连接时系统要求我先登陆?", "抱歉,只有已注册的用户能够以内部电子邮件的形式(如果管理员已经启动了这个功能)向其它人发送电子邮件.这是为了防止某些哪用户存心不良的使用这个功能.");


$faq[] = array("--","发帖问题");
$faq[] = array("我怎样可以在论坛中发表新主题?", "非常简单，您可以通过论坛或主题页面上的相关按钮发表一个新的主题，在您发表任何信息以前，您可能需要先注册。您可以用到的论坛功能会在论坛页面的底部列出(如您可以发表新主题,您可以发起新投票等等)");
$faq[] = array("我怎样编辑或删除帖子?", "除非您是论坛管理员或论坛版主,否则您只能编辑或删除您的自己帖子.您可以通过点击相应的[编辑]按钮编辑帖子(有时只有在它发表之后的一个限定的时间内).如果某人已经回覆了该主题,当您返回到主题的时候,您会在帖子下面找到一个小的本文输出框,列出您编辑了它的次数.如果没有人回复的情况下它不会出现.在版主或管理员编辑的帖子中也不会出现.请注意一般的用户不能够删除有回复的主题.");
...
$faq[] = array("为什么我不能参加投票?", "只有注册用户可以投票(为了避免欺骗结果).假如您已经注册仍然不能投票那么或许您没有适当的权限.");
...
$faq[] = array("我可以论坛贴图吗?", "可以在您的帖子中显示图像,但是您不一定需要直接上传您的图像到这个论坛,您只要指定图像的位置,例如:http://www.some-unknown-place.net/my-picture.gif 您不可以把图像连接到您自己的电脑上(除非是一台公开可访问的服务器)图像也不能在证明机制之后储存,例如 hotmail 或 yahoo 邮箱,密码保护站点等,显示图像可以使用BB代码的 [img] 标签或者恰当的HTML(如果允许)两者之一.");
$faq[] = array("什么是论坛公告?", "公告经常包含了重要的信息,您应该尽快地阅读.公告会在论坛中每页的顶端出现,您是否可以发表一个公告取决于您的权限许可,由管理员决定.");
$faq[] = array("什么是置顶主题?", "置顶主题会出现在论坛公告的下面,它们包含十分重要的信息,因此可能的话您应该阅读他们.同样地置顶的权限也是由论坛管理员决定.");
$faq[] = array("什么是锁定主题?", "管理员或版主有时会将某主题锁定,您不能回复该锁定的主题,它所包含的任何投票会被自动结束.主题可能会被为许多原因锁定.");


$faq[] = array("--","用户级别和团队");
$faq[] = array("什么是论坛管理员?", "论坛管理员(超级用户)是这个论坛的最高控制人,他们拥有控制论坛的一切权限,包括禁止用户,建立团队,管理论坛板块等,在所有的论坛中也有完整的版主管理能力.");
$faq[] = array("什么是论坛版主?", "论坛版主是独立的(或是团队),他们可以编辑或删除帖子,并且可以在他们所管理的版面中锁定,解锁,转移,删除或者分割主题.论坛版主会阻止用户发表错误,离题或有争议的内容.");
$faq[] = array("什么是论坛团队?", "用户团队是一个论坛管理员分祖管理用户的方式.(这可能不同于其他的论坛),而且每个团队能被分配个别的权限.这使得管理员容易去建立几个类似论坛版主功能的用户,或是限定团队成员进入限定版面等等.");
$faq[] = array("怎样加入一个团队?", "要加入一个团队请点击页面上方(不同的模板设计位置可能不同)的团队连接,您可以预览所有的用户团队,但不是所有的团队都是开放访问的,一些会是关闭的,一些可能是隐藏团队,如果团队是开放的,您可以按申请钮请求加入.您将需要得到该团队的团长同意您的请求才可以加入,他们可能会问为什么您想要参加团队.如果请求不幸被驳回,请不要一直重复申请.");
$faq[] = array("我怎样才能成为团长?", "用户团队是由论坛的管理员建立的,同时他们也会分配一位团长. 如果您有兴趣创建一个团队首先应该联系管理员,尝试给他们发一封邮件吧.");
...
$faq[] = array("谁开发了这个论坛系统?", "这个程式 (未经修改的格式) 由 <a href=\"http://www.phpbb.com/\" target=\"_blank\">phpBB Group</a>所开发及释放出来的. 依照GNU General Public License, 这个程式可以自由的使用及散布, 更多的资讯可以参考 GNU General Public License");
