...
if( $refresh || isset($HTTP_POST_VARS['del_poll_option']) || $error_msg != '' )
{
	...
	if( $preview )
	{
		...
		if( !empty($orig_word) )
		{
			$preview_username = ( !empty($username) ) ? str_replace($orig_word, $replacement_word, $preview_username) : '';
			$preview_subject = ( !empty($subject) ) ? str_replace($orig_word, $replacement_word, $preview_subject) : '';
			$preview_message = ( !empty($preview_message) ) ? str_replace($orig_word, $replacement_word, $preview_message) : '';
		}
