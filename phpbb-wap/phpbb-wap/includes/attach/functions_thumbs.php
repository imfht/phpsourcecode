...
function is_imagick() 
{
	global $imagick, $board_config;

	if ($board_config['img_imagick'] != '')
	{
		$imagick = $board_config['img_imagick'];
		return true;
	}
	else
	{
...
{
	global $board_config, $imagick;

	$source = phpbb_realpath($source);
	...
	if (!$used_imagick) 
	{
		...
		if ($type['gd'])
		{
			...
			if ($type['version'] == 1 || !$board_config['use_gd2'])
			{
