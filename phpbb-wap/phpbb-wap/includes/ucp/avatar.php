...
function check_image_type(&$type, &$error, &$error_msg)
{
	switch( $type )
	{
		...
		default:
			$error 		= true;
			$error_msg 	= '<p>头像文件必须是.jpg（jpeg，pjpeg，jpg）、.gif或.png格式</p>';
			break;
	}
}
...
function user_avatar_url($mode, &$error, &$error_msg, $avatar_filename)
{
	...
	{
		$error 		= true;
		$error_msg 	= '<p>您指定的 URL 远程头像不正确</p>';
		return;
	}
...
{
	global $board_config, $db, $result_ua, $opera_mini;
...
function display_avatar_gallery($mode, &$category, &$user_id, &$viewemail, &$notifypm, &$popup_pm, &$notifyreply_to_email, &$notifyreply_to_pm, &$attachsig, &$allowhtml, &$allowbbcode, &$allowsmilies, &$hideonline, &$style, &$language, &$timezone, &$dateformat, &$session_id)
{
	...
	$template->assign_vars(array(
		...
		'S_COLSPAN' => $s_colspan, 
		'S_PROFILE_ACTION' => append_sid("ucp.php?mode=$mode"), 
