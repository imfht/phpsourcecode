  ...
  <FORM method="post" action="#">
    ...
    <DIV class="form_box">
        <TABLE>
            <TR>
              <TH>关键字：</TH>
              <TD><INPUT value="{$info['keyword']}" class="input w400" type=text name=keyword></TD>
            </TR>
            <TR>
              <TH>回复类型：</TH>
              <TD><SELECT name=type>
				  ...
				  <OPTION selected value=2>图文消息</OPTION>
				  {/if}
                </SELECT>
			...
			<TR id="text">
              <TH>文字消息：</TH>
              <TD><TEXTAREA class="textarea w400 h80" name="content">{$info['content']}</TEXTAREA></TD>
            </TR>
			<TR id="article">
              <TH>图文消息：</TH>
              <TD><INPUT id="articleid" class="input w400" value="{$info['articleid']}" type="hidden" name="articleid">
