...
class IndexController extends AclController {
    ...
    public function indexNewShare(){
        ...
        $sql="SELECT mk_note.noteid,mk_note.notename,mk_note.notediscusscount,mk_user.userid,mk_note.isshare,mk_note.imagesrc,mk_user.userid,mk_user.usernickname from mk_note,mk_user WHERE mk_note.isshare=1 AND mk_note.userid=mk_user.userid ORDER BY noteid DESC limit 0,4;";
        $newShareRows=array();
    ...
    public function indexArticle(){
        $helloMarker=new \Think\Model();
        $sql="SELECT articleid,articlename,articleimg,articletime,articlebanner FROM mk_article ORDER BY articletop desc , articleid desc LIMIT 0,4";
        $articleRows=array();
    ...
    public function myNoteShareDelete(){
        ...
        if($helloMarker->execute($sql)){
            ...
            $this->assign('myErrorFlag',3);
            $this->assign('myErrorInfo',"部分信息清楚失败！");
            $this->myShare();
            exit(); 
        }
