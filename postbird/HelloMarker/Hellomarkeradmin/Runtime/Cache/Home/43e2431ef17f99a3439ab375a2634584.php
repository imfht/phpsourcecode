	 ...
	 <?php if($backFlag == 3): ?><div class="alert alert-warning alert-dismissable">
		...
		<div class="col-md-12">
			<div class="page-header">
				<h1>用户反馈 <small> | 共计：<strong><?php echo ($count); ?></strong> </small></h1>
			</div>
			<div class="">
				<?php if(is_array($bugRows)): $i = 0; $__LIST__ = $bugRows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list-group-item">
					<h4><small>标题：</small><strong><font color="red"><?php echo ($vo["bugname"]); ?></font></strong></h4>
					<h4>
					  <?php if($vo["bugcheck"] == 0): ?><small><a href="/hellomarkertest/hellomarkeradmin.php/Home/Index/bugChecked/id/<?php echo ($vo["bugid"]); ?>" class="btn btn-primary btn-xs pull-right">设为已读</a></small>
					    <?php else: ?>
					       <small><a href="#"class="btn btn-primary btn-xs pull-right" disabled>已经阅读</a></small><?php endif; ?>
					   
					</h4>
					<h4><small>反馈时间：<?php echo ($vo["bugtime"]); ?></small></h4>
					<h4><small>联系方式：</small></h4>
					<div class="well">
						<?php echo ($vo["bugtext"]); ?>
					</div>
