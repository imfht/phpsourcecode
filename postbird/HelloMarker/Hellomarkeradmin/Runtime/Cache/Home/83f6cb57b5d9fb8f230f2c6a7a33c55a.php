	 ...
	 <?php if($backFlag == 3): ?><div class="alert alert-warning alert-dismissable">
		...
		<div class="col-md-12">
			<div class="page-header">
				<h1>评论列表 <small> | 共计：<strong><?php echo ($count); ?></strong> </small></h1>
			</div>
			<div class="">
				<?php if(is_array($shareRows)): $i = 0; $__LIST__ = $shareRows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="well">
					<h4><strong><?php echo ($vo["discusstext"]); ?></strong></h4>
				</div>
					<h4><small><a href="/hellomarkertest/hellomarkeradmin.php/Home/Index/deleteDiscuss/id/<?php echo ($vo["discussid"]); ?>" onclick="javascript:return myNoteDeleteConfirm();"class="btn btn-danger btn-xs pull-right">删除该评论</a></small></h4>
					<h4><small>评论用户：<?php echo ($vo["username"]); ?></small></h4>
					<h4><small>评论时间：<?php echo ($vo["discusstime"]); ?></small></h4>
					<h4><small>点赞次数：<?php echo ($vo["discusslikecount"]); ?></small></h4>
					<h4><small>评论对象：<?php echo ($vo["notename"]); ?></small></h4>
