	 ...
	 <?php if($backFlag == 3): ?><div class="alert alert-warning alert-dismissable">
		...
		<div class="col-md-12">
			<div class="page-header">
				<h1>当前分享列表 <small>用户隐私信息自动过滤</small><small> | 共计：<strong><?php echo ($count); ?></strong> </small></h1>
			</div>
			<div class="">
				<?php if(is_array($shareRows)): $i = 0; $__LIST__ = $shareRows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><h4><strong><?php echo ($vo["notename"]); ?></strong><small><a href="/hellomarkertest/hellomarkeradmin.php/Home/Index/deleteShare/id/<?php echo ($vo["noteid"]); ?>" onclick="javascript:return myNoteDeleteConfirm();"class="btn btn-danger btn-xs pull-right">取消分享</a></small></h4>
					<h4><small>分享用户：<?php echo ($vo["username"]); ?></small></h4>
					<h4><small>记录时间：<?php echo ($vo["notetime"]); ?></small></h4>
					<h4><small>信息地址：<?php echo ($vo["noteaddress"]); ?></small></h4>
					<h4><small>相关介绍：<?php echo ($vo["noteother"]); ?></small></h4>
					<h4><small>讨论次数：<?php echo ($vo["notediscusscount"]); ?></small></h4>
					<h4><small>收藏人数：<?php echo ($vo["notecollectcount"]); ?></small></h4>
		            <img class="media-object center-block" src="/hellomarkertest/Public/uploads/markerimage/<?php echo ($vo["imagesrc"]); ?>" alt="">
