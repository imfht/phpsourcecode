...
//    month=month+1;
//    var tdChildren=$('.calendarTd');
//    var tdFont = new Array();
//    tdFont=tdChildren.children('font');
//    // tdFont.css('color','#ccc')
//   for(var i=0;i<tdFont.length;i++){
//    // var tempId=parseInt(tdFont[i]);
//    var tempDay=parseInt(tdFont[i].innerHTML);
//    if(tempDay==today && (CLD.SM.selectedIndex+1 == month)){
//       $('#'+tdFont[i].id).parent().css('background-color','pink');
//       break;
//    }
//   }
// }
...
function findClickDay(){
   $('.calendarTd').click(function(){
      var tdFontHtml=$(this).children('font').html();
      tdFontValue = parseInt(tdFontHtml);
      if(tdFontValue <=0 || tdFontValue >=32){
         return ;
      }else{
            $(this).parent('tr').parent('tbody').find('td').css("background-color","#fff");
            $(this).css("background-color","#ccc");
      }
      var selectSY=$('#selectSY option:selected').val();
      var selectSM=$('#selectSM option:selected').val();
      // $(this).css("background-color","#ccc");
      var date=selectSY+"-"+selectSM+"-"+tdFontValue;
      var postUrl=appUrl+"/Home/Work/findClickDay";
        $.post(postUrl,{
            'date':date,
                },function(data){
                  $('#clickDateSpan').text(date);
                     if(data.clickWorkCount==1){
                      var sHtml='<li class="list-group-item">'+data.clickWorkRows[0]["workname"] +'</li><div class="oneWorkWell text-center"></div><div class="oneWorkWell"><div class="panel2"><span>'+data.clickWorkRows[0]["workother"] +'</span></div></div>';
                       $('#clickDateDiv').html(sHtml);
                    }else if(data.clickWorkCount>1){
                     var sHtmlAll="";
                     for(var i=0;i<data.clickWorkCount;i++){
                        var sHtmlLevel='！';
                        if(data.clickWorkRows[i]["worklevel"]>2){
                           sHtmlLevel='！！！';
                        }else if(data.clickWorkRows[i]["worklevel"]=2){
                           sHtmlLevel='！！';
                        }
                        sHtml='<a class="list-group-item"  role="button" data-toggle="collapse" href="#clickWork'+data.clickWorkRows[i]["workid"] +'" aria-expanded="false" aria-controls="#clickWork'+data.clickWorkRows[i]["workid"] +'"><span >'+data.clickWorkRows[i]["workname"]+'</span> <font color="#D9534F">'+sHtmlLevel+'</font><i class="fa fa-plus pull-right"></i></a><div class="collapse" id="clickWork'+data.clickWorkRows[i]["workid"] +'"><div class="well">'+data.clickWorkRows[i]["workother"] +'</div></div>';
                        sHtmlAll+=sHtml;
                     }
                       $('#clickDateDiv').html(sHtmlAll);
                    }else if(data.clickWorkCount==0){
                        var sHtml='<div class="panel text-center" id="workDivText"><font class="noWork"><i class="fa fa-calendar-times-o"></i>  暂无记事</font></div>';
                        $('#clickDateDiv').html(sHtml);
                    }
          });
   });
}
function nowDayBackground(){
   var allTd=$('#calendarTr td font');
   var myDate=new Date();
   var today=myDate.getDate();
   var tmonth=myDate.getUTCMonth();
       tmonth=tmonth+1;
   var tyear=myDate.getFullYear();
   var selectSY=$('#selectSY option:selected').val();
   var selectSM=$('#selectSM option:selected').val();
    allTd.each(function(){
        if(parseInt($(this).text())==today &&(selectSY==tyear) && (selectSM==tmonth)){
           $(this).parent('td').css("background-color","#eee8aa");
           return;
        }else{
            rgb=$(this).parent('td').css("background-color");
...
function oldWorkDivClick(){
   $("#oldWorkDiv").hide();
   $("#oldWorkDivBtn").click(function(){
         $("#oldWorkDiv").toggle('slide','right','slow');
   });
}
