...
interface db_interface {
	...
	*/
	...
	// 添加列
	// table_column_add('thread', 'coverimg', 'char(64)');
	public function table_column_add($table, $colname, $colattr);
	
	// 删除列
	// table_column_drop('thread', 'coverimg');
	public function table_column_drop($table, $colname);
	
	// 用来更改表结构，将一个表的数据导入到另外一个表，可以不同结构。
	//public function table_export($fromtable, $fromcol, $totable, $tocol);
	
	// 表改名字
	//public function table_rename($oldname, $newname);
