...
function swf_upload_file(setting, recall) {
	...
	var swf_settings = {
		...
		post_params: setting.postparams,		// swfupload 传参有些环境会失败，直接通过URL传递吧，不要指望通过flash能传递什么参数了，COOKIE不可靠，POST也不可靠，还不如不加这个参数，浪费时间，shit。
		...
		debug: false,
		button_image_url: base_url+"uploadfile.png",
		button_width: "74",
		button_height: "22",
		...
		upload_start_handler : function(file) {
			$('<span class="loading"><img src="'+base_url+'loading.gif" width="16" height="16" /></span>').insertAfter($('#SWFUpload_0'));
			return true;
		},
		...
		upload_success_handler : function(file, serverData) {
			...
			return recall(serverData, this);
		},
		file_queue_error_handler : function(file, errorCode, message) {
			if(errorCode == SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT) {
				alert('您选择的文件：'+file.name+' 尺寸太大！错误信息：'+message);
				//$('.toolbar a.imageloading', _this).hide();
				return true;
			} else {
				alert('upload_queue_error: file:'+file.name+', errorCode:'+errorCode+', message:'+message);
			}
			return false;
		},
