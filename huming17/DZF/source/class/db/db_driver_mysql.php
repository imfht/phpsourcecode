...
class db_driver_mysql
{
	...
	public function query($sql, $silent = false, $unbuffered = false,$multi_query='') {
		...
		if($multi_query){
			//调用方法 DB::query($sql,array(), false, false, 1);
			$con=mysqli_connect($this->config[1]['dbhost'],$this->config[1]['dbuser'],$this->config[1]['dbpw'],$this->config[1]['dbname']);
			...
			if(defined('DZF_DEBUG') && DZF_DEBUG) {
					...
					$sqldebug = $sql.','.number_format((microtime(true) - $starttime), 6).','.sizecount(memory_get_usage());
					echo $sqldebug_html .= '<div class="sqldebug">'.$sqldebug.'</div>';
					@file_put_contents(SITE_ROOT.'./data/debug/sqldebug.txt',$sqldebug.','.json_encode(debug_backtrace()).','.$this->curlink.PHP_EOL, FILE_APPEND);
			}
			$this->querynum++;
		}
		return $query;
