...
switch($do){
	case "scroll_page":
            ...
            foreach($sql_info_result AS $key => $value){
                ...
                $data[] = array(
					'info_id'=>$sql_info_result[$key]['info_id'],
					'title_img'=>'http://'.$_SERVER["HTTP_HOST"].'/'.str_replace(array('src=','"'),'',$sql_info_result[$key]['title_img']),
					'title'=>$sql_info_result[$key]['title'],
					'content_desc'=>$sql_info_result[$key]['content_desc'],
					'create_dateline_format'=>$sql_info_result[$key]['create_dateline_format']
                );
            }
