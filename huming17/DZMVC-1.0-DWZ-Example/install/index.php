...
define('WWWROOT_PATH', dirname(__FILE__).'/../../');
...
require ROOT_PATH.'./source/function/function_ext.php';
...
if($method == 'show_license') {
	...
	if($submit) {
		...
		runquery($sql);
                //$password 更新管理员密码
                $db->query("UPDATE {$tablepre}users SET user_password='".sha1(md5($password))."' WHERE user_id=1 LIMIT 1");
		...
		VIEW_OFF && show_msg('initdbresult_succ');
                ...
                if($is_unix==1 && function_exists('shell_exec')){
                    //DEBUG 获取站点服务用户并设置定时任务
                   /* 
                    * ###db_user### 数据库用户 $dbuser
                    * ###db_passwd### 数据库密码 $dbpw
                    * ###db_host### 数据库主机 $dbhost
                    * ###root_path### 网站绝对路径 ROOT_PATH
                    * ###wwwroot_path### WEB服务器根绝对路径 WWWROOT_PATH
                    * ###db_name### 数据库名称 $dbname
                    * ###www_name### 网站目录名 trim(string_replace(WWWROOT_PATH,'',ROOT_PATH),DIRECTORY_SEPARATOR )
                    * 通过 LINUX sed -i ‘s/[匹配字符串]/[替换字符串]/g’ [文件名]
                    */
                    $replace_string = "sed -i 's:###db_user###:".$dbuser.":g' ".ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    $replace_string = "sed -i 's:###db_passwd###:".$dbpw.":g' ".ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    $replace_string = "sed -i 's:###db_host###:".$dbhost.":g' ".ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    $replace_string = "sed -i 's:###root_path###:".realpath(ROOT_PATH).":g' ".ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    $replace_string = "sed -i 's:###wwwroot_path###:".realpath(WWWROOT_PATH).":g' ".ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    $replace_string = "sed -i 's:###db_name###:".$dbname.":g' ".ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    $replace_string = "sed -i 's:###www_name###:".trim(str_replace(realpath(WWWROOT_PATH),'',realpath(ROOT_PATH)),DIRECTORY_SEPARATOR ).":g' ".ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    
                    //DEBUG 增加可执行权限
                    @shell_exec('chmod +x '.ROOT_PATH.'./source/module/crontab/cron_backup_trigger.sh');
                    @shell_exec('chmod +x '.ROOT_PATH.'./source/module/crontab/crontab_task');
                    
                    $replace_string = "sed -i 's:###root_path###:".realpath(ROOT_PATH).":g' ".ROOT_PATH.'./source/module/crontab/crontab_task';
                    //file_put_contents('exec.txt', $replace_string.'', FILE_APPEND);
                    @shell_exec($replace_string);
                    
                    $fileowner = @posix_getpwuid(fileowner($lockfile));
                    @shell_exec('crontab -u '.$fileowner['name'].' '.ROOT_PATH.'./source/module/crontab/crontab_task');
                }
		if(!VIEW_OFF) {
                        echo '<a style="text-align:center" href="../admin.php" target="_self">'.lang('install_succeed').'</a>';
	...
	if(VIEW_OFF) {
		...
		echo '<p style="text-align:center"><a href="../admin.php" target="_self">'.lang('install_succeed').'</a></p>';
		echo '</div>';
		show_footer();
	}
