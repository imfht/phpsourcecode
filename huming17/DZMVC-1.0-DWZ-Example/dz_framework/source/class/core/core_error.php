...
class core_error
{
	...
	public static function system_error($message, $show = true, $save = true, $halt = true) {
		...
		list($showtrace, $logtrace) = core_error::debug_backtrace();

		if($save) {
			$messagesave = '<b>'.$message.'</b><br><b>PHP:</b>'.$logtrace;
			core_error::write_error_log($messagesave);
		}

		if($show) {
			if(!defined('IN_MOBILE')) {
				core_error::show_error('system', "<li>$message</li>", $showtrace, 0);
			} else {
				core_error::mobile_show_error('system', "<li>$message</li>", $showtrace, 0);
			}
		}
	...
	public static function debug_backtrace() {
		$skipfunc[] = 'core_error->debug_backtrace';
		$skipfunc[] = 'core_error->db_error';
		$skipfunc[] = 'core_error->template_error';
		$skipfunc[] = 'core_error->system_error';
		...
		foreach ($debug_backtrace as $k => $error) {
			$file = str_replace(DZF_ROOT, '', $error['file']);
	...
	public static function db_error($message, $sql) {
		...
		core_error::show_error('db', $msg, $showtrace, false);
		unset($msg, $phperror);
		...
		core_error::write_error_log($errormsg);
		exit();
	...
	public static function exception_error($exception) {
		...
		foreach ($trace as $error) {
				...
				if(!empty($error['args'])) {
					...
					foreach($error['args'] as $arg) {
						...
						if(is_array($arg)) {
							...
							$fun .= (defined('DZF_DEBUG') && DZF_DEBUG) ? $arg : '%d';
						} elseif(is_float($arg)) {
							$fun .= (defined('DZF_DEBUG') && DZF_DEBUG) ? $arg : '%f';
						} else {
							$fun .= (defined('DZF_DEBUG') && DZF_DEBUG) ? '\''.dhtmlspecialchars(substr(self::clear($arg), 0, 10)).(strlen($arg) > 10 ? ' ...' : '').'\'' : '%s';
						}
						$mark = ', ';
					}
				}
			...
			$phpmsg[] = array(
			    'file' => str_replace(array(DZF_ROOT, '\\'), array('', '/'), $error['file']),
			    'line' => $error['line'],
...
EOT;
		...
		$helplink = '';
		if($type == 'db') {
			$helplink = "http://faq.512cs.com/?type=mysql&dberrno=".rawurlencode(DB::errno())."&dberror=".rawurlencode(str_replace(DB::object()->tablepre, '', DB::error()));
...
<tr>
	<td id="bodytitle" width="100%">DZF! $title Error </td>
</tr>
EOT;
