...
class table_common_syscache extends core_table
{
	...
	public function fetch_all($cachenames) {
		...
		if($this->_isfilecache) {
			...
			foreach($cachenames as $cachename) {
				if(!@include_once(DZF_ROOT.'./data/cache/cache_'.$cachename.'.php')) {
		...
		while($syscache = DB::fetch($query)) {
			...
			if($this->_isfilecache) {
				...
				if(($fp = @fopen(DZF_ROOT.'./data/cache/cache_'.$syscache['cname'].'.php', 'wb'))) {
	...
	public function insert($cachename, $data) {
		...
		$this->_isfilecache && @unlink(DZF_ROOT.'./data/cache/cache_'.$cachename.'.php');
	}
