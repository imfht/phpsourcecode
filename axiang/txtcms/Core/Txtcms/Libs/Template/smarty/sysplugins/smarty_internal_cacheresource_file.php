...
//        }
        ...
        if (isset($_compile_id) && !$_template->smarty->use_sub_dirs) {
		...
		if ($_template->smarty->use_sub_dirs){
			$hash_dir=getHashDir($_cache_id . $_compile_id . $_filepath).'/';
		}else{
			$hash_dir=$_cache_id . $_compile_id;
		}
        $cached->filepath = $_cache_dir . $hash_dir.$_filepath . '.' . basename($_source_file_path) . '.php';
