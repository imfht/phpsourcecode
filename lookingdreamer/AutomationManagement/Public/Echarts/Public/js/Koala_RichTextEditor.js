...
+function ($) {
    ...
    var RichTextEditor = function (element, options) {
        ...
        //事件列表
        this.$element.parent().children("div[data-target]").on('click', "a[data-edit]", $.proxy(function (ev) {
            this.exec(ev.currentTarget);
        }, this));
    };
    ...
    RichTextEditor.prototype = {
        ...
        init: function ($element, options) {
            ...
            if (that.options.showListCtrls) {
                listCtrls = [
                    {"name": "insertunorderedlist", "title": "插入无序列表", "icon": "list-ul"},
            ...
            if (that.options.showJustfyCtrls) {
                justfyCtrls = [
                    {"name": "justifyleft", "title": "向左对齐", "icon": "align-left"},
