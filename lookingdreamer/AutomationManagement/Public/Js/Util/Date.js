...
isLeapYear(date)
{
    return Date.ToDate(date).isLeapYear();
}
...
{
    return Date.ToDate(date).Format(formatStr);
}
...
{
    return Date.ToDate(date).DateAdd(strInterval, Number);
}
...
function Date
.
DateDiff(date1, date2, strInterval)
{
    return Date.ToDate(date1).DateDiff(strInterval, date2);
}
...
function Date
.
toString(date, showWeek, chinese)
{
    return Date.ToDate(date).toString(showWeek, chinese);
}
...
MagicInfo(date, type)
{
    return Date.ToDate(date).MagicInfo(type);
}
...
GetTZO(date)
{
    return Date.ToDate(date).GetTZO();
}
...
GetTZD(date, tzo)
{
    return Date.ToDate(date).GetTZD(tzo);
}
...
toArray(date)
{
    return Date.ToDate(date).toArray();
}
...
Date.prototype.DatePart = function (interval) {
    ...
    switch (interval) {
        ...
        case 's':
            ...
            break;
        case 'GZ':
            partStr = myDate.MagicInfo('GZ');
            break;
        case 'SX':
            partStr = myDate.MagicInfo('SX');
            break;
        case 'XZ':
            partStr = myDate.MagicInfo('XZ');
            break;
        case 'MD':
            partStr = myDate.MaxDayOfDate();
            break;
    }
    return partStr;
}
function Date
.
DatePart(date, interval)
{
    return Date.ToDate(date).DatePart(interval);
}
...
MaxDayOfDate(date)
{
    return Date.ToDate(date).MaxDayOfDate();
}
...
WeekNumOfYear(date)
{
    return Date.ToDate(date).WeekNumOfYear();
}
