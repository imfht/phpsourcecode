...
RemoveBlank(str)
{
    return String.Convert(str).RemoveBlank();
}
...
EncodeHTML(str)
{
    return String(str).EncodeHTML();
}
//换行，空格，等。。。
function String
.
prototype.EncodeInnerHTML()
{
    var str = this;
...
EncodeInnerHTML(str)
{
    return String(str).EncodeInnerHTML();
}
//编码HTML的属性 href="xxxx"
function String
.
prototype.EncodeAttr()
{
    var str = this;
    str = String.EncodeHTML(str);
    return str;
...
function String
.
prototype.ToStringByZero(count)
{
    var str = this;
...
function String
.
ToStringByZero(str, count)
{
    return String(str).ToStringByZero(count);
}
...
EncodeURL(str)
{
    return String.Convert(str).EncodeURL();
}

function String
.
prototype.EasyEncode(key)
{
    var str = this;
...
EasyEncode(str, key)
{
    return String.Convert(str).EasyEncode(key);
}

function String
.
prototype.EasyDecode(key)
{
    var str = this;
    key = String.Convert(key);
    if (key === "")return str;
...
EasyDecode(str, key)
{
    return String.Convert(str).EasyDecode(key);
}
...
SHA1(str)
{
    return String.Convert(str).SHA1();
}
//---------------------------------------------------
//功能：简体与繁体的相互转换
//参数：content　要转换的字符串
