...
FCKXHtml._AppendAttributes = function (A, B, C) {
    ...
    for (var n = 0; n < D.length; n++) {
        ...
        if (E.specified) {
            ...
            var G;
            if (F.StartsWith('_fck')) continue; else if (F.indexOf('_moz') == 0) continue; else if (F == 'class') G = E.nodeValue; else if (E.nodeValue === true) G = F; else G = B.getAttribute(F, 2);
...
FCKDialog.CheckFocus = function () {
    ...
    if (FCKDialog.Window && !FCKDialog.Window.closed) FCKDialog.Window.focus(); else {
        try {
