<?php
class CmAction extends CommonAction {
	// 查询连接数
	public function index() {
		...
		$str =$this->_param( 'str','htmlspecialchars', 'go2' ); // verify 连接数
		$table_array = C ( 'Connect' );
		$str_array=C('Cm');
		switch ($depart) {		
				case "1" :
					$departname="cm";
					switch ($query_type) {
						case "Day" :
							...
							R('Admin/Cm/getdepartall',array($table_array,$time,$order_str,$unit,$query_date,$action_name,$departname,$str_array));
							//getdepartall($table_array,$time,$order_str,$unit,$query_date,$action_name,$departname)
// 							R('Admin/Cm/common',array($query_date,$receive,$title,$unit,$legend));
							exit();
							break;
						case "Week" :
							$action_name='Admin/Show/show_week_ins_multi_quote';
							R('Admin/Cm/getdepartallweek',array($table_array,$time,$order_str,$unit,$query_date,$action_name,$departname,$str_array));
							exit();
							break;
						case "Month" :
					        $action_name='Admin/Show/show_month_ins_multi_quote';
							R('Admin/Cm/getdepartallmonth',array($table_array,$time,$order_str,$unit,$query_date,$action_name,$departname,$str_array));
							exit();
							break;
					}
					break;
...
// 							R('Admin/Cm/common',array($query_date,$receive,$title,$unit,$legend));
	...
	public function getdepartall($table_array,$time,$order_str,$unit,$query_date,$action_name,$departname,$str_array){

		        $tb = get_tablename($table_array, $departname);
				$title = $departname;
...
// 		dump($series_array);die();	
	...
	//每周所有分区的连接数的查询
	public function getdepartallweek($table_array,$time,$order_str,$unit,$query_date,$action_name,$departname,$str_array){
		$tb = get_tablename($table_array, $departname);
		$title = $departname;
		$receive = R ( $action_name, array (
				$time,
	...
	//每月所有分区的连接数的查询
	public function getdepartallmonth($table_array,$time,$order_str,$unit,$query_date,$action_name,$departname,$str_array){
		$tb = get_tablename($table_array, $departname);
		$title = $departname;
		$receive = R ( $action_name, array (
				$time,
