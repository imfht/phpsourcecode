...
class  TemplateSeophp {
    ...
    public function loadTemplate ($tmplTemplateFile,$prefix='') {
        ...
        $tmplContent = $this->TemplateparseCode($tmplContent, $this->templateFile);
         // 根据模版文件名定位缓存文件
    ...
    private function replaceBlock($name,$content) {
		...
		private function TemplateparseCode($string, $tplfile, $operation = 'DECODE'){
			return $string;
		}    
    ...
    private function parseTemplateName($templateName){
        ...
        foreach ($array as $templateName){
            ...
            // 获取模板文件内容
            $parseStr .= $this->TemplateparseCode(file_get_contents($templateName), $templateName);
        }
        return $parseStr;
    }
