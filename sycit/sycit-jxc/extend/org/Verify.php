...
class Verify
{
    ...
    public function entry($id = '')
    {
        ...
        $this->_color = imagecolorallocate($this->_image, mt_rand(0, 255), mt_rand(200, 255), mt_rand(255, 255));
        //$this->_color = imagecolorallocate($this->_image, 225, 225, 225);
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '/verify/' . ($this->useZh ? 'Tahoma' : 'ttfs') . '/';
    ...
    private function _writeNoise()
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            //杂点颜色
            $noiseColor = imagecolorallocate($this->_image, mt_rand(150, 220), mt_rand(150, 220), mt_rand(150, 220));
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
