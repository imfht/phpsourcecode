...
<head>
    <!--头部文件-->
    <meta charset="UTF-8">
    <title>{$title}-{$data.pbname}-{:Config('syc_webname')}</title>
        ...
        <div class="col-lg-12">
            <div class="console-title console-title-border clearfix">
                <div class="col-md-6 pull-left order-title">
                    ...
                    <span class="head-title">{$data.pbname}</span>
                </div>
        ...
        <div class="col-md-12">
            <div class="syc-table">
                <form class="form-horizontal" method="post" enctype="multipart/form-data" id="HandleDepositEditForm">
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">已 收 款</label>
                        <div class="col-sm-4">
                            <div class="radio">
                                {volist name="finance" id="vo" empty="暂无详情"}
                                <p>【{$vo.shoukuan_time|date="Y-m-d",###}】收到 {$vo.sort.sname} ￥{$vo.amount}</p>
                                {/volist}
                                <span class="label label-sm label-success">共计 <b>{$fcount}</b> 笔款 ￥{$fsum}</span>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <div class="form-inline">
                            <label class="col-sm-2 control-label">余款金额</label>
                            <div class="col-sm-1">
                                <input type="text" class="form-control text-danger" id="yukuan" value="{$yukuan}" disabled>
                            </div>
                        </div>
                        <div class="form-inline">
                            <label class="col-sm-2 control-label text-danger">收款金额</label>
                            <div class="col-sm-4">
                                <input type="text" class="form-control text-danger" name="djin_amount" id="djin_amount" value="">&nbsp;<label id="yubilv"></label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-inline">
                            ...
                            <div class="col-sm-3">
                                <input type="text" class="form-control" name="shoukuan_time" id="shoukuan_time" value="" placeholder="年-月-日 时:分">
                            </div>
                        </div>
                    </div>
...
<script>
    $(function () {
        ...
        $('input[name="djin_amount"]').watch(function(val) {
            var yukuan = $('input[id="yukuan"]').val();
            var ykbl = '';
            if (val > 0 && val != '') {
                ykbla = val - yukuan;
                $('#yubilv').text(ykbla.toFixed(2));
            } else {
                $('#yubilv').text(ykbl);
            }
        });
