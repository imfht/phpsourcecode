...
if ( !defined( '_MB_VERSION_' ) )
  exit;
class PostMail extends CarrierModule
{
	public function __construct()
	{
		...
		$this->displayName = $this->l('PostMail Express');
		$this->description = $this->l('Domestic PostMail Express shipping');
		...
		$this->_fieldsList = array(
			...
			'step_fee1' => 
				array(
					'name' =>$this->l('Please enter step_fee1'),
					'validate'=>'isFloat',
					'type'=>'text',
					'dbtype'=>'float',
					'value'=>'2.5'
			...
			'pack_fee' => 
				array(
					'name' =>$this->l('Please enter pack_fee'),
					'validate'=>'isFloat',
					'type'=>'text',
					'dbtype'=>'float',
					'value'=>'0'
	...
	*/
	...
	public function getOrderShippingCost($params, $shipping_cost)
	{
		...
		if($result){
            $fee = $result[0]['base_fee'] + $result[0]['pack_fee'];
			$goods_weight = $params->getTotalWeight();
            if ($goods_weight > 5)
            {
                $fee += 4 * $result[0]['step_fee'];
                $fee += (ceil(($goods_weight - 5))) * $result[0]['step_fee1'];
            }
            else
            {
                if ($goods_weight > 1)
                {
                    $fee += (ceil(($goods_weight - 1))) * $result[0]['step_fee'];
                }
            }
            return $fee;
		}
