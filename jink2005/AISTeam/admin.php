...
$accesskey = $_SESSION['genaccesskey'];//getArrayVal($_POST, "accesskey");
$company = getArrayVal($_POST, "company");
$client_id = getArrayVal($_POST, "client_id");
$client_name = getArrayVal($_POST, "client_name");
...
// get admin client
$client_admin = $user->getProfileField($userid, "client_id");

if (!($userpermissions["admin"]["add"] || $action=="addpro" || 
      (in_array($action, array("users")) && ($userpermissions["user"]["add"] || $userpermissions["user"]["edit"] || $userpermissions["user"]["del"])) ||
      (in_array($action, array("adduser")) && $userpermissions["user"]["add"]) ||
      (in_array($action, array("editform", "edituser")) && $userpermissions["user"]["edit"]) ||
      (in_array($action, array("deleteuserform", "deleteuser")) && $userpermissions["user"]["del"])
     )
){
...
} elseif ($action == "users")
{
    ...
    if (!empty($users))
    {
        foreach($users as $usr)
        {
            ...
            if (!empty($projects))
            {
                ...
                {
                    $chk = chkproject($usr["ID"], $pro["ID"]) ? 1 : 0;
    ...
    if($userpermissions["admin"]["root"]) {
        $template->assign("clients", $clientobj->getAllClients());
    }
    $template->assign("add_enabled", $clientobj->enabledAddUsers($userid));
    $template->display("adminusers.tpl");
} elseif ($action == "adduser")
{
    if(!$clientobj->enabledAddUsers($userid)) {
    ...
    if ($newid)
    {
        ...
        $roleobj = (object) new roles();
        if(!$userpermissions["admin"]["root"] && $role<2)
            $role = 2;
        if(!$userpermissions["admin"]["add"] && $role<3)
            $role = 3;
        $roleobj->assign($role, $newid);
        
        if($userpermissions["admin"]["root"]){
            if($client_name!="") {
                $client_id = $clientobj->add($client_name);
            }
        } else{
            $client_id = $client_admin;
        }
        $clientobj->assign($client_id, $newid);
        ...
        if ($settings["mailnotify"])
        {
            if (!empty($email))
            {
				...
				$themail->send_mail($email, $langfile["profileaddedsubject"] , $langfile["profileaddedtext"] . "<br /><br />" . $langfile["profileusername"] . ":&nbsp;" . "$email<br />" . $langfile["profilepass"] . ":&nbsp;" . "$pass<br /><br />" . "<a href = \"$url\">$url</a>");
            }
        }
...
{
    if(!$userpermissions["admin"]["root"] && $client_admin!=$user->getProfileField($id, "client_id")) {
        header("Location: admin.php?action=users");
        exit;
    }
    ...
    $template->assign("roles", $roleobj->getAllRoles());
    $template->assign("clients", $clientobj->getAllClients());
...
{
    if(!$clientobj->enabledAddProjects($userid)) {
        header("Location: admin.php?action=projects");
        exit;
    }
    ...
    $add = $project->add($name, $desc, $end, $budget, 0, $start, $accesskey);
    if ($add)
    {
