...
#-- operation
@define("CAPTCHA_TEMP_DIR", (isset($_SERVER['TEMP']) ? $_SERVER['TEMP'] : dirname(__FILE__)."/../../templates_c/") . "/captcha/");
...
$tmpurl = (empty($_SERVER['HTTPS'])? "http": "https") . "://$_SERVER[SERVER_NAME]:$_SERVER[SERVER_PORT]/" .  substr(realpath(__FILE__), strlen(realpath($_SERVER["DOCUMENT_ROOT"])));
$tmpurl = str_replace("/\\", "\\", $tmpurl);
$tmpurl = str_replace("\\", "/", $tmpurl);
@define("CAPTCHA_BASE_URL", $tmpurl);
...
if (easy_captcha_utility::canonical_path("http://ignored.xxx/$_SERVER[REQUEST_URI]") == easy_captcha_utility::canonical_path(CAPTCHA_BASE_URL)) {
...
class easy_captcha {
   ...
   function form($add_text="") {
      ...
      $HTML =
         ...
         '<input type="text" class="text" name="'.$p_input.'" id="'.$p_input.'" required="1" realname="Image Text" /><br />' .
         '<img src="'.$img_url .'&" width="252" height="80" alt="'.$alt_text.'" align="middle" '.$onClick.' title="' . CAPTCHA_REDRAW_TEXT . '" style="border:1px solid #CACCBC;margin-top:2px;" />' .
         '&nbsp;' .
         $add_text .
...
class easy_captcha_graphic_image_waved extends easy_captcha_graphic {
   ...
   */
   ...
   function text() {
      ...
      imagettftext($this->img, $SIZE, $DEG, rand(5,$left-5), $h-rand(3, $top-3), $this->rgb(222,0,0), $this->font(), $this->solution);
   }
...
END_____JSRPC__JSRPC__JSRPC__JSRPC__JSRPC__JSRPC_____END;
   ...
   /* static */ function canonical_path($url) {
     $path = @parse_url($url);
