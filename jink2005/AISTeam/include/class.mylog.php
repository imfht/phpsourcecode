...
class mylog extends TableBase
{
    ...
    */
     ...
     * @return bool
     */
    function add($name, $type, $action = "", $project = 0)
    {
        $user = $this->userid;
        $uname = $this->uname;
        ...
        $ins = mysql_query("INSERT INTO ".$this->getTableName()." (user,username,name,type,action,project,datum) VALUES ('$user','$uname','$name','$type',$action,$project,'$now')");
        if ($ins)
        {
    ...
    function getProjectLog($project, $lim = 10)
    {
        ...
        $sel = mysql_query("SELECT COUNT(*) FROM ".$this->getTableName()." WHERE project = $project ");
        ...
        $sql = "SELECT * FROM ".$this->getTableName()." WHERE project = $project ORDER BY ID DESC LIMIT $start,$lim";
        ...
        while ($log = mysql_fetch_array($sel2))
        {
            if (!empty($log))
            {
                $sel3 = mysql_query("SELECT name FROM ".$this->getTablePrefix()."projekte WHERE ID = $log[project]");
    ...
    function getUserLog($user, $limit = 10)
    {
        ...
        $sel = mysql_query("SELECT * FROM ".$this->getTableName()." WHERE user = $user ORDER BY ID DESC LIMIT $limit");
    ...
    function getLog($limit = 5)
    {
        ...
        $mylog = array();
        $sel3 = mysql_query("SELECT ".$this->getTablePrefix()."projekt FROM projekte_assigned WHERE user = $userid");
        $prstring = "";
        ...
        if ($prstring)
        {
            $sel = mysql_query("SELECT * FROM ".$this->getTableName()."  WHERE project IN($prstring) OR project = 0 ORDER BY ID DESC LIMIT $limit");

            while ($log = mysql_fetch_array($sel))
            {
                $sel2 = mysql_query("SELECT name FROM ".$this->getTablePrefix()."projekte WHERE ID = $log[project]");
