...
class timetracker extends TableBase
{
    private $mylog;
    ...
    function add($user, $project, $task, $comment, $started, $ended, $logday = "")
    {
        ...
        $ins = mysql_query("INSERT INTO ".$this->getTableName()." (user,project,task,comment,started,ended,hours,pstatus) VALUES ($user,$project,$task,'$comment','$started','$ended','$hours',0)");
    ...
    function edit($id, $task, $comment, $started, $ended)
    {
        ...
        $upd = mysql_query("UPDATE ".$this->getTableName()." SET task='$task',comment='$comment',started='$started',ended='$ended',hours='$hours' WHERE ID = $id");
        if ($upd)
        {
            return true;
        }
    ...
    function setPaystatus($pstatus, $id)
    {
        $pstatus = (int) $pstatus;
        $id = (int) $id;
        $upd = mysql_query("UPDATE ".$this->getTableName()." SET pstatus = $pstatus WHERE ID = $id");
        if ($upd)
        {
            return true;
        }
    ...
    function getUserTrack($user, $project = 0, $task = 0, $start = 0, $end = 0 , $lim = 50)
    {
        ...
        if ($project > 0)
        {
            $sql = "SELECT * FROM ".$this->getTableName()." WHERE user = $user AND project = $project";
            $num = "SELECT COUNT(*) FROM ".$this->getTableName()." WHERE project = $project AND user = $user";
            $order = " ORDER BY ended ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$this->getTableName()." WHERE user = $user";
            $num = "SELECT COUNT(*) FROM ".$this->getTableName()." WHERE user = $user";
            $order = " ORDER BY ended ASC";
        }
        if ($task > 0)
        {
        ...
        if (isset($sel))
        {
            while ($data = @mysql_fetch_array($sel))
            {
                ...
                $pname = mysql_query("SELECT name FROM ".$this->getTablePrefix()."projekte WHERE ID = $data[project]");
                $pname = mysql_fetch_row($pname);
                $pname = stripslashes($pname[0]);

                $uname = mysql_query("SELECT name FROM ".$this->getTablePrefix()."user WHERE ID = $data[user]");
    ...
    function getProjectTrack($project, $user = 0, $task = 0, $start = 0, $end = 0, $lim = 50)
    {
        ...
        if ($user > 0)
        {
            $sql = "SELECT * FROM ".$this->getTableName()." WHERE project = $project AND user = $user";
            $num = "SELECT COUNT(*) FROM ".$this->getTableName()." WHERE project = $project AND user = $user";
            $order = " ORDER BY ended ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$this->getTableName()." WHERE project = $project";
            $num = "SELECT COUNT(*) FROM ".$this->getTableName()." WHERE project = $project";
            $order = " ORDER BY ended ASC";
        }
        if ($task > 0)
        {
