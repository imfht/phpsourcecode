...
class datei extends TableBase
{
    ...
    function __construct()
    {
        ...
        $this->table_name = 'projectfolders';
    }
    // FOLDER METHODS
    /**
     * Create a new folder
    ...
    function addFolder($parent, $project, $folder, $desc, $visible = "")
    {
        ...
        $ins = mysql_query("INSERT INTO ".$this->getTableName()." (parent,project,name,description,visible) VALUES ($parent,$project,'$folder','$desc','$visstr')");
        if ($ins)
        {
            // create the folder
            global $userid;
            ...
            $makefolder = CL_ROOT . "/files/" . CL_CONFIG . "$clientpath/$project/$folder/";
    ...
    {
        $id = (int) $id;
        $sel = mysql_query("SELECT * FROM ".$this->getTableName()." WHERE ID = $id LIMIT 1");
    ...
    {
        $parent = (int) $parent;
        $sel = mysql_query("SELECT * FROM ".$this->getTableName()." WHERE parent = $parent");
    ...
    function getProjectFolders($project, $parent = 0)
    {
        ...
        $sel = mysql_query("SELECT * FROM ".$this->getTableName()." WHERE project = $project AND parent = $parent");
        $folders = array();
    ...
    function getAllProjectFolders($project)
    {
        ...
        $sel = mysql_query("SELECT * FROM ".$this->getTableName()." WHERE project = $project");
        $folders = array();
    ...
    function edit($id, $title, $desc, $tags)
    {
        ...
        $sel = mysql_query("SELECT ".$this->getTablePrefix()."project FROM files WHERE ID = $id");
        $proj = mysql_fetch_row($sel);
        $project = $proj[0];

        $sql = mysql_query("UPDATE ".$this->getTablePrefix()."files SET `title` = '$title', `desc` = '$desc', `tags` = '$tags' WHERE id = $id");
        if ($sql)
        {
    ...
    function loeschen($datei)
    {
        ...
        $sel1 = mysql_query("SELECT datei,name,project,title FROM ".$this->getTablePrefix()."files WHERE ID = $datei");
        ...
        if (!empty($thisfile))
        {
            ...
            $del = mysql_query("DELETE FROM ".$this->getTablePrefix()."files WHERE ID = $datei");
            $del2 = mysql_query("DELETE FROM ".$this->getTablePrefix()."files_attached WHERE file = $datei");
            if ($del)
            {
                if (unlink($delfile))
                {
    ...
    function getFile($id)
    {
        ...
        $sel = mysql_query("SELECT * FROM ".$this->getTablePrefix()."files WHERE ID=$id");
    ...
    function moveFile($file, $target)
    {
        ...
        // update database
        $upd = mysql_query("UPDATE ".$this->getTablePrefix()."files SET datei = '$targetstr', folder = '$thefolder[ID]' WHERE ID = $thefile[ID]");
    ...
    function getProjectFiles($id, $lim = 25, $folder = "")
    {
        ...
        if ($folder > 0)
        {
            ...
            $sel = mysql_query("SELECT COUNT(*) FROM ".$this->getTablePrefix()."files WHERE project = $id AND folder = $folder ORDER BY ID DESC");
        }
        else
        {
            $sel = mysql_query("SELECT COUNT(*) FROM ".$this->getTablePrefix()."files WHERE project = $id AND folder = 0 ORDER BY ID DESC");
        }
        ...
        if ($folder > 0)
        {
            $sql = "SELECT ID FROM ".$this->getTablePrefix()."files WHERE project = $id AND folder = $folder ORDER BY  ID DESC LIMIT $start,$lim";
            $sel2 = mysql_query($sql);
        }
        else
        {
            $sel2 = mysql_query("SELECT ID FROM ".$this->getTablePrefix()."files WHERE project = $id AND folder = 0 ORDER BY  ID DESC LIMIT $start,$lim");
        }
    ...
    function getAllProjectFiles($id)
    {
        ...
        $sel2 = mysql_query("SELECT ID FROM ".$this->getTablePrefix()."files WHERE project = $id  ORDER BY  ID DESC");
     ...
     * @return bool $insid
     */
    public function add_file($name, $desc, $project, $milestone, $tags, $datei, $type, $title, $folder = 0, $visstr = "", $userid = 0)
    {
        ...
        $folder = (int) $folder;
        $userid = (int)$userid>0 ? (int)$userid : $_SESSION["userid"];
        $type = mysql_real_escape_string($type);
        $title = mysql_real_escape_string($title);
        $now = time();

        $ins = mysql_query("INSERT INTO ".$this->getTablePrefix()."files (`name`,`desc`,`project`,`milestone`,`user`,`tags`,`added`,`datei`,`type`,`title`,`folder`,`visible`) VALUES ('$name','$desc',$project,$milestone,$userid,'$tags','$now','$datei','$type','$title','$folder','$visstr')");
