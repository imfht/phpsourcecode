...
class tags extends TableBase
{
    ...
    {
        $this->cloudlimit = 0;
        $this->table_name = null; //This is not a table itself, but needs to access table prefix
    }
	...
	*/
    ...
    public function getTagcloud($project)
    {
        ...
        $sel1 = mysql_query("SELECT tags FROM ".$this->getTablePrefix()."files WHERE tags != '' AND project = $project");
        $sel2 = mysql_query("SELECT tags FROM ".$this->getTablePrefix()."messages WHERE tags != '' AND project = $project");
    ...
    private function getFiles($query, $project = 0)
    {
        ...
        if ($project > 0)
        {
            $sel = mysql_query("SELECT `ID`,`name`,`desc`,`type`,`datei`,`title`,`project`,`tags` FROM `".$this->getTablePrefix()."files` WHERE `tags` LIKE '%$query%' HAVING project = $project");
        }
        else
        {
            $sel = mysql_query("SELECT `ID`,`name`,`desc`,`type`,`datei`,`title`,`project`,`tags` FROM `".$this->getTablePrefix()."files` WHERE `tags` LIKE '%$query%'");
        }
        ...
        while ($result = mysql_fetch_array($sel))
        {
            if (!empty($result))
            {
                $project = mysql_query("SELECT name FROM ".$this->getTablePrefix()."projekte WHERE ID = $result[project]");
    ...
    private function getMessages($query, $project = 0)
    {
        ...
        if ($project > 0)
        {
            $sel = mysql_query("SELECT `ID`,`title`,`text`,`posted`,`user`,`username`,`project`,`tags` FROM ".$this->getTablePrefix()."messages WHERE `tags` LIKE '%$query%'  HAVING project = $project ");
        }
        else
        {
            $sel = mysql_query("SELECT `ID`,`title`,`text`,`posted`,`user`,`username`,`project`,`tags` FROM ".$this->getTablePrefix()."messages WHERE `tags` LIKE '%$query%'");
        }
    ...
    private function getUser($query)
    {
        ...
        $sel = mysql_query("SELECT `ID`,`email`,`name`,`avatar`,`lastlogin`,`tags` FROM ".$this->getTablePrefix()."user WHERE tags LIKE '%$query%'");
