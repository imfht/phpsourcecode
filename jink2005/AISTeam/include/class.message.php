...
class message extends TableBase
{
    public $mylog;
    ...
    function add($project, $title, $text, $tags, $user, $username, $replyto, $milestone)
    {
        ...
        $sql = "INSERT INTO ".$this->getTableName()." (`project`,`title`,`text`,`tags`,`posted`,`user`,`username`,`replyto`,`milestone`) VALUES ($project,'$title','$text','$tags','$posted',$user,'$username',$replyto,$milestone) ";
        ...
        if ($ins)
        {
            $this->mylog->add($title, 'messages', 1, $project);
            return $insid;
        }
        else
        {
    ...
    function edit($id, $title, $text, $tags)
    {
        ...
        $upd = mysql_query("UPDATE ".$this->getTableName()." SET title='$title', text='$text', tags='$tags' WHERE ID = $id");

        if ($upd)
        {
            $proj = mysql_query("SELECT ".$this->getTablePrefix()."project FROM ".$this->getTableName()." WHERE ID = $id");
    ...
    function del($id)
    {
        ...
        $msg = mysql_query("SELECT title,project FROM ".$this->getTableName()." WHERE ID = $id");
        $msg = mysql_fetch_row($msg);

        $del = mysql_query("DELETE FROM ".$this->getTableName()." WHERE ID = $id LIMIT 1");
        $del2 = mysql_query("DELETE FROM ".$this->getTableName()." WHERE replyto = $id");
        $del3 = mysql_query("DELETE FROM ".$this->getTablePrefix()."files_attached WHERE message = $id");
        if ($del)
        {
    ...
    function getMessage($id)
    {
        ...
        if (!empty($message))
        {
            $replies = mysql_query("SELECT COUNT(*) FROM ".$this->getTableName()." WHERE replyto = $id");
            ...
            $sel = mysql_query("SELECT gender FROM ".$this->getTablePrefix()."user WHERE ID = $message[user]");
            $ds = mysql_fetch_row($sel);
            $gender = $ds[0];
            $message["gender"] = $gender;

            $project = mysql_query("SELECT name FROM ".$this->getTablePrefix()."projekte WHERE ID = $message[project]");
    ...
    function getReplies($id)
    {
        ...
        $sel = mysql_query("SELECT ID FROM ".$this->getTableName()." WHERE replyto = $id ORDER BY posted DESC");
        $replies = array();
    ...
    function getLatestMessages($limit = 5)
    {
        ...
        $sel3 = mysql_query("SELECT projekt FROM ".$this->getTablePrefix()."projekte_assigned WHERE user = $userid");
        $prstring = "";
        ...
        if ($prstring)
        {
            $sel1 = mysql_query("SELECT ID FROM ".$this->getTableName()." WHERE project IN($prstring) ORDER BY posted DESC LIMIT $limit ");
            $messages = array();
    ...
    function getProjectMessages($project)
    {
        ...
        $messages = array();
        $sel1 = mysql_query("SELECT ID FROM ".$this->getTableName()." WHERE project = $project AND replyto = 0 ORDER BY posted DESC");
    ...
    function attachFile($fid, $mid, $id = 0)
    {
        ...
        if ($fid > 0)
        {
            $ins = mysql_query("INSERT INTO ".$this->getTablePrefix()."files_attached (ID,file,message) VALUES ('',$fid,$mid)");
        }
        else
        {
            $num = $_POST["numfiles"];
            ...
            for($i = 1;$i <= $num;$i++)
            {
                ...
                $clientpath = $clientobj->getClientPath($_SESSION['userid']);
                if(!file_exists(CL_ROOT . "/files/" . CL_CONFIG . "$clientpath/"))
                    mkdir(CL_ROOT . "/files/" . CL_CONFIG . "$clientpath/", 0777);
                $fid = $myfile->upload("userfile$i", "files/" . CL_CONFIG . "$clientpath/$id", $id);
                $ins = mysql_query("INSERT INTO ".$this->getTablePrefix()."files_attached (ID,file,message) VALUES ('',$fid,$mid)");
            }
        }
        if ($ins)
    ...
    private function getAttachedFiles($msg)
    {
        ...
        $files = array();
        $sel = mysql_query("SELECT file FROM ".$this->getTablePrefix()."files_attached WHERE message = $msg");
        while ($file = mysql_fetch_row($sel))
        {
            $sel2 = mysql_query("SELECT * FROM ".$this->getTablePrefix()."files WHERE ID = $file[0]");
