...
$efforttocomplete = str_replace(",",".", getArrayVal($_POST, "efforttocomplete"));
$redirect_tracker = isset($_POST["redirect_tracker"])? true : false;
...
if ($action == "add")
{
    ...
    if ($efforttocomplete >= 0) {
    	// create a task class object to access the task methods
    	$taskobj = new task();
    	
    	// efforttocomplete is 0 only when the task is set to be closed
    	if ($efforttocomplete == 0) {
            $taskobj->close($task); // close task
    	}
    	// update the efforttocomplete for the task
    	else {
    		$t = $taskobj->getTask($task);
    		$taskobj->editEffortToComplete($t['ID'], $efforttocomplete);
    	}
    }
	if ($tracker->add($userid, $tproject, $task, $comment, $started, $ended, $logdate))
    {
        ...
        {
            $loc = $url . ($redirect_tracker ? "managetimetracker.php?action=showproject&id=$tproject&mode=timeadded" : "manageproject.php?action=showproject&id=$tproject&mode=timeadded");
            header("Location: $loc");
        }
    }
    else
...
} elseif ($action == "showproject")
{
    ...
    foreach($ptasks as $task) {
        $etcrequired[$task['ID']] = $task['optionaletc'];
    } 
    ...
    $template->assign("etcrequired", $etcrequired);
    $template->assign("start", $start);
    $template->assign("end", $end);
