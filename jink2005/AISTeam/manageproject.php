...
if ($action == "editform")
{
    if (!$userpermissions["projects"]["edit"] || ($project->getProjectClient($id)!=$userobj->getProfileField($userid, "client_id") && !$userpermissions["admin"]["root"]))
    {
...
} elseif ($action == "edit")
{
    ...
    if ($project->edit($id, $name, $desc, $start, $end, $budget))
    {
...
} elseif ($action == "donegraph")
{
    ...
    echo $g->render();
} elseif ($action == "showproject" || $action == "showactivities")
{
    ...
    if (getArrayVal($_COOKIE, "chartpro"))
    {
        $chartprostyle = "display:" . $_COOKIE['chartpro'];
        $template->assign("chartprostyle", $chartprostyle);
    }
    ...
    // Get the total time tracked for this project in hours, then convert to days
    $timetracker = new timetracker();
    $track = $timetracker->getProjectTrack($id);
    $hours = $track == false ? 0 : floatval($timetracker->getTotalTrackTime($track));
    // This is the actual effort in days
    $actualeffort = round($hours / 8.0, 2);
    $template->assign("actualeffort", $actualeffort);
    
    $template->assign("chart_data", $task->estimateChartData($task->estimateListProject($id), $project->estimateListProject($id)));
    $template->assign("chart_data2", $task->estimateChartData($task->estimateListProject2($id), $project->estimateListProject($id,"planned")));
    
    $task = new task();
    $tasks = $task->getProjectTasks($id);
    $workpackage = new tasklist();
    $workpackages = $workpackage->getProjectTasklists($id);
    $milestone = new milestone();
    $milestones = $milestone->getProjectMilestones($id);
    $template->assign("tasknum", $tasks ? count($tasks) : 0);
    $template->assign("workpackagenum", $workpackages ? count($workpackages) : 0);
    $template->assign("milestonenum", $milestones ? count($milestones) : 0);
    SmartyPaginate::assign($template);
    $template->display($action == "showactivities" ? "projectactivities.tpl" : "project.tpl");
} elseif ($action == "cal")
{
    if (!chkproject($userid, $id))
    {
	...
	$date = getArrayVal($_GET, "date");
	$date = strtotime($date!="" ? $date : date("Y-m-d"));
	
	$week_day = date("N", $date)-1;
	$start = $date-$week_day*86400;
	$end = $date+(6-$week_day)*86400;
	
	$events = array();
	for($d=$start;$d<=$end;$d=$d+86400) {
		$taskobj = new task();
		$tasks = $taskobj->getTodayTasks(date("m", $d),date("Y", $d),date("d", $d));
		$milestoneobj = new milestone();
		$milestones = $milestoneobj->getTodayMilestones(date("m", $d),date("Y", $d),date("d", $d));
		$events[$d] = array("tasks" => $tasks, "milestones" => $milestones);
	}
	
	$template->assign("date", array("prev"=>date("Y-m-d", $date-7*86400), "next"=>date("Y-m-d", $date+7*86400)));
	$template->assign("events", $events);
	$template->display("calendar.tpl");
}