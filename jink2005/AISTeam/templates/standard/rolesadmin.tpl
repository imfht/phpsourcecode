					...
					{/if}
						<tr>
							...
							<!--	<a class="tool_edit" href="manageproject.php?action=editform&amp;id={$roles[role].ID}" title="{#edit#}" ></a>-->
								{if $userpermissions.admin.root || $roles[role].client>0}<a class="tool_del" href="javascript:confirmfunction('{#confirmdel#}','deleteElement(\'role_{$roles[role].ID}\',\'manageroles.php?action=delrole&amp;id={$roles[role].ID}\')');"  title="{#delete#}"></a>{/if}
							</td>
						</tr>
	...
	<fieldset>
        ...
        <div class = "row"><label>{#permissions#}:</label>
            ...
            <div class = "row">
            <label></label><input type = "checkbox" class = "checkbox" value = "1" name = "permissions_user[add]" {if $roles[role].user.add}checked{/if}  />{#add#}
            </div>
            <div class = "row">
            <label></label><input type = "checkbox" class = "checkbox" value = "1" name = "permissions_user[edit]" {if $roles[role].user.edit}checked{/if}  />{#edit#}
            </div>
            <div class = "row">
            <label></label><input type = "checkbox" class = "checkbox" value = "1" name = "permissions_user[del]" {if $roles[role].user.del}checked{/if}  />{#delete#}
            </div>
...
{if $userpermissions.admin.root}
            <div class = "row">
            <label></label><input type = "checkbox" class = "checkbox" value = "1" name = "permissions_admin[root]" {if $roles[role].admin.root}checked{/if}  />{#rootadmin#}
            </div>
{/if}
            <div class = "row">
            ...
            <div class = "row">
            <label></label><input type = "checkbox" class = "checkbox" value = "1" name = "permissions_api[read]" {if $roles[role].api.read}checked{/if} />{#read#}
            </div>
            <div class = "row">
            <label></label><input type = "checkbox" class = "checkbox" value = "1" name = "permissions_api[write]" {if $roles[role].api.write}checked{/if} />{#write#}
            </div>
        </div>
