...
{section name=list loop=$lists}
			...
			<div class="headline">
				<a href="javascript:void(0);" id="block-{$lists[list].ID}_toggle" class="win_block" onclick = "toggleBlock('workpblock-{$lists[list].ID}');"></a>

				<div class="wintools">
                    <a href="#" style="background:none;font-weight:bold;color:{if $lists[list].daystostart > 0}#00ff66{else}#ff4444{/if};width:25px;font-size:16px;">{$lists[list].daystostart}<span>Days left until Start</span></a>
                    <a href="#" style="background:none;font-weight:bold;color:{if $lists[list].daystofinish > 0}#00ff66{else}#ff4444{/if};width:25px;font-size:16px;">{$lists[list].daystofinish}<span>Days left until Finish</span></a>
                ...
                <div style="position:absolute; top:6px; width:200px; text-align:center; left:255px;color:#fff; height:35px; font-weight: bold; font-size: 18px">{$lists[list].responsible}</div>
			</div>

{assign var=style value=workpblock-`$lists[list].ID`}
{assign var=style value=$smarty.cookies.$style}
			<div id="workpblock-{$lists[list].ID}" class="block"{if $style!=""} style="display:{$style}"{/if}>
				...
				<table id="acc_{$lists[list].ID}" cellpadding="0" cellspacing="0" border="0">
					...
					<thead>
						<tr>
							...
							<th class="actual">{#efforttocomplete#} ({#actualeffort#})</th>
							<th class="tools"></th>
						</tr>
					</thead>
					...
					{/if}
						<tr {if $lists[list].tasks[task].daysleft < 0} class="marker-late"{elseif $lists[list].tasks[task].daysleft == 0} class="marker-today"{/if}>
							...
							<td>{$lists[list].tasks[task].priority_name}</td>
							<td>{$lists[list].tasks[task].efforttocomplete} ({$lists[list].tasks[task].actual})</td>
							<td class="tools">
								{if $userpermissions.tasks.edit}
					...
					<table cellpadding="0" cellspacing="0" border="0" id = "done_{$lists[list].ID}">
						...
						{/if}
							...
							<tr>
								...
								<td><a href="manageuser.php?action=profile&amp;id={$lists[list].oldtasks[oldtask].user_id}">{$lists[list].oldtasks[oldtask].user|truncate:23:"...":true}</a></td>
								<td class="priority">{$lists[list].oldtasks[oldtask].priority_name}</td>
								<td class="actual">{$lists[list].oldtasks[oldtask].efforttocomplete} ({$lists[list].oldtasks[oldtask].actual})</td>
								<td class="tools">
									{if $userpermissions.tasks.edit}
