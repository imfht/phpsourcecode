...
<div class="tasks">
	...
	{if $myprojects[project].tasknum}
			...
			<div class="headline">
				<a href="javascript:void(0);" id="block-{$myprojects[project].ID}_toggle" class="win_block" onclick = "toggleBlock('taskblock-{$myprojects[project].ID}');"></a>
...
{assign var=style value=taskblock-`$myprojects[project].ID`}
{assign var=style value=$smarty.cookies.$style}
			<div id="taskblock-{$myprojects[project].ID}" class="block"{if $style!=""} style="display:{$style}"{/if}>
				...
				<table id="acc_{$myprojects[project].ID}" cellpadding="0" cellspacing="0" border="0">
					...
					{/if}
						<tr {if $myprojects[project].tasks[task].daysleft < 0} class="marker-late"{elseif $myprojects[project].tasks[task].daysleft == 0} class="marker-today"{/if}>
							...
							<td><a href="managetasklist.php?action=showtasklist&amp;&amp;tlid={$myprojects[project].tasks[task].workpackage}&amp;id={$myprojects[project].ID}">{$myprojects[project].tasks[task].list|truncate:23:"...":true}</a></td>
							<td>{$myprojects[project].tasks[task].priority_name}</td>
					...
					<table cellpadding="0" cellspacing="0" border="0" id = "done_{$myprojects[project].ID}">
						...
						{/if}
							...
							<tr>
								...
								<td class="c"><a href="managetasklist.php?action=showtasklist&amp;&amp;tlid={$myprojects[project].oldtasks[oldtask].workpackage}&amp;id={$myprojects[project].ID}">{$myprojects[project].oldtasks[oldtask].list|truncate:23:"...":true}</a></td>
								<td class="c">{$myprojects[project].oldtasks[task].priority_name}</td>
