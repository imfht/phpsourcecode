...
<h1>{$projectname}<span>/ {#files#}</span></h1>
			...
			<div id="block_files" class="blockwrapper"{if $smarty.cookies.block_files!=""} style="display:{$smarty.cookies.block_files}"{/if}>
				{*Add Folder*}
				{if $userpermissions.files.add}
			...
			<div class="tablemenue">
					<div class="tablemenue-in">
						{if $userpermissions.files.add}
						<a class="butn_link" href="#" onclick="blindtoggle('form_file');document.getElementById('upfolder').options[document.getElementById('folderid_current').value].selected = true;return false;" id="add_file_butn" onclick="toggleClass('addfile','addfile-active','addfile');toggleClass(this,'butn_link_active','butn_link');toggleClass('sm_files','smooth','nosmooth');">{#addfile#}</a>
						<a class="butn_link" href="#" onclick="blindtoggle('form_folder');document.getElementById('folderparent').options[document.getElementById('folderid_current').value].selected = true;return false;" id="add_folder_butn" onclick="toggleClass(this,'butn_link_active','butn_link');toggleClass('addfolder','addfolder-active','addfolder');toggleClass('sm_files','smooth','nosmooth');">{#addfolder#}</a>
						{/if}
					</div>
			</div>
			</div> {*block_files End*}
