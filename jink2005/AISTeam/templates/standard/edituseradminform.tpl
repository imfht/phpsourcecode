...
<tr>
	...
	<a href="#avatarbig" id="ausloeser">
		<div class="avatar-profile"><img src = "thumb.php?pic=files/{$cl_config}{$user.client_path}/avatar/{$user.avatar}&amp;width=122;" alt="" /></div>
	</a>
	{else}
		{if $user.gender == "f"}
	...
	<div id="avatarbig" style="display:none;">
		<a href="javascript:Control.Modal.close();"><img src = "thumb.php?pic=files/{$cl_config}{$user.client_path}/avatar/{$user.avatar}&amp;width=480&amp;height=480;" alt="" /></a>
	</div>
				...
				<tbody class="color-a">
					<tr>
						...
						<select name="client_id" id="client_id">
							{section name=client loop=$clients}<option value = "{$clients[client].ID}"{if $user.client_id==$clients[client].ID} selected="selected"{/if}>{$clients[client].name}</option>{/section}
						</select>
						</td>
					</tr>
				</tbody>
{/if}
				<tbody class="color-a">
					<tr>
						<td><label for = "client">{#company#}:</label></td>
						<td class="right"><input type="text" class="text" value = "{$user.company}" name = "company" id="company" /></td>
					</tr>
				</tbody>
