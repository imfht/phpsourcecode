...
<h1>{$projectname}<span>/ {#members#}</span></h1>
			...
			<div id="block_members" class="blockwrapper"{if $smarty.cookies.block_members!=""} style="display:{$smarty.cookies.block_members}"{/if}>
				{*Add User*}
				{if $userpermissions.user.add}
									...
									<div class="itemwrapper" id="iw_{$folders[fold].ID}">
											...
											<div class="moreinfo-wrapper">
												<div class="moreinfo" id="info_{$members[member].ID}" style="display:none">
													<img src = "thumb.php?pic=files/{$cl_config}{$members[member].client_path}/avatar/{$members[member].avatar}&amp;width=82" alt="" onclick="fadeToggle('info_{$members[member].ID}');" />
