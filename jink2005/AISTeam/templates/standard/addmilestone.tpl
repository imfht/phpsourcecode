			...
			<h2>{#addmilestone#}</h2>
				...
				<form class="main" method="post" action="managemilestone.php?action=add&amp;id={$project.ID}" {literal} onsubmit="return validateCompleteForm(this);"{/literal} >
					<fieldset>
						...
						<div class = "row">
							<label for="desc">{#description#}:</label><textarea name="desc" id="desc" realname="{#description#}" rows="3" cols="1" ></textarea>
							<br/>
						</div>

                        <div class="row">
                            <label for="external">{#external#}:</label>
                            <input type="checkbox" class="checkbox" name="external"  id="external" realname="{#external#}" />
                        </div>
