...
if ($action == "addform") {
    ...
    // add the task
    $tid = $task->add($end, $title, $text, $tasklist, $id, $efforttocomplete, $optionaletc, $priority);
    ...
    $tl = $tasklist->getTasklist($thistask['workpackage']);
    ...
    $tl["finishdate"] = strtotime($tl["finishdate"]);
    ...
    // edit the task
    if ($task->edit($tid, $end, $title, $text, $tasklist, $efforttocomplete, $optionaletc, $priority)) {
...
} elseif ($action == "showproject" || $action == "showprojecttasks") {
    ...
    $template->display($action == "showprojecttasks" ? "projectalltasks.tpl" : "projecttasks.tpl");
    ...
    $tasklist = new tasklist();
    $tasklists = $tasklist->getProjectTasklists($id);
    $tl = $tasklist->getTasklist($task['workpackage']);
    $task['listid'] = $tl['ID'];
