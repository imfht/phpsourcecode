...
//               $this->_sign();
   ...
   public function getUser(){
    ...
    public function __s($msg='',$data=null,$code=200,$header=[],$type=''){
        ...
        $data = (is_array($data) && $data) ? ['status'=>1,'msg'=>$msg,'code'=>$code,'data'=>$data,'sign'=>$this->sign] : ['status'=>1,'msg'=>$msg,'code'=>$code,'sign'=>$this->sign];
     ...
     * @return \think\Response
     */
   public function __e($msg='',$data=[],$code=200,$header=[],$type=''){
       $data = is_object($data) ? $data->toArray() : $data;
       $msg = $msg ? $msg : 'error';
       $type = $type?$type : config('app.default_return_type');
       $data = (is_array($data) && $data) ? array_merge($data,['status'=>0,'code'=>$code]) : ['status'=>0,'msg'=>$msg,'code'=>$code];
       return response($data,$code,$header,$type)->send();
   }
}