...
@ob_end_flush();
     ...
     */
    function backup($tablename = '', $dir, $size, $time) {
        $dir = $dir ? $dir : './backup/';
        // 创建目录
        if (! is_dir ( $dir )) {
            ...
            while ( $volume_id ) {
                ...
                if (file_exists ( $tmpfile )) {
                    ...
                    if ($this->_import ( $tmpfile )) {
                        ...
                        $this->_showMsg ("导入分卷：<span style='color:#f00;'>" . $tmpfile . '</span>失败！可能是数据库结构已损坏！请尝试从分卷1开始导入');
                        exit ();
                    }
                } else {
