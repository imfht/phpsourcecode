...
class Phpredis {
    ...
    public function listPush($list, $value, $direction = 0, $repeat = 0){
        ...
        switch($direction){
            case 0 :
                if($repeat) $return = $this->_REDIS->lPushx($list, $value);
                else $return = $this->_REDIS->lPush($list, $value);
                break;
            case 1 :
                if($repeat) $return = $this->_REDIS->rPushx($list, $value);
                else $return = $this->_REDIS->rPush($list, $value);
                break;
            default :
                $return = false;
                break;
    ...
    public function listPop($list1, $deriction = 0, $list2 = null, $timeout = 0){
        ...
        switch($direction){
            case 0 :
                if($timeout && $list2) $return = $this->_REDIS->blPop($list1, $list2, $timeout);
                else $return = $this->_REDIS->lPop($list1);
                break;
            case 1 :
                if($timeout && $list2) $return = $this->_REDIS->brPop($list1, $list2, $timeout);
                else $return = $this->_REDIS->rPop($list1);
                break;
            default :
                $return = false;
                break;
...
;
    ...
    public function hashGet($hash, $key = array(), $type = 0){
        ...
        if($key){
            if(is_array($key) && !empty($key)) $return = $this->_REDIS->hMGet($hash, $key);
            else $return = $this->_REDIS->hGet($hash, $key);
        }else{
            switch($type){
                case 0 :
