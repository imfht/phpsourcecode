 ...
 * 微博登录用户处理
 */
class WeiboUser_model extends MY_Model
{
    ...
    {
        if ($this->db->insert('weibo_user_tb', $open_user)) {
    ...
    public function update($open_user)
    {
        ...
        if ($this->db->update('weibo_user_tb', $open_user, $where)) {
            return $this->get_by_openid($open_user['openid']);
        }
        return 0;
    }
    ...
    public function del_by_userId($user_id)
    {
        ...
        return $this->db->delete('weibo_user_tb', $where);
    }
}
