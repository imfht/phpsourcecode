 ...
 * 开源中国登录用户处理
 */
class OschinaUser_model extends MY_Model
{
    ...
    {
        if ($this->db->insert('oschina_user_tb', $open_user)) {
    ...
    public function update($open_user)
    {
        ...
        if ($this->db->update('oschina_user_tb', $open_user, $where)) {
            return $this->get_by_openid($open_user['openid']);
        }
        return 0;
    }
    ...
    public function del_by_userId($user_id)
    {
        ...
        return $this->db->delete('oschina_user_tb', $where);
    }
}
