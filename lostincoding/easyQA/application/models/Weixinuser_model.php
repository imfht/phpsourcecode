 ...
 * 微信登录用户处理
 */
class WeixinUser_model extends MY_Model
{
     ...
     */
    public function get_by_unionid($unionid)
    {
    ...
    {
        if ($this->db->insert('weixin_user_tb', $open_user)) {
            return $this->get_by_openid($open_user['unionid']);
        }
        return 0;
    }
    ...
    public function update($open_user)
    {
        ...
        if ($this->db->update('weixin_user_tb', $open_user, $where)) {
    ...
    public function del_by_userId($user_id)
    {
        ...
        return $this->db->delete('weixin_user_tb', $where);
    }
}
