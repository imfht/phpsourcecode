...
class Q extends MY_Controller
{
    ...
    public function index($type = 'latest', $page_index = 1, $page_size = 50)
    {
        ...
        if ($type == 'latest') {
            $this->data['title'] = '最新问答';
            $this->data['article_lists'] = $this->article_model->gets_by_latest(1, $page_index, $page_size);
            $this->data['article_counts'] = $this->article_model->get_counts(1);
        } else if ($type == 'fine') {
            $this->data['title'] = '精帖问答';
            $this->data['article_lists'] = $this->article_model->gets_by_fine(1, $page_index, $page_size);
            $this->data['article_counts'] = $this->article_model->get_counts_by_fine(1);
        }

        //分页
        $config['base_url'] = base_url("q/{$type}/");
