...
class Discuss extends MY_Controller
{
    ...
    public function index($type = 'latest', $page_index = 1, $page_size = 50)
    {
        ...
        if ($type == 'latest') {
            $this->data['title'] = '最新讨论';
            $this->data['article_lists'] = $this->article_model->gets_by_latest(2, $page_index, $page_size);
            $this->data['article_counts'] = $this->article_model->get_counts(2);
        } else if ($type == 'fine') {
            $this->data['title'] = '精帖讨论';
            $this->data['article_lists'] = $this->article_model->gets_by_fine(2, $page_index, $page_size);
            $this->data['article_counts'] = $this->article_model->get_counts_by_fine(2);
        }

        //分页
        $config['base_url'] = base_url("discuss/{$type}/");
