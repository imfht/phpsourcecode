...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        //   <div class="am-progress-bar am-progress-bar-info" role="progressbar" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100" style="width: 20%">
        ...
        progressBarWrapper.addClass("am-progress am-progress-striped");

        var progressBar = $("<div/>");
        progressBar.addClass("am-progress-bar am-progress-bar-info")
...
FileProgress.prototype.reset = function() {
    ...
    this.fileProgressWrapper.find('td .am-progress .am-progress-bar-info').attr('aria-valuenow', 0).width('0%').find('span').text('');
    this.appear();
};
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    for (var i = 1; i <= chunk_amount; i++) {
        var col = $('<div class="am-u-sm-2"/>');
        var progressBarWrapper = $('<div class="am-progress am-progress-striped"></div');

        var progressBar = $("<div/>");
        progressBar.addClass("am-progress-bar am-progress-bar-info text-left")
            .attr('role', 'progressbar')
            .attr('aria-valuemax', 100)
            .attr('aria-valuenow', 0)
            .attr('aria-valuein', 0)
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    var progressbar = this.fileProgressWrapper.find('td .am-progress').find('.am-progress-bar-info');
...
FileProgress.prototype.setComplete = function(up, res) {
    var td = this.fileProgressWrapper.find('td:eq(2)'),
        tdProgress = td.find('.am-progress');

    //var res = $.parseJSON(info);
    var url;
    if (res.url) {
        url = res.url;
        if(res.privateDownloadUrl){
            str = '<div></strong><a href="' + res.privateDownloadUrl + '" target="_blank">下载地址</a></div>';
        }
        else{
            str = '<div><a href="' + res.url + '" target="_blank">下载地址</a></div>';
        }
    } else {
        return;
    }
    ...
    var showImg = $('<img class="file_icon" src="/static/img/pan/default.png"/>');

    progressNameTd.prepend(showImg);

    if (!isImg) {
        showImg.attr('src', '/static/img/pan/default.png');
        progressNameTd.prepend(showImg);
    } else {
        if(res.privateDownloadUrl){
            showImg.attr('src', res.privateDownloadUrl);
        }
        else{
...
FileProgress.prototype.setError = function() {
    ...
    this.fileProgressWrapper.find('td:eq(2) .am-progress').css('width', 0).hide();
...
FileProgress.prototype.setCancelled = function(manual) {
    ...
    this.fileProgressWrapper.attr('class', progressContainer);
    this.fileProgressWrapper.find('td .am-progress').remove();
