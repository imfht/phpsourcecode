...
/**
 * 生成签名结果
 * @param <type> $sort_array	要签名的数组
 * @param <type> $key
 * @param <type> $sign_type
 * @return <type>		签名结果字符串
 */
...
/**
 * 日志消息,把支付宝返回的参数记录下来（请注意服务器是否开通fopen配置）
 * @param <type> $word 要写入的日志数据
 */
function  log_result($word) {
    $fp = fopen( WWWROOT . "data/alipay_log.txt","a");
    flock($fp, LOCK_EX) ;
...
/**
 * 实现多种字符编码方式
 * @param <type> $input		    需要编码的字符串
 * @param <type> $_output_charset   输出的编码格式
 * @param <type> $_input_charset    输入的编码格式
 * @return <type>		    编码后的字符串
 */
...
/**
 * 实现多种字符解码方式
 * @param <type> $input		    需要解码的字符串
 * @param <type> $_input_charset    输出的解码格式
 * @param <type> $_output_charset   输入的解码格式
 * @return <type>		    解码后的字符串
 */
