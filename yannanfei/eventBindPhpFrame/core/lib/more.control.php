...
class Control{
    ...
    protected function  display_layout($main, $layout = 'layout',$layout_setting=array())
    {
        ...
        //标题自动添加返回按钮
        $jmp_url=$_SERVER['HTTP_REFERER'];
        $a='&nbsp;&nbsp;<a href="'.$jmp_url.'" class="btn btn-info">返回</a>';
        $main = preg_replace('~(<h2 class="sub-header">.+?)</h2>~', '\\1'.$a.'</h2>', $main, 1);//只替换一次
    ...
    protected  function  fetch_layout($main, $layout = 'layout',$layout_setting=array()){
        ...
        $te_t=c('template');
        if($te_t&&$te_t!='views'){
            $layout_file = BASE_PATH.'/template/'.$te_t.'/'.$layout.'.html';
        }
        else{
        ...
        //美容合众app设置layout的美容院名称，特有特性，节约时间


        $meirong_name=c('meirong_name');
        $content = str_replace( array('../resource','../core/','../uploads','{{meirong_name}}'),array('./resource','../../core/','../../separate/'.APP_NAME.'/uploads',$meirong_name),$content);
    ...
    protected  function  show_message($title,$jump_url='index.php',$detail='',$count_down=5,$exit=true){
        ...
        if(file_exists(BASE_PATH.'/views/show_message.html')){
             ...
             $content=file_get_contents(ROOT_PATH.'/tpl/show_message.tpl');
        }
