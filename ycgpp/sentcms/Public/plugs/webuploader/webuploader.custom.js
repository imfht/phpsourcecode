...
;(function($, w) {
		...
		initUploader: function() {
			...
			if (!self.options.uploadEvents.uploadSuccess) {
				self.BDUploader.on('uploadSuccess', function(file, response) {
					...
					var filebox_html = '';
					if (type == 'image') {
						filebox_html = '<img src="' + response.info.path + '" class="img-responsive" />';
					}else{
						filebox_html = '';
					}
	...
	$.fn[pluginName].defaults = {
		...
		*/
		...
		server: 'index.php?m=admin&c=File&a=upload&flash=1', // 文件接收服务端。
		/**
		...
		delUrl: 'index.php?m=admin&c=File&a=delete',
		separator: ',', //默认逗号
	}
