...
class Datatable{
	...
	public function __construct(){
		...
		$this->model_table_prefix = C('MODEL_TABLE_PREFIX');
	}
	 ...
	 * @author colin <colin@tensent.cn>
	 */
	public function create_id($length = 11 , $comment = '主键' , $is_auto_increment = true){
		$auto_increment = $is_auto_increment ? 'AUTO_INCREMENT' : '';
		$this->sql .= "`id` int({$length}) unsigned NOT NULL $auto_increment COMMENT '{$comment}',";
		return $this;
	}
	...
	public function colum_field($table,$attr = array()){
		...
		$field_attr['comment'] = $attr['comment'] ? $attr['comment'] : '';
		$field_attr['oldname'] = $attr['oldname'];
		$field_attr['newname'] = $attr['newname'] ? $attr['newname'] : $field_attr['field'];
		$field_attr['after'] = $attr['after'] ? ' AFTER `'.$attr['after'].'`' : '';
		$field_attr['action'] = $attr['action'] ? $attr['action'] : 'ADD';
		//确认表是否存在
	...
	public function create(){
		$res = $this->model->execute($this->sql);
		return $res !== false;
	}
	...
	public function CheckTable($table){
		//获取表名
		$this->table = $this->getTablename($table,true);
		$result = $this->model->execute("SHOW TABLES LIKE '%$this->table%'");
		return $result;
	}
	...
	public function CheckField($table,$field){
		...
		if(!$this->model->query("Describe $table $field")){
			return false;
		}else{
			return true;
		}
