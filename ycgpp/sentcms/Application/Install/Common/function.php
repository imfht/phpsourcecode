...
function check_dirfile(){
    $items = array(
        ...
        array('dir', '可写', 'success', './Data/Conf')
...
function write_config($config, $auth){
    if(is_array($config)){
        ...
        if(!IS_WRITE){
            ...
            <p>./Data/Conf/config.php</p>
            <textarea name="" style="width:650px;height:125px">'.$user.'</textarea>';
        }else{
            if(file_put_contents('./Data/Conf/config.php', $conf)){
                show_msg('配置文件写入成功');
            } else {
...
function register_administrator($db, $prefix, $admin,$auth){
    ...
    $sql = str_replace(
        ...
        array($prefix, $admin['username'],$password, $admin['email'],$salt, NOW_TIME, get_client_ip(1)),
        $sql);
    $db->execute($sql);
    show_msg('创始人帐号注册完成！');
}
