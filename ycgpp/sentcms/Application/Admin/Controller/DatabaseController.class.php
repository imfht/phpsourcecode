...
class DatabaseController extends \Common\Controller\AdminController {
	...
	public function export($tables = null, $id = null, $start = null) {
		if (IS_POST && !empty($tables) && is_array($tables)) { //初始化
			...
			//读取备份配置
			$config = array('path' => realpath($path) . DIRECTORY_SEPARATOR, 'part' => C('DATA_BACKUP_PART_SIZE'), 'compress' => C('DATA_BACKUP_COMPRESS'), 'level' => C('DATA_BACKUP_COMPRESS_LEVEL'),);
			//检查是否有正在执行的任务
	...
	public function import($time = 0, $part = null, $start = null) {
		...
		elseif (is_numeric($part) && is_numeric($start)) {
			...
			$db = new Database($list[$part], array('path' => realpath(C('DATA_BACKUP_PATH')) . DIRECTORY_SEPARATOR, 'compress' => $list[$part][2]));
