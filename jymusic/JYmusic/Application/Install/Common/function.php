...
function check_dirfile(){
    $items = array(
        array('dir',  '可写', 'success', './Uploads/Avatars'),
        array('dir',  '可写', 'success', './Uploads/Picture'),
        array('dir',  '可写', 'success', './Uploads/Editor'),
		array('dir',  '可写', 'success', './Uploads/UserPic'),
		array('dir',  '可写', 'success', './Uploads/UserUp'),
		...
		array('file', '可写', 'success', './Application/Common/Common/function.php'),
...
function write_config($config, $auth){
    if(is_array($config)){
		...
		$home_view = '<?php	return array (
		  "URL_MODEL" => "3",
		  "HTML_CACHE_ON" => "0",
		  "HTML_CACHE_TIME" => "86400",
		  "HTML_FILE_SUFFIX" => ".html",
		  "SHOW_PAGE_TRACE" => "0",
		  "DEFAULT_THEME" => "JYmusic",
		  "TMPL_PARSE_STRING" => 
		  array (
			"__STATIC__" => "'.__ROOT__.'/Public/static",
			"__TMPL__" => "'.__ROOT__.'/Template/theme/JYmusic",
		  ),
		);';
		$user_view =  '<?php	return array (
		  "URL_MODEL" => "3",
		  "HTML_CACHE_ON" => "0",
		  "HTML_CACHE_TIME" => "86400",
		  "HTML_FILE_SUFFIX" => ".html",
		  "SHOW_PAGE_TRACE" => "0",
		  "VIEW_PATH"=> "./Template/theme/JYmusic/",
		  "DEFAULT_THEME" => "User",
		  "TMPL_PARSE_STRING" => 
		  array (
			"__STATIC__" => "'.__ROOT__.'/Public/static",
		...
		$up_drive  = file_get_contents(MODULE_PATH . 'Data/up_drive_config.tpl');
		$email = file_get_contents(MODULE_PATH . 'Data/email_config.tpl');
        //替换配置项
        ...
        if(!IS_WRITE){
            ...
            if(file_put_contents(APP_PATH . 'Common/Conf/config.php', $conf) &&
               file_put_contents(APP_PATH . 'User/Conf/config.php', $user) &&
				file_put_contents(APP_PATH . 'Common/Conf/home_view_config.php', $home_view) &&				   
				file_put_contents(APP_PATH . 'Common/Conf/user_view_config.php', $user_view) &&
				file_put_contents(APP_PATH . 'Common/Conf/up_drive_config.php', $up_drive) &&
				file_put_contents(APP_PATH . 'Common/Conf/email_config.php', $email)
				){
                show_msg('配置文件写入成功');
            } else {
...
function register_administrator($db, $prefix, $admin, $auth){
    ...
    $sql = "INSERT INTO `[PREFIX]member` VALUES ".
           "('1', '[NAME]','0', '0', '0', '0', '0', '0', '0', '0000-00-00', '0','NULL', '0','0', '1', '0', '[TIME]', '0', '[TIME]', '1','NULL');";
    $sql = str_replace(
