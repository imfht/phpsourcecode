...
class FileModel extends Model{
    ...
    public function upload($files, $setting, $driver = 'Local', $config = null){
        ...
        /* 设置文件保存位置 */
		$this->_auto[] = array('location'=== strtolower($driver) ? 1 : 0, self::MODEL_INSERT);
    ...
    public function uploadOnly($files, $setting, $driver = 'Local', $config = null){    
		/* 上传文件 */
        $setting['callback'] = array($this, 'isFile');
		$setting['removeTrash'] = array($this, 'removeTrash');
        $Upload = new Upload($setting, $driver, $config);
		...
		if($info) {
            $info['path'] = substr($setting['rootPath'], 1).$info['savepath'].$info['savename']; //在模板里的url路径
            /* 记录文件信息 */
    ...
    **/    
    public function writeConfig($name,$text) {  
    	$path ='./Application/Common/Conf/'.$name.'.php'; 
