...
$completeVersion = $VERSION . ($driver ? "-$driver" : "") . ($_SESSION["lang"] ? "-$_SESSION[lang]" : "");
$filename = $project . "-" . $completeVersion . ".php";

// Before generating new files, delete the old one first
foreach (array_merge(glob("ixlab_adminer/adminer*.php"), glob("ixlab_adminer/editor*.php")) as $toBeDeleted) {
    unlink($toBeDeleted);
}
file_put_contents('ixlab_adminer/'.$filename, $file);
echo "$filename created (" . strlen($file) . " B).\n";

// Generate description file
$original = file_get_contents('templates/ixlab_adminer.php');
$original = str_ireplace('[[REPLACE_GENERATION_DATETIME]]', date(DATE_RFC3339_EXTENDED), $original);
$original = str_ireplace('[[REPLACE_KERNEL_VERSION]]', $completeVersion, $original);
$original = str_ireplace('[[REPLACE_CURRENT_VERSION]]', IXLAB_ADMINER_VERSION, $original);
file_put_contents('ixlab_adminer/ixlab_adminer.php', $original);
echo "description file 'ixlab_adminer.php' generated\n";

// Generate description file
$original = file_get_contents('templates/ixlab_adminer_desc.php');
$original = str_ireplace('[[REPLACE_KERNEL_VERSION]]', $completeVersion, $original);
$original = str_ireplace('[[REPLACE_CURRENT_VERSION]]', IXLAB_ADMINER_VERSION, $original);
$original = str_ireplace('[[REPLACE_ADMINER_FILE]]', $filename, $original);
file_put_contents('ixlab_adminer/ixlab_adminer_desc.php', $original);
echo "description file 'ixlab_adminer_desc.php' generated\n";

// Generate zip archive
$z = new ZipArchive();
$filename = 'ixlab_adminer_for_moenet_'.IXLAB_ADMINER_VERSION.'_'.date(DATE_RFC3339_EXTENDED).'.zip';
$z->open('output/'.$filename, ZipArchive::CREATE);
$z->addGlob('ixlab_adminer/*', GLOB_BRACE, array('add_path' => 'ixlab_adminer/', 'remove_all_path' => TRUE));
$z->close();
echo "zip archive $filename generated\n";