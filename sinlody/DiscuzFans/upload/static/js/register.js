...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = '如不需要更改密码，此处请留空',smscodesendtime = 60,smscodesendtimefuc;
...
function sendsms(smsid,smscodeid){
	...
	if(smscode.match(/<|"/ig)) {
		errormessage(smscodeid, '验证码包含敏感字符');
		return;
	}
	var x = new Ajax();
...
function disabledsendsms(time, changestr){
	...
	if(changestr){
		$('smscodesend').value = smscodesendtime + '秒后重新发送';
