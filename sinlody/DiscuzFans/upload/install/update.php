...
$first_to_f_1_0 = !C::t('common_setting')->skey_exists('ec_wxpay_appid');
if($_POST['delsubmit']) {
...
if($_GET['step'] == 'start') {
	...
	if(version_compare($version, '1.6.0') <= 0) {
		show_msg('请先升级 UCenter 到 1.7.0 及以上版本。<br>如果使用为Discuz! F自带UCenter，请先运行 <a href="../uc_server/upgrade/upgrade4.php" target="_blank">UC升级程序</a> 进行升级！');
	} else {
	...
	if($i>=$count_i) {
	    if(DB::query("SHOW CREATE TABLE ".DB::table('common_member_archive'), 'SILENT')) {
	...
	if(empty($_GET['op']) || $_GET['op'] == 'realname') {
		...
		if(!isset($settings['sendregisterverify']) && isset($settings['sendregisterurl']) && $settings['sendregisterurl']){
		    $newsettings['sendregisterurl'] = 0;
		    $newsettings['sendregisterverify'] = 1;
		}

		if(!isset($settings['welcomemsg'])){
		    $newsettings['welcomemsg'] = serialize(array('1'));
		}else{
		    $welcomemsg = dunserialize($settings['welcomemsg']);
		    if(empty($welcomemsg)){
		        if($settings['welcomemsg'] == 1){
		            $newsettings['welcomemsg'] = serialize(array('1'));
		        }elseif($settings['welcomemsg'] == 2){
		            $newsettings['welcomemsg'] = serialize(array('2'));
		        }elseif($settings['welcomemsg'] == 3){
		            $newsettings['welcomemsg'] = serialize(array('1','2'));
		        }else{
		            $newsettings['welcomemsg'] = '';
		        }
		    }
		}

		if(!isset($settings['sms'])){
		    $newsettings['sms'] = serialize(array(
		        'type'=>'1',
		        'auth_username'=>'',
		        'auth_passwd'=>'',
		        'silent'=>'1',
		...
		if(!isset($settings['smscodetype'])){
		    $newsettings['smscodetype'] = 2;
		}
		if(!isset($settings['smscodelength'])){
		    $newsettings['smscodelength'] = 6;
		}
