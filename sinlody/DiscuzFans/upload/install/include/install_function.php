...
function env_check(&$env_items) {
	...
	foreach($env_items as $key => $item) {
		if($key == 'php') {
			...
			$env_items[$key]['current'] = isset($lang['opcache_'.$opcache]) ? $lang['opcache_'.$opcache] : 'unknow';
		} elseif($key == 'curl') {
			$v = curl_version();
...
function check_adminuser($username, $password, $email) {
	$result = check_adminuser_new($username, $password, $email, null);
	unset($result['sms']);
	return $result;
}

function check_adminuser_new($username, $password, $email, $sms) {

	include ROOT_PATH.CONFIG_UC;
	include ROOT_PATH.'./uc_client/client.php';

	$error = '';
	$ucresult = uc_user_login_new($username, $password);
	list($tmp['uid'], $tmp['username'], $tmp['password'], $tmp['email'], $tmp['sms']) = uc_addslashes($ucresult);
	$ucresult = $tmp;
	if($ucresult['uid'] <= 0) {
		$uid = uc_user_register_new($username, $password, $email, $sms);
		if($uid == -1 || $uid == -2) {
