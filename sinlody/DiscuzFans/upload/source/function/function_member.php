	...
	if(!$uid_login) {
		if($isuid == 4) {
			if(!strcmp(dintval($username), $username) && getglobal('setting/uidlogin')) {
				$return['ucresult'] = uc_user_login_new($username, $password, 1, 1, $questionid, $answer, $ip);
			} elseif(isemail($username)) {
				$return['ucresult'] = uc_user_login_new($username, $password, 2, 1, $questionid, $answer, $ip);
				$_GET['loginfield'] = 'email';
			} elseif (issms($username)){
			    $return['ucresult'] = uc_user_login_new($username, $password, 3, 1, $questionid, $answer, $ip);
			    $_GET['loginfield'] = 'sms';
			}
			if($return['ucresult'][0] <= 0 && $return['ucresult'][0] != -3) {
				$return['ucresult'] = uc_user_login_new(addslashes($username), $password, 0, 1, $questionid, $answer, $ip);
			}
		} else {
			$return['ucresult'] = uc_user_login_new(addslashes($username), $password, $isuid, 1, $questionid, $answer, $ip);
		}
		$useloginName = false;
	} else {
		$return['ucresult'] = uc_user_login_new($uid_login, $password, 1, 1, $questionid, $answer, $ip);
		$useloginName = true;
	}
	$tmp = array();
	$duplicate = '';
