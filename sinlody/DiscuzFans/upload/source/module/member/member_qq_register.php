...
if (empty($_POST)) {
	...
	if (empty($conopenid)) {
		showmessage('qq:connect_login_first', $referer);
	}

	$_G['qc']['connect_is_feed'] = true;

	$_G['qc']['connect_app_id'] = $_G['cache']['plugin']['qq']['appid'];
	$_G['qc']['connect_openid'] = $conopenid;

	$_G['qc']['connect_qq_figureurl'] = $_G['cookie']['connect_qq_figureurl'];
	...
	if (!empty($_G['setting']['checkuinlimit']) && !empty($_GET['uin'])) {
		if ($_G['qc']['uinlimit']) {
			showmessage('qq:connect_register_uinlimit', '', array('limit' => $this->setting['connect']['register_uinlimit']));
		}
		if (!$_G['setting']['regconnect']) {
			showmessage('qq:connect_register_closed');
		}
	}
	...
	if (!$access_token || !$conopenid) {
		showmessage('qq:connect_get_request_token_failed');
	}


	if (C::t('#qq#qq_member')->fetch_fields_by_openid($conopenid, 'uid')) {
		showmessage('qq:connect_register_bind_uin_already');
	}
