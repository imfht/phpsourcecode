...
if($_GET['action'] == 'checkusername') {
...
} elseif($_GET['action'] == 'checksms') {

	require_once libfile('function/member');
	checksms($_GET['sms']);

} elseif($_GET['action'] == 'sendsmscode') {

    require_once libfile('function/member');
    checksms($_GET['sms']);

    $code = make_smscode($_GET['sms']);
    ...
    $message = array(
        ...
        'username'=>lang('member/template', 'ning'),
        'code'=>$code,
    );
    require_once libfile('function/sms');
    if(!sendsms($_GET['sms'], $message, $template)) {
        runlog('sendsms', "$_GET[sms] sendsms failed.");
        showmessage('register_sms_send_fail', '', array(), array('handle' => false));
    }
    showmessage('register_sms_send_success', '', array(), array('handle' => false));
