...
EOT;
	...
	if(!submitcheck('ordersubmit')) {
		...
		if(submitcheck('searchsubmit', 1)) {
			...
			$ordercount = C::t('forum_order')->count_by_search_new(null, $_GET['orderstatus'], $_GET['orderid'], null, null, ($_GET['users'] ? explode(',', str_replace(' ', '', $_GET['users'])) : null), $_GET['buyer'], $_GET['admin'], strtotime($_GET['sstarttime']), strtotime($_GET['sendtime']), strtotime($_GET['cstarttime']), strtotime($_GET['cendtime']));
	...
	if(!submitcheck('ordersubmit')) {
		...
		$ordercount = C::t('forum_order')->count_by_search_new(0, $_GET['orderstatus'], $_GET['orderid'], $_GET['email'], $_GET['sms'], null, $buyer);
