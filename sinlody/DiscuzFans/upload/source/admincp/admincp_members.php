...
EOF;
	...
	if(!submitcheck('addsubmit')) {
		...
		$uid = uc_user_register_new(addslashes($newusername), $newpassword, $newemail, $newsms);
		if($uid <= 0) {
			if($uid == -1) {
				...
				if(empty($_GET['confirmed'])) {
					...
					list($uid,, $newemail, $newsms) = uc_get_user_new(addslashes($newusername));
				}
			} elseif($uid == -4) {
		...
		C::t('common_member')->insert_new($uid, $newusername, $password, $newemail, $newsms, 'Manual Acting', $_GET['newgroupid'], $init_arr, $newadminid);
		if($_GET['emailnotify']) {
...
EOF;
	...
	if(!submitcheck('editsubmit')) {
		...
		$ucresult = uc_user_edit_new(addslashes($member['username']), $_GET['passwordnew'], $_GET['passwordnew'], addslashes(strtolower(trim($_GET['emailnew']))), addslashes(strtolower(trim($_GET['smsnew']))), 1, $questionid);
		if($ucresult < 0) {
			if($ucresult == -4) {
				cpmsg('members_email_illegal', '', 'error');
			} elseif($ucresult == -5) {
