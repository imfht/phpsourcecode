...
$submenu = array(
    array(array('menu' => $catid ? $catlist[$catid]['catname'] : ('plugins_list'), 'submenu' => $catmenu), !$operation),
    array('plugins_cat', 'plugins&operation=cat', ($operation == 'cat' ? true : false)),
    $isplugindeveloper ? array('plugins_add', 'plugins&operation=add', ($operation == 'add' ? true : false)) : array(),
...
if (!$operation) {
	...
	if (!submitcheck('submit')) {
		...
		loadcache('plugin');
		shownav('plugin');
		showsubmenu('nav_plugins', $submenu, '<a href="' . ADMINSCRIPT . '?action=plugins&operation=upgradecheck&formhash='.FORMHASH.'" class="bold" style="float:right;padding-right:40px;">' . $lang['plugins_validator'] . '</a>');
		showformheader('plugins' . ($catid ? '&catid=' . $catid : ''));
		$outputsubmit = false;
		$plugins = $addonids = $pluginlist = array();
		$plugins = C::t('common_plugin')->fetch_all_data(false, $catid);
		...
		foreach ($plugins as $plugin) {
			...
			$pluginlist[$order][$plugin['pluginid']] = listrow($plugin);
		}
...
EOF;
			...
			echo '<center>' . cplang('plugins_empty') . '</center>';
		}
		showtableheader('', 'psetting');
		if ($outputsubmit) {
			showsubmit('submit', 'submit', '<select name="catidnew">' . get_selectcat($plugin['catid']) . '</select>', '<a href="' . ADMINSCRIPT . '?action=cloudaddons">' . cplang('cloudaddons_plugin_link') . '</a>', '');
		} else {
		...
		if ($exists) {
			...
			foreach (C::t('common_plugin')->fetch_all_by_identifier(array_keys($exists)) as $plugin) {
				$plugins[] = '<b>' . $plugin['name'] . '</b>:' .
					...
					'&nbsp;<a href="' . cloudaddons_pluginlogo_url($plugin['identifier']) . '" onerror="this.src=\'static/image/admincp/plugin_logo.png\';this.onerror=null" target="_blank">' . cplang('plugins_conflict_info') . '</a>' .
	...
	if ($operation == 'enable') {
		if (!$conflictplugins) {
			cpmsg('plugins_enable_succeed', 'action=plugins' . (!empty($_GET['system']) ? '&system=1' : '').($catid ? '&catid='.$catid : ''), 'succeed');
		} else {
			cpmsg('plugins_conflict', 'action=plugins' . (!empty($_GET['system']) ? '&system=1' : '').($catid ? '&catid='.$catid : ''), 'succeed', array('plugins' => $conflictplugins));
		}
	} else {
		cpmsg('plugins_disable_succeed', 'action=plugins' . (!empty($_GET['system']) ? '&system=1' : '').($catid ? '&catid='.$catid : ''), 'succeed');
	}
	...
	unset($plugin['catid']);
	$tempcat = dunserialize($plugin['tempcat']);
	unset($plugin['tempcat']);

	$pluginarray = array();
	$pluginarray['plugin'] = $plugin;
	$pluginarray['version'] = strip_tags($_G['setting']['version']);
	if ($tempcat['catcode'] && $tempcat['catname']) {
		$pluginarray['catinfo']['catcode'] = $tempcat['catcode'];
		$pluginarray['catinfo']['catname'] = $tempcat['catname'];
		$pluginarray['catinfo']['catstatus'] = $tempcat['catstatus'];
	}
	...
	if ($modules['extra']['langexists'] && file_exists($file = DISCUZ_ROOT . './data/plugindata/' . $pluginarray['plugin']['identifier'] . '.lang.php')) {
		include $file;global $scriptlang,$templatelang,$installlang,$systemlang;
	...
	if (!$_GET['confirmed']) {
		...
		if ($upgrade) {
			...
			cpmsg('plugins_config_upgrade_confirm', 'action=plugins&operation=upgrade&pluginid=' . $pluginid . '&confirm=yes'.($catid ? '&catid=' . $catid : ''), 'form', array('pluginname' => $plugin['name'], 'version' => $plugin['version'], 'toversion' => $newver));
		} elseif ($upgradestr) {
	...
	if (!submitcheck('addsubmit')) {
		...
		showsetting('plugins_edit_catcode', 'catcodenew', '', 'text');
		showsetting('plugins_edit_catname', 'catnamenew', '', 'text');
		showsetting('plugins_edit_catstatus', 'catstatusnew', 1, 'radio');
		...
		$data = array(
		    ...
		    'tempcat' => serialize(array('catcode' => trim($_GET['catcodenew']), 'catname' => dhtmlspecialchars(trim($_GET['catnamenew'])), 'catstatus' => intval($_GET['catstatusnew']))),
		);
	...
	if (!submitcheck('editsubmit')) {
		...
		showsetting('plugins_edit_catcode', 'catcodenew', $plugin['tempcat']['catcode'], 'text');
		showsetting('plugins_edit_catname', 'catnamenew', $plugin['tempcat']['catname'], 'text');
		showsetting('plugins_edit_catstatus', 'catstatusnew', $plugin['tempcat']['catstatus'], 'radio');
	...
	cpmsg('plugins_delete_succeed', "action=plugins" . ($catid ? '&catid=' . $catid : ''), 'succeed');
	...
	if (!submitcheck('catsubmit')) {
		...
		<script type="text/JavaScript">
			...
			[1, '<input type="text" class="txt" name="newcatname[]" value="">', 'td26'],
			[1, '<input type="text" class="txt" name="newcatcode[]" value="">', ''],
...
EOF;
		...
		foreach ($allcats as $list) {
			$clist .= showtablerow('', array('', 'class="td30"', 'class="td26"'), array(
		...
		showsubtitle(array('', cplang('setting_profile_group_displayorder'), cplang('plugins_cat_catname'), cplang('plugins_cat_catcode'), cplang('plugins_cat_status')));
		echo $clist;
		...
		updatecache(array('plugin', 'setting', 'styles'));
		updatemenu('plugin');
		cpmsg((($ig || $done) ? cplang('plugins_cat_success_add', array('done' => $done, 'ig' => $ig)) : cplang('plugins_cat_success')), "action=plugins&operation=cat", 'succeed');
	}
}
...
function listrow($plugin) {
	global $_G, $lang, $isplugindeveloper, $submenuitem, $hookexists, $updateinfo, $hookorder, $catid, $cats;
	return '
<li onmouseover="getMemo(this, \'' . $plugin['identifier'] . '\')">
<div id="base_' . $plugin['identifier'] . '">
<div class="x1 cl">
<a ' . ($submenuitem ? 'href="' . ADMINSCRIPT . '?action=plugins&operation=config&do=' . $plugin['pluginid'] . '" ' : '') . 'class="avt">
<img src="' . cloudaddons_pluginlogo_url($plugin['identifier']) . '" onerror="this.src=\'static/image/admincp/plugin_logo.png\';this.onerror=null" />
</a>
<p class="cl"><span class="y">' .
	(!$plugin['new'] ?
		(!$plugin['available'] ?
			...
			'<a href="' . ADMINSCRIPT . '?action=plugins&operation=import&dir=' . $plugin['identifier'] . '" class="bold">' . $lang['plugins_install_switch'] . '</a>'
	) .
	'</span>
	<a href="' . ADMINSCRIPT . '?action=cloudaddons&id=' . $plugin['identifier'] . '.plugin" target="_blank" >' . dhtmlspecialchars($plugin['name']) . '</a></p>
	<p class="cl">'.'<span class="xg1">' . $lang['author'] . ':</span> <a href="' . ADMINSCRIPT . '?action=cloudaddons&copyright=' . strip_tags($plugin['copyright']) . '" target="_blank" >' . cutstr(strip_tags($plugin['copyright']), 20) .'</a>'.'</p>'.
'</div>'.
($updateinfo ? '<b>' . $updateinfo . '</b>' : '') .
'<div class="x2 cl" id="memo_' . $plugin['identifier'] . '">' .
	(!$plugin['new'] ?
		...
		($hookexists !== FALSE && $plugin['available'] ? "<input title=\"$lang[display_order]\" class=\"txt num y\" type=\"text\" id=\"displayorder_$plugin[pluginid]\" name=\"displayordernew[$plugin[pluginid]][$hookexists]\" value=\"$hookorder\" />" : '').
		($submenuitem ? '<a href="' . ADMINSCRIPT . '?action=plugins&operation=config&do=' . $plugin['pluginid'] . '">' . $lang['config'] . '</a>&nbsp;' : '') .
		("<a href=\"" . ADMINSCRIPT . "?action=plugins&operation=upgrade&pluginid=$plugin[pluginid]" . ($catid ? '&catid=' . $catid : '') . "\">$lang[plugins_config_upgrade]</a>&nbsp;" .
