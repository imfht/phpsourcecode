 ...
 *
 *      $Id: table_common_smscron.php 27806 2012-02-15 03:20:46Z svn_project_zhangjie $
 */
...
class table_common_smscron extends discuz_table
{
	public function __construct() {

		$this->_table = 'common_smscron';
		$this->_pk    = 'cid';
	...
	public function delete_by_touid($touids) {
		...
		return DB::query('DELETE FROM mc, mq USING %t AS mc, %t AS mq WHERE mc.'.DB::field('touid', $touids).' AND mc.cid=mq.cid',
				array($this->_table, 'common_smsqueue'), false, true);
	}

	public function fetch_all_by_sms($sms, $start, $limit) {
		return DB::fetch_all('SELECT * FROM %t WHERE sms=%s '.DB::limit($start, $limit), array($this->_table, $sms));
	}
