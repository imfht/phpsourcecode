...
$_G['connect']['callback_url'] = $_G['siteurl'].'plugin.php?id=qqconnect:connect&op=callback';
parse_str(substr($_GET['referer'], 1), $refererarray);
$_G['connect']['siteurl_m'] = $_G['siteurl']."m/";
$referer = $_G['connect']['siteurl_m'].$_GET['referer'];
...
if($op == 'init') {
	...
	if($connect_member) {
		...
		$loginUrl = $_G['connect']['siteurl_m']."?".http_build_query($get);
		...
		header('location: '.str_replace('&loginErr=1001', '', $referer));
		exit;
	} else {
