...
class plugin_qqconnect_base {
	...
	function common_base() {
		...
		$this->callback_domain = $_G['setting']['connect']['callback_domain'] ? "http://" . rtrim(str_replace("http://", "", $_G['setting']['connect']['callback_domain']), "/") . "/" : $_G['siteurl'];
		...
		if (!isset($_G['connect'])) {
			...
			$_G['connect']['login_url'] = $this->callback_domain . 'connect.php?mod=login&op=init&referer=' . urlencode($_G['connect']['referer'] ? $_G['connect']['referer'] : 'index.php');
			$_G['connect']['callback_url'] = $this->callback_domain . 'connect.php?mod=login&op=callback';
