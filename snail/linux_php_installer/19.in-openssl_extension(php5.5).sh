...
make && make install
if [ -f "/server/php5.5/lib/php/extensions/no-debug-non-zts-20121212/openssl.so" ];then
    echo "[openssl]">/server/php5.5/etc/conf.d/openssl.ini
    echo "extension=openssl.so">>/server/php5.5/etc/conf.d/openssl.ini
