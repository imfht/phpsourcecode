...
<div class="row">
	<div class="col-lg-12">
		<form class="form-inline" id="view-form" action="{$public}news/index/index" method="post">
			{$ecms_hashur['form']??''}
		...
		<form class="form-inline" id="editor_form" action="{$public}news/index/editor">
			{$ecms_hashur['form']??''}
			...
			<table class="table table-bordered table-hover text-center">
				...
				{volist name="list" id="v" key="k" empty="<tr><td colspan=7>暂无数据</td></tr>"}
			    	<tr>
			            <td><input name="ids[]" type="checkbox" id="ids" value="{$k}" {$aid==$v.aid?'':"disabled"}/></td>
						<td>{$k}</td>
			            <td>
			            	<input type="hidden" name="id[{$k}]" value="{$v.id}" >
			            	<a href="{$public}../view.php?type=news&id={$v.id}" target="_blank">{$v['id']}</a>
			            </td>
			            ...
			            <td>
			            	<a href="{php} if($aid==$v['aid']){ {/php}
			            			{$public}news/index/up2wx?id={$v.id}{$ecms_hashur.href??''}
			            		{php}}else{ {/php}
			            			javascript:void(0);
			            		{php}}{/php}" 
			            		{$aid==$v['aid']?'':"disabled"} class="btn btn-primary">{$v['media_id']?'更新':'上传'}</a>
			            	<a href="{php} if($aid==$v['aid']){ {/php}
			            			{$public}news/index/toEditor/id/{$v.id}?{$ecms_hashur.href??''}
			            		{php}}else{ {/php}
			            			javascript:void(0);
			            		{php}}{/php}" 
				...
				{/volist}
			        <tr>
			            ...
			            <td colspan="6">
			            	...
			            	<a href="{$public}news/index/addView?{$ecms_hashur['href']??''}" class="btn btn-success">新增图文</a>
			            </td>
			        </tr>
				<?php if(isset($page)){?>
					<tr align="left">
...
function tPage(obj,form){
	...
	$('#tpage').val(page);
	var u="<?php echo $public.'news/index/index?'.$ecms_hashur['href'];?>";
	form = form || 'list_form';
	$('#'+form).submit();
	//Ajax(u,a);
}
