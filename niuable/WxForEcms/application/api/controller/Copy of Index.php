...
use app\msgReply\model\WxMsgreply; //注意路径中的大小写
...
// 				] 
	...
	private function transReplyContent($r = NULL) {
		if (empty ( $r ) || $r ['msg_type'] == 'text') {
			...
			$res = $file->getUp2WxShortFile( $r ['img'] );
			if ($res ['errCode']) {
				$r ['msg_type'] = 'text';
				$r ['text'] = '';
			} else {
...
// 		$postStr = $GLOBALS ["HTTP_RAW_POST_DATA"];
	...
	private function responseMsg() {
		if (empty ( $this->my_output ) || $this->my_input === NULL) {
			...
			if ($this->my_output ['msg_type'] == 'text') {
				...
				if ($this->my_output ['text']) {
					$msg = $this->getTextReplyMsg ();
				} else {
					$msg = '';
				}
			} elseif ($this->my_output ['msg_type'] == 'img') {
				$msg = $this->getImgReplyMsg ();
			} elseif ($this->my_output ['msg_type'] == 'voice') {
			} elseif ($this->my_output ['msg_type'] == 'video') {
			} elseif ($this->my_output ['msg_type'] == 'news') {
				$msg = $this->getNewsReplyMsg ();
			} elseif ($this->my_output ['msg_type'] == 6) {
			} else {
	 ...
	 */
	private function getTextReplyMsg() {
		$time = time ();
	 ...
	 * @return string 可回复的xml格式字符串；未加密
	 */
	private function getImgReplyMsg() {
		return ReplyMsg::getImageMsg($this->my_input ['FromUserName'], $this->my_input ['ToUserName'], $this->my_output ['media_id'], time ());
	}
	/**
	 * getNewsReplyMsg
	 * 拼装图文类型的回复
