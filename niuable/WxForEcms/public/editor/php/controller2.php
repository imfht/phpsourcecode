 ...
 *
 */
if($public_r[fpath]==0)$public_r[fpath]='/'.$classpath['filepath'];
elseif($public_r[fpath]==1) $public_r[fpath]='/d/file/p/';
elseif($public_r[fpath]==2) $public_r[fpath]='/d/file/';
	...
	if($action=="catchimage") //远程保存写数据库
	{
		for($i=0;$i<count($file_r['list']);$i++)
		{
			if($file_r['list'][$i]['state']=="SUCCESS")
			{
				...
				$r=$empire->fetch1("select `aid` from {$dbtbpre}wx_index where `default`=1;");
 				$res=$empire->query("insert {$dbtbpre}wx_file 
 					(`aid`,title`,`name`,`isOk`,`path`,`size`,`ext`,`createTime`,`type`,`description`)
 					values('$r[aid]','$original','$title','1','$filepath','$filesize','$file_r[type]','$time','$type','$original');
 					");
...
function action_list($classid,$username){
	...
	// 统计总数
	$r=$empire->fetch1("select `aid` from {$dbtbpre}wx_index where `default`=1;");
	$total=$empire->gettotal("select count(*) as total from {$dbtbpre}wx_file where `aid`='$r[aid]'".$where);
	$sql=$empire->query("select * from {$dbtbpre}wx_file where `aid`='$r[aid]'".$where." order by id DESC limit ".$limit);
	$bqno=0;
	while($r=$empire->fetch($sql))
	{
		//$classpath = ReturnFileSavePath($r['classid']);
		$list[$bqno]['url'] = $r[path].'/'.$r['name'];
		$list[$bqno]['mtime'] =$r['createTime'];
		$bqno++;
	}
	/* 返回数据 */
