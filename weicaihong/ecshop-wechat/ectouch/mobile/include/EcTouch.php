...
try {
    ...
    if ($method->isPublic() && ! $method->isStatic()) {
        ...
        // 微彩虹云服务 接口 weicaihong.com
        require_once(ROOT_PATH.'../wechat/wch_lib.php');

        if(!empty($wxid) and strlen($wxid) == 28)
        {
            $wch_users = init_users();
            $query_sql = "SELECT * FROM  ".$wch_users->table('users')." WHERE `wxid` = '$wxid'";
            $wch_M = M();
            ...
            if(!empty($wch_arr['user_name']) )
            {
                ...
                if($_SESSION['user_name'] == $wch_user_name)
                {
