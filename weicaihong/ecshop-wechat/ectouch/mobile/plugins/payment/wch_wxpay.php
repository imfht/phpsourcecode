...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['config']  = array(
        array(
            'name' => 'wch_wxpay_config',
            'type' => 'text',
            'value' => '请在微彩虹云服务中配置微信支付->http://www.weicaihong.com/'
        ),
    );
...
class wch_wxpay
{
    ...
    function get_code($order, $payment)
    {
        ...
        $query_sql = "SELECT * FROM  ".$wch_users->table('users').'WHERE `user_id`='.$_SESSION['user_id'].';';
        $wch_M = M();
        ...
        $js = '<script language="javascript">
            function callpay(){WeixinJSBridge.invoke("getBrandWCPayRequest",'.$wx_pay_js.',function(res){if(res.err_msg == "get_brand_wcpay_request:ok"){location.href="respond.php?code=wch_wxpay&status=1"}else{alert(res.err_msg);location.href="respond.php?code=wch_wxpay&status=0"}});}
            callpay();
            </script>';

        $button = '<div><input id="wch_button" class="btn btn-info ect-btn-info ect-colorf ect-bg" type="button" onclick="callpay()" value="'.L('wch_wxpay_button').'"></div>'.$js;
