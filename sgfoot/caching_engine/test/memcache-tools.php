...
define('ADMIN_USERNAME', '请设置您用户名');    // Admin Username
define('ADMIN_PASSWORD', '请设置您的密码');    // Admin Password
...
function memcache_gc_info($host, $port)
{
    global $PHP_SELF;
    if (stripos($PHP_SELF, '?') === false) {
        $PHP_SELF .= '?';
    } elseif(stripos($PHP_SELF, '&') !== false) {
        $PHP_SELF .= '&';
    }
    echo "<a class='btn' href='{$PHP_SELF}op=8&host={$host}&port={$port}'>{$host}:{$port}垃圾回收</a><br/>";
    $mem = new Memcache();
    $mem->connect($host, $port);
    $sid   = "{$host}:{$port}";
    $items = $mem->getExtendedStats('items');  //获取memcached状态
    $at    = time();
    if (isset($items[$sid]['items']) && count($items[$sid]['items']) > 0) {
        foreach ($items[$sid]['items'] as $slab_id => $slab)  //获取指定server id 的 所有Slab
        {
            $item = $mem->getExtendedStats('cachedump', $slab_id, 0);        //遍历所有Slab
            foreach ($item[$sid] as $key => $info)  //获取Slab中缓存对象信息
            {
                echo $key, ' => Exp:', $info[1] - $at, 's<br/>'; //查看缓存对象的剩余过期时间
            }
        }
    }
    $mem->close();
...
function memcache_gc($host, $port)
{
    global $PHP_SELF;
    if (stripos($PHP_SELF, '?') === false) {
        $PHP_SELF .= '?';
    ...
    echo "<a class='btn' href='{$PHP_SELF}op=7&host={$host}&port={$port}'>{$host}:{$port}列出全部Key</a><br/>";
    $mem = new Memcache();
    $mem->connect($host, $port);
    $sid   = "{$host}:{$port}";
    $items = $mem->getExtendedStats('items');  //获取memcached状态
...
EOB;
        foreach ($MEMCACHE_SERVERS as $server) {
            ...
            echo "<tr class=tr-1><td class=td-0><a target='_blank' href='{$PHP_SELF}op=7&host={$host}&port={$port}'>列出所有的Keys</a></td>";
            echo "<td><a href='{$PHP_SELF}op=8&host={$host}&port={$port}'>垃圾回收gc</a></td></tr>";
            echo '</tbody></table>';
        }
        echo <<<EOB
