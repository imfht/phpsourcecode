...
function check_env() {
    $items = [
        ...
        'php' => ['PHP版本', '7.0.0', '5.3+', PHP_VERSION, 'success'],
...
function create_tables($db, $prefix = '') {
    ...
    $sql = explode(";\n", str_replace("\r", "\n", $sql));
    //替换表前缀
    $orginal = Config::get('orginal_table_prefix');
...
function register_administrator($db, $prefix, $admin, $auth) {
    ...
    $sql = str_replace(
            ['[PREFIX]', '[NAME]', '[PASS]', '[EMAIL]', '[TIME]', '[IP]'], [$prefix, $admin['username'], $password, $admin['email'], Request::instance()->time(), Request::instance()->ip(1)], $sql);
    //执行sql
    $db->execute($sql);
