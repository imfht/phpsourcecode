...
class verify {
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
        $secode = \think\Session::get($key);
        if (empty($code) || empty($secode)) {
            return false;
        }
        // session 过期
        if (time() - $secode['verify_time'] > $this->expire) {
            \think\Session::delete($key, null);
            return false;
        }

        if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && \think\Session::delete($key, null);
            return true;
        }
    ...
    public function entry($id = '') {
        ...
        \think\Session::set($key . $id, $secode);
        ...
        return \think\Response::create($content, 'html', 200, ['Content-Length' => strlen($content)])->contentType('image/png');
    }
