 ...
 * @return string
 */
function U($method = '', $args = [], $ignores = [], $type = null)
{
    ...
    $type = $type ?: PT_Base::getInstance()->input->get('f', ['html', 'json', 'xml'], 'html');
    //忽视args中的部分参数
    if (!empty($ignores)) {
    ...
    if (!empty($rules[$method]) && empty($args['_force'])) {
        ...
        if ($power) {
            $module_domain = PT_Base::getInstance()->config->get('module_domain', '');
            //优化url模式
            if (!empty($module_domain[$param['m']])) {
                // 模块域名
                $scheme = parse_url(PT_Base::getInstance()->config->get('siteurl'), PHP_URL_SCHEME);
...
function get_auto_map()
{
    ...
    foreach ($dirs as $dir) {
        ...
        while (($dirname = readdir($handle)) !== false) {
            if (in_array($dirname, ['model', 'block', 'library'])) {
 ...
 * @return mixed
 */
function I($name, $filter = 'int', $default = null, $input = [])
{
    static $handler = null;
