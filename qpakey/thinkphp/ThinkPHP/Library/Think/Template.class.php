...
class  Template {
    ...
    public function parseTagLib($tagLib,&$content,$hide=false) {
        ...
        if(strpos($tagLib,'_')){
            // 支持指定标签库的命名空间
            $className  =   $tagLib;
            $tagLib     =   substr($tagLib,strrpos($tagLib,'_')+1);
        }else{
            $className  =   'Think_Template_TagLib_'.ucwords($tagLib);            
        }
...
class Think_Template extends Template{}
