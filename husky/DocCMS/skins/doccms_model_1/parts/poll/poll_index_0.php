...
if( !empty( $tag['data.results'] ) )
{
	foreach($tag['data.results'] as $k =>$data)
	{
      ...
      <p><span style=" font-weight:bold;"><?php echo $data['title']; ?></span> （<?php echo date('Y年m月d日',strtotime($data['dtTime'])); ?>开始投票）
       <?php
			if( !empty( $data['children'] ) )
			{
              ...
              <p><input type="<?php echo $data['choice']=='a'?'radio':'checkbox'?>" name="choice<?php echo $data['choice']=='a'?'':'[]'?>" value="<?php echo $children_data['id']; ?>" <?php echo $children_data['isdefault']=='a'?'checked':'';?>>
