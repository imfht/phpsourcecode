 ...
 * @copyright DOCCMS
 */
function shl_article(  $channelId, $fromcount=0, $n=0, $ordering='pageId',
$opts=array( 'hastag'=>true,'hasstyle'=>array('tbxstyle'=>'article','style'=>0),'fun'=>'doc_article','isintercepted'=>array('strcount'=>0,'strcount1'=>0,'strcount2'=>0,'isellipsis'=>true) ) )
{
	$opts['fun'] = isset($opts['fun']) ? $opts['fun'] : 'doc_article';
	/********************参数验证 start*******************/
	...
	$sql = "SELECT * FROM `".TB_PREFIX."article` WHERE channelId IN (".$channelId.") ORDER BY ".$ordering." ASC";
...
function doc_article($channelId=0,$n=0,$style=0,$strcount=0,$strcount1=0,$strcount2=0,$isellipsis=true,$hastag=false,$ordering='pageId',$fromcount=0)
{
	$opts=array( 
		'hastag' => $hastag,
		'hasstyle' => array( 'tbxstyle' => 'article', 'style' => $style ),
		'fun'=>'doc_article',
		'isintercepted'=>array('strcount'=>$strcount,'strcount1'=>$strcount1,'strcount2'=>$strcount2,'isellipsis'=>$isellipsis)
	);
	shl_article(  $channelId, $fromcount, $n, $ordering, $opts );
}