 ...
 * @copyright  DOCCMS
 */
function shl_download( $channelId, $fromcount=0, $n=0, $ordering='id',
$opts=array('hastag'=>true,'hasstyle'=>array('tbxstyle'=>'download','style'=>0),'fun'=>'doc_download','isintercepted'=>array('strcount'=>0,'strcount1'=>0,'strcount2'=>0,'isellipsis'=>true)))
{
	$opts['fun'] = isset($opts['fun']) ? $opts['fun'] : 'doc_download';
	/********************参数验证 start*******************/
	...
	$sql="SELECT * FROM `".TB_PREFIX."download` WHERE channelId IN(".$channelId.") ORDER BY ".$ordering." DESC";
	...
	if($results)
	{
		...
		{
			$tbxstyle = isset($opts['hasstyle']['tbxstyle']) ? $opts['hasstyle']['tbxstyle'] : 'download';
...
function doc_download($channelId=0,$n=0,$style=0,$strcount=0,$strcount1=0,$strcount2=0,$isellipsis=true,$hastag=false,$ordering='id',$fromcount=0)
{
	$opts=array(
		'hastag'=>$hastag,
		'hasstyle'=>array('tbxstyle'=>'download','style'=>$style),
		'fun'=>'doc_download',
		'isintercepted'=>array('strcount'=>$strcount,'strcount1'=>$strcount1,'strcount2'=>$strcount2,'isellipsis'=>$isellipsis)
	);
	shl_download($channelId, $fromcount, $n, $ordering, $opts);
}