...
window.ImgCollapsed = "data:image/gif;base64,R0lGODlhHAALAMQAAP////7++/z8/Pb29fb18PHx7e/w6/Hw6e3s5unp4+jm2ODg3t3a0dnVy9bQ xtLMv8zJurDC1L+9sMK4p32buDMzMwAAAP///wAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAACH5BAEH ABcALAAAAAAcAAsAAAVU4CWOZGmeV0StLBWhsEgBdA1QMUwJvMUTuNyJMihaBodFUFiiECxQKGMp qlSq14uVRCkUEJbEokHVZrdmrqLRsDgekDLzQoFIJni8nKlqrV5zgYIhADs=";
window.ImgExpanded = "data:image/gif;base64,R0lGODlhHAALAMQAAP////7++/z8/Pb29fb18PHx7e/w6/Hw6e3s5unp4+Dg3t3a0djY0dnVy9fT xNbQxtLMv8zJurDC1L+9sMK4p32buAAAAP///wAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAACH5BAEH ABcALAAAAAAcAAsAAAVL4CWOZGmel1StbCWhsFgBdA1UMVwJQd8TuNypMigWD4qgsFQhWJ7PhXI5 qhQKCERC0ZhSLxUFo+FwQCJeagUyobjd6aWqtXp979QQADs=";
...
function ProcessJson(divId, str) {
	SetTab();
	// window.IsCollapsible = $id("CollapsibleView").checked;
	...
	try {
		...
		var obj = eval("[" + json + "]");
		html = ProcessObject(obj[0], 0, false, false, false);
		$id(divId).innerHTML = "<PRE class='CodeContainer'>" + html + "</PRE>";
	} catch (e) {
		alert("JSON数据格式不正确:\n" + e.message);
		$id(divId).innerHTML = "";
	}
	...
	if (IsArray(obj)) {
		...
		if (obj == null) {
			...
			if (numProps == 0) {
				...
				// clpsHtml = window.IsCollapsible ? "<span><img
				// src=\""+window.ImgExpanded+"\"
				// onClick=\"ExpImgClicked(this)\" /></span><span
				// class='collapsible'>" : "";

				clpsHtml = window.IsCollapsible ? "<span><img src=\""
						+ window.ImgCollapsed
						+ "\" onClick=\"ExpImgClicked(this)\" /></span><span class='collapsible' style='display: none;'>"
						: "";
...
function SetTab() {
	...
	// window.TAB =
	// MultiplyString(parseInt(select.options[select.selectedIndex].value),
	// window.SINGLE_TAB);
