...
function dir_writable($dir){
    if(touch($dir.'/our_test_file')){
        unlink($dir.'/our_test_file');
        return true;
    }
    return false;
}
...
function check_dirfile(){
    $items = array(
	array('dir',  '可写', 'ok', './Uploads/Avatar'),
	array('dir',  '可写', 'ok', './Uploads/goods_img'),
	array('dir',  '可写', 'ok', './Uploads/Ad'),
        ...
        array('file', '可写', 'ok', './Application/Common/Conf'),
    ...
    foreach ($items as &$val) {
        ...
        if('dir' == $val[0]){
            if (!is_dir($item)) {
                $dir_exits = mkdir($item);
            }
            if(!dir_writable($item)){
                $val[1] = '不可写';
                    $val[2] = 'remove';
                    session('remove', true);
                continue;
...
function update_tables($db, $prefix = ''){
    ...
    //替换表前缀
    $sql = str_replace(" `ke_", " `{$prefix}", $sql);
