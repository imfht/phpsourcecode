...
use Admin\Model\TagRelationModel;
...
class DocumentModel extends Model{
    ...
    public function detail($id){
        ...
        $tag['tag'] = get_tag_ids_by_article_id($info['id']);
        $info = array_merge($info,$tag, $detail);

        return $info;
    }
    public function getTitle($id){
        /* 获取基础数据 */
        $info = $this->field('title')->find($id);
        if(!(is_array($info) && 1 !== $info['status'])){
            $this->error = '文档被禁用或已删除！';
            return false;
        }
    ...
    public function update($data = null){
        ...
        /* 保存标签 */
        $TagRelationModel = new TagRelationModel();
        $TagRelationModel->update($id);
        hook('documentSaveComplete', array('model_id'=>$data['model_id'],'article_id'=>$id,'data_id'=>$data['id']));
