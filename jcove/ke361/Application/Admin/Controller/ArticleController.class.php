...
class ArticleController extends AdminController {
    ...
    public function add(){
        ...
        $this->meta_title = '新增'.$model['title'];
         $tags = $this->listAll(D('Tag'));
        foreach ($tags as $k=>$v){
            if(in_array($v['id'], $data['tag'])){
                $tags[$k]['check'] =1;
            }else{
                $tags[$k]['check'] =0;
            }
    ...
    public function update(){
        ...
        $res = $document->update();
        if(!$res){
            $this->error($document->getError().session('url_to_baidu_msg'));
        }else{
            $this->success($res['id']?'更新成功'.session('url_to_baidu_msg'):'新增成功'.session('url_to_baidu_msg'), Cookie('__forward__'),8);
        }
    }
