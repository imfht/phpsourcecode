...
;$(function(){
    ...
    $('.editer').click(function(){
    	...
    	id	   = that.attr('eid');
    	if(that.children().eq(0).is('input')){
    		return;
    	}
    	txt = $(this).html();
    	input = $('<input class="form-control">');
    	input.outerWidth(that.outerWidth()+12);
    	input.val(txt);
    	input.blur(function(){
    		if($.trim(input.val())!=''&&$.trim(input.val())!=$.trim(txt)){
    			query = {'field':field,'value':input.val(),'id':id};
    			$.post(target,query).success(function(data){
                    if (data.status==1) {
                    	that.html(input.val());
                        updateAlert(data.info ,'alert-success');                         
