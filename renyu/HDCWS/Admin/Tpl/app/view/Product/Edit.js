Ext.define('HDCWS.view.Product.Edit', {
	...
	items : {
		...
		items : [
...

			{
				...
				html : '如果没有产品类型,<span style="color:#3892D3;cursor:pointer;font-weight:700">点击此处添加</span>'
	...
	bbar : [
		...
		{xtype : 'button', width : 120, height : 40, text: '取消', action : 'proEditCancel', handler : function(){this.up('window').close();}},

		{xtype : 'button', width : 120, height : 40, text: '确定', action : 'proEditSure', cls : 'edit-sure', overCls : 'edit-sure-hover'}
	...
	initComponent : function(){
		...
		Ext.Loader.loadScript({
			...
			onLoad : function(){
				...
				me.keditor.html(proData.content);
		...
		this.down('hiddenfield[name=id]').setValue(proData.id);

		this.down('textfield[name=name]').setValue(proData.name);

		this.down('textfield[name=price]').setValue(proData.price);

		var proCat = this.down('combobox[name=cid]');

		proCat.getStore().load({

			callback : function(records, operation, success) {

				proCat.select(this.findRecord('id', proData.cid));

			}

		});

		this.down('textfield[name=keywords]').setValue(proData.keywords);

		this.down('textareafield[name=description]').setValue(proData.description);

		this.down('textfield[name=buypurl]').setValue(proData.buypurl);

		this.down('radiogroup[name=status]').setValue({status : proData.status});
