loader.define(function(require,exports,module) {
	...
	var mid = 1;
	var aid = 0; //圈子ID
	var mod_array = {paper:'问卷',question:'试题'};
	var page = 1;
	var scroll_get = true;
	...
	function get_list_data(){
		layer.msg("加载中,请稍候...");
		var url = "/index.php/dati/wxapp.paper/index.html?uid="+uid+"&page="+page+"&rows=10";	//列出试卷
		if(type=='question'){
			url = "/index.php/dati/wxapp.index/listbyuid.html?uid="+uid+"&page="+page+"&rows=10";  //列出试题
		}		
		$.get(url,function(res){
			layer.closeAll();
			if(res.code==0){
	...
	function add_action(){
		...
		router.$("#add_model").click(function(){
			//Qibo.open(web_url + "/member.php/dati/paper/add.html");
			bui.load({ 
				url: "/public/static/libs/bui/pages/frame/show.html",
				param:{
					url:"/member.php/dati/paper/add.html",
				}
			});
		});
		...
		router.$('.list-hack .add').click(function(){
			var id = $(this).data("id");
			$(".chat_mod_btn").hide();
			var url = "/index.php/dati/wxapp.paper/setlive.html?type=add&aid="+aid+"&id="+id; //获取试卷信息
			if(type=='question'){
				url = "/index.php/dati/wxapp.show/index.html?aid="+aid+"&id="+id;  //获取试题信息
			}
			$.get(url,function(res){
				if(res.code==0){
					...
					setTimeout(function(){
						ws_send({
							type:"qun_to_alluser",
							tag:type=='question'?"give_datiQuestion_state":"give_dati_state",
							data: arr,
						});
						bui.back();
					},100);
	...
	pageview.format_list_data = function(array){
		...
		array.forEach((rs)=>{
			...
			if(rs.content == undefined){	//试卷
				...
				rs.content = rs.content.replace(/<[^<]+>/g,'');
				about = '暂无介绍';
			}
