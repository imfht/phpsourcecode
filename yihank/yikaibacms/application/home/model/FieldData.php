...
class FieldData extends Model {
     ...
     * @return array 列表
     */
    public function loadPageList($where,$limit = 0,$order='data_id DESC'){
        return  $this->where($where)->order($order)->paginate($limit);
    }
    ...
    public function saveData($type = 'add' , $fieldsetInfo){
        if(is_array($fieldsetInfo)||is_object($fieldsetInfo)){
        ...
        $fieldList=model('kbcms/Field')->loadList($where);
        ...
        foreach ($fieldList as $value) {
            if ($value['field']=='time'){
                if (empty($_POST['Fieldset_'.$value['field']])){
                    $data[$value['field']] = time();
                }else{
                    $data[$value['field']] = strtotime($_POST['Fieldset_'.$value['field']]);
                }
            }else{
                $data[$value['field']] = $_POST['Fieldset_'.$value['field']];
            }
