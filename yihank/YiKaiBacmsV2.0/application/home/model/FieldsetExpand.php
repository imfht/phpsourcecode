...
class FieldsetExpand extends Model {
    ...
    public function getDataInfo($fieldsetId,$dataId)
    {
        ...
        $extInfo = model('FieldData')->getInfo($dataId);
        if(empty($extInfo)){
            return ;
        }
        //获取字段列表
        ...
        $fieldList = model('FieldExpand')->loadList($where);
        if(empty($fieldList)){
            return ;
        }
        $extArray = array();
        foreach ($fieldList as $value) {
            $extArray[$value['field']] = model('FieldData')->revertField($extInfo[$value['field']],$value['type'],$value['config']);
        }
        return $extArray;
