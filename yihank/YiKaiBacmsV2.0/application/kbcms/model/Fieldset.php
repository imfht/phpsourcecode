...
class Fieldset extends Model {
     ...
     * 新增
     */
    public function add($autoKey = false){
        $data=input('post.');
        //创建数据表
        $sql=" CREATE TABLE IF NOT EXISTS `".config('database.prefix')."ext_".$data['table']."` ( ";
        if($autoKey){
            $sql .= '
                    `data_id` int(10) NOT NULL AUTO_INCREMENT ,
                    PRIMARY KEY (`data_id`)
    ...
    public function edit(){
        $data=input('post.');
        $fieldset_id=input('post.fieldset_id');
        if(empty($fieldset_id)){
            return false;
        }
        //获取信息
        $info = $this->getInfo($fieldset_id);
        //修改数据表
        $sql="
            ALTER TABLE ".config('database.prefix')."ext_".$info['table']." RENAME TO ".config('database.prefix')."ext_".$data['table']."
            ";
        $statusSql = $this->execute($sql);
        if($statusSql === false){
            return false;
    ...
    public function del($fieldsetId)
    {
        ...
        //删除数据表
        $sql="
            DROP TABLE `".config('database.prefix')."ext_".$info['table']."`
        ";
        $statusSql = $this->execute($sql);
        if($statusSql === false){
            return false;
    ...
    public function getInfoClassId($classId){
        //获取栏目信息
        $classInfo=model('kbcms/Category')->getInfo($classId);
        if(empty($classInfo)){
            return false;
        }
        //获取完整栏目信息
        $classInfo=model(strtolower($classInfo['app']).'/Category'.ucfirst($classInfo['app']))->getInfo($classId);
