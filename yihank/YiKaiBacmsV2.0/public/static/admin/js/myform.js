    ...
    if($('#uploader').length>0){
        ...
        uploader.on('uploadSuccess', function(file, ret) {
            ...
            html_img_show='<div style="float: left">' +
                ...
                '</div>';
            html='<input type="hidden" name="pics['+i+']" value="'+ret.url+'">';
            $('#webupload_hidden_img_show').append(html_img_show);
            $('#webupload_hidden_input').append(html);
            i++;
            //$('#webupload_hidden_input').attr('pics_number',i);
        });
        window.webupload_img_del=function(obj) {
            if(i!=0){
                key_id=$(obj).attr('webupload_img_del_id');
                $(obj).parent().remove();
                $("input[name='pics["+key_id+"]']").remove();
                //$('#webupload_hidden_input').attr('pics_number',i-1);
                //i--;
            }
        }
        /*$('.webupload_img_del').click(function () {
            return false;
            if(i!=0){
                key_id=$(this).attr('webupload_img_del_id');
                $(this).parent().remove();
                $("input[name='pics["+key_id+"]']").remove();
                $('#webupload_hidden_input').attr('pics_number',i-1);
                i--;
