				...
				constructGrid : function(){
					...
					if(dtGridReflectionObj.exhibitDatas!=null){
						...
						for(var i=0; i<dtGridReflectionObj.exhibitDatas.length; i++){
							...
							if (dtGridReflectionObj.option.extra) {
							...
							for(var j=0; j<columns.length; j++){
								...
								if (columns[j].rowspan) {
									var compareValue = dtGridReflectionObj.exhibitDatas[i][columns[j].id];
									//使用计算合并的字段，获取比较值
									var rowspanColumnIds = columns[j].rowspanColumnIds;
									if (rowspanColumnIds && rowspanColumnIds.length>0) {
										compareValue = '';
										var columnIdArry = rowspanColumnIds.split(',');
										for(var k=0; k<columnIdArry.length; k++)
											compareValue += dtGridReflectionObj.exhibitDatas[i][$.trim(columnIdArry[k])];
									}
									
									if (tmpColVals.hasOwnProperty(j)) {
										if (i<dtGridReflectionObj.exhibitDatas.length && tmpColVals[j].value==compareValue) {
											tmpColVals[j].count++;
											missShow = true;
											//logjs_info('miss '+ compareValue);
											if (i==dtGridReflectionObj.exhibitDatas.length-1 && tmpColVals[j].count>1) {
												rowspans.push(tmpColVals[j]);
												delete tmpColVals[j];
											}
										} else {
											if (tmpColVals[j].count>1)
												rowspans.push(tmpColVals[j]);
											delete tmpColVals[j];
											
											//下一个首行
											if (i<dtGridReflectionObj.exhibitDatas.length) {
												tmpColVals[j] = new Object();
												tmpColVals[j].column = j;
												tmpColVals[j].value = compareValue;
												tmpColVals[j].no = i;
												tmpColVals[j].count = 1;
											}
										}
									} else { //合并行单元格的'首行'
										tmpColVals[j] = new Object();
					...
					$grc = $('#'+dtGridReflectionObj.option.gridContainer).append(gridContent);
					//执行合并行单元格
					for (var inx=0; inx<rowspans.length; inx++) {
						var rowspan = rowspans[inx];
						//logjs_info(rowspan);
						$grc.find('tr[datano="'+rowspan.no+'"] td[columnno="'+rowspan.column+'"]').attr('rowspan', rowspan.count);
					}
					//备份gridId
