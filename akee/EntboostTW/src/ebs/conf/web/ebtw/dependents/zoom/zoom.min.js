/**
 * 注册事件实现点击图片弹出大图层
 * @param {string} openLinkSelector 点击链接或按钮的选择器
 * @param {string} (可选) linkUrlAttrName 保存链接访问url的属性名
 */
function registerZoom(openLinkSelector, linkUrlAttrName) {
	var e = $;
    function u(u) {
        function c() {
            function h(e) {
    ...
    (function() {
    	var zFunc = function(t) {
            t.preventDefault();
            if (e(t.target).attr("id") == "zoom") l();
        };
        t.off("click", zFunc).on("click", zFunc);
        
        e("#zoom .content").off("click", l).on("click", l); //点击图片也执行关闭
        e("#zoom .close").off("click", l).on("click", l);
        
        e("#zoom .previous").off("click", a).on("click", a);
        e("#zoom .next").off("click", f).on("click", f);
        ...
        if (e(openLinkSelector).length == 1)
        	e(openLinkSelector).addClass("zoom"); //e(".pic-list a")[0].addClass("zoom");
        e(".zoom, "+openLinkSelector).off("click", u).on("click", u);
    })();
    
    (function() {
        e(window).off("resize", c).on("resize", c);
    })();
    ...
    (function() {
        ...
        e(window).off("mousewheel DOMMouseScroll", mdFunc).on("mousewheel DOMMouseScroll", mdFunc);
    })();
}