...
class CCaptchaAction extends CAction
{
	...
	protected function renderImage($code)
	{
		...
		//$line_color = imagecolorallocate($image,rand(10,100),rand(200,255),rand(80,150));
		$line_color = imagecolorallocate($image,32,64,160);
		imageline($image,($x/2),($y/2),$w,($y/2),$line_color);
		imageline($image,($x/2),($y/2),$w,($y/2+5),$line_color);
		for($i=0; $i<100; $i++){
			imagesetpixel($image,rand()%50,rand()%30,$line_color);
		}
		for($i = 0; $i < $length; ++$i)
		{
			...
			$x = $box[2] + $this->offset;
			imageline($image,rand(10,40),($y/2),rand(10,40),$y,$line_color); 
		}
