...
class Security {
	...
	static function xss_clean($str, $is_image = FALSE)
	{
		 ...
		 */
		$str = Fliter::remove_invisible_characters($str);
		...
		do
		{
			...
			{
				$str = preg_replace_callback("#<a\s+([^>]*?)(>|$)#si", array('\\qeephp\\tools\\Security', '_js_link_removal'), $str);
			}

			if (preg_match("/<img/i", $str))
			{
				$str = preg_replace_callback("#<img\s+([^>]*?)(\s?/?>|$)#si", array('\\qeephp\\tools\\Security', '_js_img_removal'), $str);
			}
	...
	static function sanitize_filename($str, $relative_path = FALSE)
	{
		...
		$str = Fliter::remove_invisible_characters($str, FALSE);
