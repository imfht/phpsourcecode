...
class Appshop extends Common{
   ...
   public function index(){
       $view['list']  = SystemMiniapp::where(['is_lock' => 0])->order('id desc')->paginate(10,true);
   ...
   public function review(int $id){
        $view['info']  = SystemMiniapp::where(['id' => $id,'is_lock' => 0])->find();
        if(!$view['info']){
   ...
   public function buy($input){
        if(request()->isPost()){
            ...
            //判断安全密码是否正确
            $isPass = empty($this->user->safepassword) ? SystemMember::checkPasspord($this->user->id,$data['safepassword']) : Member::checkSafePasspord($this->user->id,$data['safepassword']);
            if(!$isPass){
