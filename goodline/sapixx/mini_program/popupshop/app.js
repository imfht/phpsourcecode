...
App({
  ...
  onLaunch: function (options) {
    let that = this;
    that.apiroot = siteInfo.siteroot; //设置api地址
    that.appname = siteInfo.name; //设置应用名称
    that.updateMiniapp() //小程序更新
    that.isAndroid(); //判断操作系统是Android/iOS
    that.setNavigation();  //工具栏
    //微信配置
    var wechat_config = wx.getStorageSync('wechat_config');    
    if (util.isNull(wechat_config)) {
      api.Get('openapi/v1/config', function (result) {
        wx.setStorageSync('wechat_config',result.data);
        that.globalData.wechat_config = result.data;
      })
    }else{
      that.globalData.wechat_config = wechat_config;
    }
    //登录用户信息
  ...
  getConfig:function(){
    let that = this;
    that.api().Get("api/v1/popupshop/index/config", {'signkey': 'config' }, function (result) {
      that.globalData.config = result.data;
    })
  },
  //用全局变量保存邀请码
