...
class Category extends Model{
    ...
    protected $pk     = 'id';
    protected $table  = 'ai_fastshop_cate';
     ...
     * @param int $parent_id
     */
    public function selectPath(int $miniapp_id,$parent_id) {
        $pathMaps[] = ['name'=>'根目录','url'=>url('fastshop/category/index')];
        $getPath = self::getPath($miniapp_id,$parent_id);
        foreach ($getPath as $value) {
            $pathMaps[] = ['name' => $value['title'],'url' => url('fastshop/category/index',['parent_id'=>$value['id']])];
        }
        return $pathMaps;
    }
     ...
     * @return type
     */
    public function getPath($miniapp_id,$parent_id){
