...
use app\popupshop\model\Order as AppOrder;
...
class Order extends Manage{
    ...
    public function initialize() {
        parent::initialize();
        $this->assign('pathMaps',[['name'=>'订单管理','url'=>url("popupshop/Order/index")]]);
    }
    ...
    protected function orderList(){
        ...
        $path[] = ['name'=>'订单管理','url'=>url("popupshop/Order/index")];
        $condition = [];
        if(!empty($starttime) && !empty($endtime)){
        ...
        $view['count']      = AppOrder::where($condition)->count();
        $view['payment']    = AppOrder::where($condition)->where(['paid_at' => 1, 'is_del' => 0])->sum('order_amount');
        $view['no_payment'] = AppOrder::where($condition)->where(['paid_at' => 0, 'is_del' => 0])->sum('order_amount');
        switch ($view['status']) {
            case 1:
                $condition[] = ['paid_at','=',0];
                $condition[] = ['is_del','=',0];
        ...
        $view['lists']     = AppOrder::where($condition)->order('id desc')->paginate(10);
        $view['order']     = AppOrder::order_data($view['lists']);
    ...
    public function sendgoods(){
        if(request()->isAjax()){
            ...
            if($rel){
                return enjson(200,'操作成功',['url' => url('popupshop/Order/index',['order_no' => $data['order_no']])]);
            }
            return enjson(0);
        }else{
            $order_no = Request::param('order_no/s');
    ...
    public function paid(){
        $order_no = Request::param('order_no');
        $result = AppOrder::where(['paid_at' => 0,'order_no' => $order_no])->update(['paid_at' => 1,'payment_id' => 0,'order_starttime' => time(),'paid_no' => 'GM'.order_no()]);
        if($result){
    ...
    public function delete($order_no){
        $rel = AppOrder::where(['order_no' => $order_no])->find();
        if($rel){
            if($rel['is_del'] == 0){
                $result = AppOrder::update(['is_del'=>1],['order_no' => $order_no]);
            }else{
                $result = AppOrder::where(['order_no' => $order_no])->delete();
                if($result){
                    OrderCache::where(['order_no' => $order_no])->delete();
                }
            }
            if($result){
