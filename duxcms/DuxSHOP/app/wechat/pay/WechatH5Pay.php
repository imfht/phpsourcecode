 ...
 * 微信H5移动端服务
 */
class WechatH5Pay extends \app\base\service\BaseService {


    public function getConfig() {
        $config = target('member/PayConfig')->getConfig('wechat_h5');
        if (empty($config['mch_id']) || empty($config['md5_key'])) {
            return $this->error('请先配置支付接口信息!');
        }
        $notifyUrl = url('api/wechat/WechatH5/index',[], true);
        return [
    ...
    public function getData($data, $returnUrl) {
        ...
        $config['return_url'] = DOMAIN . $returnUrl;
        ...
        try {
            $pay = \Yansongda\Pay\Pay::wechat($config)->wap($payData);
            return $this->success([
                'url' => $pay->getTargetUrl()
            ]);
        } catch (\Exception $e) {
