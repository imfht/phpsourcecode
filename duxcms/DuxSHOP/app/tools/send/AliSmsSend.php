...
class AliSmsSend extends \app\base\service\BaseService {
    ...
    public function send($info) {
        ...
        $content = html_clear(html_out($info['content']));
		preg_match("/\[ID:(.*?)\]/", $content, $contentArr);
		if($contentArr){
			$TemplateCode = $contentArr[1];	
		}else{
			return $this->error('未填写模版ID');	
		}
		$info['param'] = json_decode($info['param'], true);
		if($info['param']['url']){
			unset($info['param']['url']);
		}
		$url = $this->alisms($receive, $config['apiid'], $config['apikey'], $config['name'], $TemplateCode, $info['param']);
