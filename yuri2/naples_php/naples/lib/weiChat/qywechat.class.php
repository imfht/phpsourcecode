...
class Wechat extends Service
{
    ...
    const APPS_LIST_URL         = '/agent/list?';//自加 获取应用概况
    const MATERIA_GET_URL       = '/material/batchget?';//自加 本接口可以获取当前管理组指定类型的素材列表。
    const MPNEWS_ADD_URL        = '/material/add_mpnews?';//自加 上传图文消息素素材。
    const MPNEWS_GET_URL        = '/material/get?';//自加 获得图文消息素素材。
    ...
    const ADD_MATERIAL_URL 		= '/material/add_material?';
	...
	public function init()
	{
	    ...
	    $this->loadOption($this->config($configuration));
	}

	protected function loadOption($options){
	...
	public function getRevScanInfo(){
	    if (isset($this->_receive['ScanCodeInfo'])){
	        if (isset($this->_receive['SendPicsInfo']) and !is_array($this->_receive['SendPicsInfo'])) {
	...
	public function reply($msg=array(),$return = false)
	{
		...
		$this->log($xmldata);
		if ($return){return $xmldata;} //yuri2修改于2016/12/30 如果返回，不加密,可删除行
		$pc = new Prpcrypt($this->encodingAesKey);
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$authname = 'qywechat_access_token'.$appid.$appsecret;
	...
	public function resetAuth($appid=''){
		...
		$this->access_token = '';
		$authname = 'qywechat_access_token'.$appid.$this->appsecret;
	...
	public function resetJsTicket($appid=''){
		if (!$appid) $appid = $this->appid;
		$this->jsapi_ticket = '';
		$authname = 'qywechat_jsapi_ticket'.$appid.$this->appsecret;
		$this->removeCache($authname);
		return true;
	}
	...
	public function deleteMedia($media_id){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX.'/material/del?'.'access_token='.$this->access_token.'&media_id='.$media_id);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
	 ...
	 * @param string $type 媒体文件类型:图片（image）、语音（voice）、视频（video），普通文件(file)
     ...
     * }
     */
    public function uploadMediaForever($data, $type){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::ADD_MATERIAL_URL.'access_token='.$this->access_token.'&type='.$type,$data,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
	...
	/**
	 * 获取成员信息
	 * @param $userid string 员工UserID。对应管理端的帐号
	 * @return boolean|array	 成功返回结果
	 * {
	 *    "errcode": 0,
	 *    "errmsg": "ok",
	...
	public function getAppList(){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::APPS_LIST_URL.'access_token='.$this->access_token);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 本接口可以获取当前管理组指定类型的素材列表。
     * @param $type string 	素材类型，可以为图文(mpnews)、图片（image）、音频（voice）、视频（video）、文件（file）
     * @param $offset int
     * @param $count int
     * @return array|bool
     */
    public function getBatches($type='image',$offset=0,$count=10){
        $param=['type'=>$type,'offset'=>$offset,'count'=>$count];
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MATERIA_GET_URL.'access_token='.$this->access_token,self::json_encode($param));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return array|bool
     */
    public function getMpnews($media_id){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::MPNEWS_GET_URL.'access_token='.$this->access_token.'&media_id='.$media_id);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     */
    public function addMpNews($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MPNEWS_ADD_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function addNews($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::NEWS_ADD_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function getImgUrl($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $url="https://qyapi.weixin.qq.com/cgi-bin/media/uploadimg?access_token=".$this->access_token;
