...
class FileUpload extends Service
{
    ...
    private $newFileFullPath;          //新文件全路径
    private $errorNum = 0;             //错误号
    private $errorMess="";             //错误报告消息

    /** 自动初始化 */
    function init(){
        //['path'=>PATH_DATA.DS.'upload','allowtype'=>array('jpg','gif','png'),'maxsize'=>10000000,'israndname'=>true]
        $this->set('path',$this->config('path'))
             ->set('allowtype',$this->config('allowtype'))
             ->set('maxsize',$this->config('maxsize'))
             ->set('israndname',$this->config('israndname'));
    }
    ...
    private function copyFile() {
        if(!$this->errorNum) {
            ...
            $path=\Yuri2::autoSysCoding($path);
            \Yuri2::createDir(dirname($path));
            if (move_uploaded_file($this->tmpFileName, $path)) {
                $this->newFileFullPath[]=\Yuri2::autoEncoding($path);
                return true;
            }else{
