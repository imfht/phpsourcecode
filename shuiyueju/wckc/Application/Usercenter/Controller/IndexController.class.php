...
class IndexController extends BaseController
{
    ...
    public function information($uid = null, $info_type = "base_info")
    {
        ...
        if ($info_type == "base_info") {
            ...
            //TODO tox 获取省市区数据
            //添加用户查询username;－－水月居
            $user = query_user(array('nickname', 'name' , 'username', 'signature', 'email', 'mobile', 'rank_link', 'sex', 'pos_province', 'pos_city', 'pos_district', 'pos_community'), $uid);
            if(empty($user['name'])){ 
                  $sql=M('UcenterMember');
                  $upid=$sql->save(array('id'=>$uid, 'name'=>$user['nickname']));//更新没填写用户名的用户－－水月居
                  //dump($sql);
            }
