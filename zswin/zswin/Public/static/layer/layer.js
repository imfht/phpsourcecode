...
window.layer = {
    ...
    statusinfo: function(msg, status, fun, fundata, time, stitle){
        var conf = {
        	shade:false,
        	stsign:1,
        	fadeIn: 300,
            ...
            closeBtn: false,
            time: time === undefined ? 2 : time,
            sttitle: stitle === undefined ? "温馨提示" : stitle,
            dialog: {msg: (msg === '' || msg === undefined) ? '&nbsp;' : msg}
            
        };
        
        
        if(fundata === '' || fundata === undefined){
        	conf.end = fun;
        	
        }else{
        	conf.end =function(){fun(fundata);};
        }
        //else{
        	//alert('fun');
...
Class.pt.space = function(html){
    ...
    config.closeBtn[1] && (closebtn += '<a class="xubox_close xubox_close' + config.closeBtn[0] +'" href="javascript:;" style="'+ (config.type === 4 ? 'position:absolute; right:-3px; _right:7px; top:-4px;' : '') +'"><i class="icon-remove"></i></a>'); 
...
Class.pt.creat = function(){
    ...
    if(config.title && (config.type !== 3 || config.type !== 4)){
        ...
        switch(confbtn.btns){
            ...
            case 1:
                layerBtn.html('<a href="javascript:;" class="xubox_yes xubox_botton1  btn btn-primary">'+ confbtn.btn[0] +'</a>');
            break;
            case 2:
                layerBtn.html('<a href="javascript:;" class="xubox_yes xubox_botton2 btn btn-primary">'+ confbtn.btn[0] +'</a>' + '<a href="javascript:;" class="xubox_no xubox_botton3 btn btn-default">'+ confbtn.btn[1] + '</a>');
            break;                
        }
    }
...
Class.pt.set = function(times){
    ...
    switch(config.type){
        case 0:
        	if(config.title){
                layerE.find(doms[3]).css({paddingTop: 5 + layerTitle.outerHeight()});
            }else{
            ...
            if(config.stsign){
            	layerE.find(doms[5]).css({'color': '#fff'});
            	layerE.find('.xubox_msgico').css({top: 25});
                layerE.find(doms[3]).css({marginTop : 8}); 
                
                layerE.find(doms[3]).prepend('<div class="stitle"><b>'+config.sttitle+'</b></div>');
                layerE.find(doms[5]).prepend('<span class="stinfoclose"><a class="xubox_close"><i class="icon-remove"></i></a></span>');
                layerE.find(doms[5]).css({'height': layerE.find(doms[3]).height()+20});
                layerE.find(doms[5]).addClass('statusinfo');
...
Class.pt.callback = function(){
    ...
    layerE.find('.statusinfo').on('click',function(){
        //config.close(that.index);
        //alert(config.fadeIn);
        layer.close(that.index);
    });
...
layer.close = function(index){
    ...
    if(type == ready.type[1]){
    	...
    	if(layero.find('.statusinfo')){
    		
                  
            	layero.fadeOut("slow",function(){layero.remove();});
