...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			$this->load->model('localisation/zone');
			$zone_info = $this->model_localisation_zone->getZone($order_query->row['zone_id']);
			if($zone_info){
			...
			$city_info = $this->model_localisation_city->getCity($order_query->row['city_id']);
			if($city_info){
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, o.telephone, o.customer_id, CONCAT(o.firstname) AS customer,o.email, o.shipping_firstname AS username ,(SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added,o.type, o.date_modified FROM `" . DB_PREFIX . "order` o";
		...
		if (isset($data['filter_type']) && !is_null($data['filter_type'])) {
		    $sql .= " AND o.type = '" . (int)$data['filter_type'] . "'";
		}

		if (isset($data['filter_date_added']) && !is_null($data['filter_date_added'])) {
			$sql .= " AND DATE(o.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}
		if (isset($data['new_date']) && !is_null($data['new_date'])) {
			$sql .= " AND o.date_added >= NOW()-5";
		}
	...
	public function getTotalSales() {
      	$query = $this->db->query("SELECT SUM(value) AS total FROM `" . DB_PREFIX . "order_total` WHERE code = 'sub_total'");
      
		return $query->row['total'];
	}
	
	public function getTotalShipping() {
      	$query = $this->db->query("SELECT SUM(value) AS total FROM `" . DB_PREFIX . "order_total` WHERE code = 'shipping'");
	...
	public function addOrderHistory($order_id, $data) {
		...
		if($data['express']!='')
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET express = '" . $this->db->escape($data['express']) . "', express_website = '" . $this->db->escape($data['express_website']) . "', order_status_id = '" . (int)$data['order_status_id'] . "', date_modified = NOW() WHERE order_id = '" . $order_id . "'");
		else 	
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			if ($order_status_query->num_rows) {
			  ...
			  if($order_info['type']==2 && (int)$data['order_status_id']==2){
				  $content = '您好，您的订单：'.$order_info['order_id'].' '.$order_status_query->row['name'].'，您当面交给配送员'.$order_info['total'].'元现金。';
				  }elseif($order_info['type']==1 && (int)$data['order_status_id']==2){
					  $content = '您好，您的订单：'.$order_info['order_id'].' '.$order_status_query->row['name'].'，您当面交给服务员'.$order_info['total'].'元现金。';
					  }else{
						   $content = '您好，您的订单：'.$order_info['order_id'].' '.$order_status_query->row['name'];
						  }
			...
			$sms->mobile = $order_info['telephone'];
