...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		$name='';
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET `top` =1, code = '" . $this->db->escape($data['code']) . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
	...
	public function editCategory($category_id, $data) {
		$name='';
		$this->db->query("UPDATE " . DB_PREFIX . "category SET `top` = 1, code = '" . $this->db->escape($data['code']) . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
