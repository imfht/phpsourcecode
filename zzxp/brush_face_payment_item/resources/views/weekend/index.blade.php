...
<script type="text/javascript">
    ...
    function edit_func(id){
        ...
        sendData('{{url('admin/weekend/edit')}}','id='+id,function(obj){
    ...
    function sure_add(id){
        ...
        send_request('{{url('admin/weekend/add')}}',get_form_data(id),function(msg){
            if(msg != 1){
    ...
    function sure_edit(id){
        send_request('{{url('admin/weekend/update')}}',get_form_data(id),function(msg){
            if(msg != 1){
                show_message.alert(msg);
            }else{
                location.reload();
    ...
    function del_sub(){
        ...
        show_message.confirm('您确定要删除吗？',function(){
            send_request('{{url('admin/weekend/del')}}',query.join('&'),function(msg){
                if(msg != 1){
                    show_message.alert(msg);
                }else{
                    location.reload();
...
<section >
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a class="maincolor" href="/">周排行</a><span class="c-999 en">&gt;</span>周排行管理<span class="c-999 en">&gt;</span><span class="c-666">列表</span></nav>
    ...
    <form name="delform" action="{{ url('admin/weekend/del')}}" method="post">
      ...
      <table class="table table-border table-bordered table-striped mt-20">
        <thead>
          <tr>
            ...
            <th>会员编号</th>
				<th>金额</th>
				<th class="col2">开始时间</th>
				<th class="col2">结束时间</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$line['member_id']}}</td>
					<td>{{$line['money']}}</td>
					<td class="col2">{{$line['start_time']}}</td>
					<td class="col2">{{$line['end_time']}}</td>
    ...
    <div class="modal-content radius">
      ...
      <div class="modal-body">
		...
		<div class="row cl">
                          ...
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="start_time" placeholder="开始时间" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                          <label class="form-label col-xs-3">结束时间：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="end_time" placeholder="结束时间" name="username" id="username">
                          </div>
                        </div>
          </form>
      </div>
