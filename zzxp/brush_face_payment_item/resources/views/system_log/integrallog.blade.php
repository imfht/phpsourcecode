...
@section('content')
    ...
    <section >
        <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i>日志管理<span class="c-999 en">&gt;</span>积分记录</nav>
        <div class="text-c mt-20">
            <form>
                <input type="text" name="member_id" style="width:150px" class="input-text" value="{{$member_id}}" placeholder="会员ID" />
                积分类型： <span class="select-box inline">
                    <select name="operation_type" class="select">
                        <option value="" selected>全部</option>
                        <option value=1 @if($operation_type==1) selected @endif >充值赠送</option>
                        <option value=2 @if($operation_type==2) selected @endif >转介绍</option>
                        <option value=3 @if($operation_type==3) selected @endif >充值订单会员推荐会员奖励</option>
                        <option value=4 @if($operation_type==4) selected @endif >水消费赠送</option>
                    </select>
                </span>
                日期范围：
            ...
            <form name="delform" action="{{ url('admin/system_log/moneylog')}}" method="post">
                <div class="panel-body">
                    <table class="table table-border table-bordered table-striped mt-20">
                        ...
                        <?php
                            $index = 1;
                            $filed = [1=>'充值赠送',2=>'转介绍',3=>'充值订单会员推荐会员奖励',4=>'水消费赠送'];
                        ...
                        <tr>
                            ...
                            <td>{{$line['integral']}}</td>
                            <td>{{$line['less_integral']}}</td>
                            <td>{{$filed[$line['category']]}}</td>
                            <td>
                                @if($line['type'] == 1)
                                    增加
                                @else
