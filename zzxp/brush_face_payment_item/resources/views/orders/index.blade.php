@extends('layouts.common')

@section('content')

<script type="text/javascript">
    var global_submit_bool = true;
    function set_func(){
        global_submit_bool = true;
        $('#modal-demo input[name]').val('');
        $('#modal-demo select[name]').val('');
        $('#modal-demo textarea[name]').val('');
        $("#modal-demo").modal("show");
    }
    function edit_func(id){
        global_submit_bool = false;
        sendData('{{url('admin/orders/edit')}}','id='+id,function(obj){
            // console.log(obj);
            if(typeof obj == 'object'){
                setDefaultValue(obj.orders,'modal-demo');
                setGoodsList(obj.order_info);
                $("#modal-demo").modal("show");
            }
        },'GET');
    }  
    function sure_add(id){
        if(!global_submit_bool){
            return sure_edit(id);
        }
        send_request('{{url('admin/orders/add')}}',get_form_data(id),function(msg){
            if(msg != 1){
                show_message.alert(msg);
            }else{
                location.reload();
            }
        },function(){

        })
    }
    function setGoodsList(goods){
      var html = '';
      for(var i in goods){
        html += '<ul>';
        html += '<li>' + goods[i].goods_id + '</li>';
        html += '<li>' + goods[i].title + '</li>';
        html += '<li>' + goods[i].price + '</li>';
        html += '<li>' + goods[i].number + '</li>';
        html += '<ul>';
      }
      $('.goods_list').html(html);
    }
    function sure_edit(id){
        send_request('{{url('admin/orders/update')}}',get_form_data(id),function(msg){
            if(msg != 1){
                show_message.alert(msg);
            }else{
                location.reload();
            }
        },function(){

        })
    }
    function del_sub(){
        var bool = false;
        var query = [];
        $.each($(document.delform).find('input[name="ids[]"]'),function(i,n){
            if(n.checked){
                bool = true;
                query[query.length] = 'ids[]='+n.value;
            }
        });
        if(!bool){
            return show_message.alert('没有选中任何记录,不能删除');
        }
        show_message.confirm('您确定要删除吗？',function(){
            send_request('{{url('admin/orders/del')}}',query.join('&'),function(msg){
                if(msg != 1){
                    show_message.alert(msg);
                }else{
                    location.reload();
                }
            },function(){

            })
            // document.delform.submit();
        },function(){});
    }

    function exportData(){
        // alert('xxx');
        $('#search').attr('action',"{{url('admin/orders/export')}}");
        document.getElementById('search').submit();
        $('#search').removeAttr('action');
    }
    function uploadFile(){
        $('#search').attr('action',"{{url('admin/orders/import')}}");
        $('#search').attr('method',"POST");
        $('#search').attr('enctype',"multipart/form-data");
        document.getElementById('search').submit();
        $('#search').removeAttr('action');
        $('#search').removeAttr('method');
        $('#search').removeAttr('enctype');


    }
</script>

<style type="text/css">
    .upload_file{
        border: 1px solid #000;
        position: absolute;
        background: #fff;
        height: 36px;
        width: 60px;
        cursor: pointer;
        opacity:0;
        filter:alpha(opacity=0);
    }

</style>
<section >
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a class="maincolor" href="/">物料管理</a><span class="c-999 en">&gt;</span>订单表管理<span class="c-999 en">&gt;</span><span class="c-666">列表</span></nav>
    <div class="text-c mt-20">
        <form id="search">

            <span class="select-box inline">
              <select name="status" class="select">
                <option value="">请选择支付状态</option>
                <option value="0" {{isset($search['status']) && $search['status'] === '0' ? 'selected' :'' }}>未支付</option>
                <option value="1" {{isset($search['status']) && $search['status'] == 1 ? 'selected' :'' }}>已支付</option>
                
              </select>
            </span>
            <span class="select-box inline">
             <select name="send_status" class="select">
              <option value="">请选择发货状态</option>
              <option value="0" {{isset($search['send_status']) && $search['send_status'] === '0' ? 'selected' :'' }}>未发货</option>
              <option value="1" {{isset($search['send_status']) && $search['send_status'] == 1 ? 'selected' :'' }}>已发货</option>
              
            </select>
            </span>
            <input type="text" name="id" style="width:126px" class="input-text" value="{{isset($search['id']) ? $search['id'] :'' }}" placeholder="订单表编号" />
            
            日期范围：
            <input type="text" autocomplete="off" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}'})" id="logmin"  name="start_time" class="input-text Wdate" style="width:120px;" value={{isset($search['start_time']) ? $search['start_time'] :'' }}>
            -
            <input type="text" autocomplete="off" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'#now'})" id="logmax" name="end_time" class="input-text Wdate" style="width:120px;" value={{isset($search['end_time']) ? $search['end_time'] :''}}>
            <button name=""  id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i>&nbsp;查&nbsp;询</button>

            <a  onclick="exportData()" class="btn btn-warning radius">导&nbsp;出</a>
            <input type="file" name="import" class="upload_file" onchange="uploadFile()" />
            <a   class="btn btn-success radius">导&nbsp;入</a>
        </form>
    </div>
    <div class="panel panel-default mt-20">


    <div class="panel-header">
        订单表
        <button class="btn btn-danger radius" type="button" onclick="del_sub()">删除</button>
    </div>
    <form name="delform" action="{{ url('admin/orders/del')}}" method="post">
    <div class="panel-body">
      <table class="table table-border table-bordered table-striped mt-20">
        <thead>
          <tr>
            <th class="col1" width="6%"><input type="checkbox" /></th>
            <th class="col1">编号</th>
            <th>用户信息</th>
    				<th>订单编号</th>
            <th class="clo1">收货人信息</th>
    				<th>金额</th>
    				<th class="clo1">支付状态</th>
    				<th class="clo1">发送状态</th>
    				<th>快递信息</th>
    				<th class="col2">创建时间</th>
            <th class="col1" align="center" >操作</th>
          </tr>
        </thead>
        <tbody>

            <?php
            $index = 1;
            foreach($orders as $line){ $index ++; ?>
            <tr>
                <td class="col1"><input type="checkbox" value="{{ $line['id'] }}" name="ids[]" /></td>
                <td class="col1">{{ $line['id']}}</td>
                
                <td>
                  会员姓名：{{$line['member_name']}}<br />
                  会员电话：{{$line['member_phone']}}<br />
                </td>
      					<td>{{$line['order_sn']}}</td>
      					<td>
                  收货人姓名:{{ $line['name']}}<br />
                  收货人电话:{{ $line['phone']}}<br />
                  收货人地址:{{$line['province']}} {{$line['city']}} {{$line['county']}} {{$line['address']}}<br />
                </td>
      					<td>{{$line['money']}}</td>
      					<td class="col1"><?php $filed=[0=>'未支付',1=>'已支付)']; ?>{{isset($filed[$line['status']]) ? $filed[$line['status']] : $line['status']}}</td>
      					<td class="col1"><?php $filed=[0=>'未发货',1=>'已发货',2=>'已收货)']; ?>{{isset($filed[$line['send_status']]) ? $filed[$line['send_status']] : $line['send_status']}}</td>
      					<td>
                  单号:{{$line['express_sn']}}<br />
                  快递公司:{{$line['express_name']}}<br />
                  寄送时间:{{$line['express_time']}}<br />
                </td>
      					<td class="col2">{{$line['created_at']}}</td>

                @if(isset($line['id']))
                <td class="col1" width="6%">
                   <a href="#" class="btn btn-warning radius" onclick="edit_func({{$line['id']}});">修改</a>
                </td>
                @else
                <td>''</td>
                @endif          
            </tr>
          <?php } ?>
        </tbody>
      </table>
        @include('layouts.page')
    </div>
    </form>
  </div>

</section>

<div id="modal-demo" class="modal fade middle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content radius">
      <div class="modal-header">
        <h3 class="modal-title">订单表修改</h3>
        <a class="close" data-dismiss="modal" aria-hidden="true" href="javascript:void();">×</a> 
      </div>
      <div class="modal-body">

          <div class="row cl goods_frame">
            <div class="row_title">商品信息</div>
            <div class="goods_header">
                <ul >
                  <li>商品编号</li>
                  <li>商品名称</li>
                  <li>商品价格</li>
                  <li>商品数量</li>
                </ul>
            </div>
            <div class="goods_list">
                <ul >
                  <li>商品编号</li>
                  <li>商品名称</li>
                  <li>商品价格</li>
                  <li>商品数量</li>
                </ul>
            </div>
          </div>
          <form action="" method="post" class="form form-horizontal responsive" id="demoform" novalidate="novalidate">
            <input type="hidden" value="" name="id">
            
		          <div class="row cl">
                <label class="form-label col-xs-3">订单编号：</label>
                <div class="formControls col-xs-8">
                  <input type="text" class="input-text" name="order_sn" placeholder="订单编号" name="username" id="username">
                </div>
              </div>
		          <div class="row cl">
                <label class="form-label col-xs-3">寄送地址：</label>
                <div class="formControls col-xs-8">
                  <input type="text" class="input-text" name="address" placeholder="寄送地址" name="username" id="username">
                </div>
              </div>
		          <div class="row cl">
                <label class="form-label col-xs-3">状态：</label>
                <div class="formControls col-xs-8">
                  <select name="send_status">
                    <option value="0">未发货</option>
          					<option value="1">已发货</option>
                  </select>
                </div>
              </div>
		          <div class="row cl">
                <label class="form-label col-xs-3">快递单号：</label>
                <div class="formControls col-xs-8">
                  <input type="text" class="input-text" name="express_sn" placeholder="快递单号" name="username" id="username">
                </div>
              </div>
		          <div class="row cl">
                <label class="form-label col-xs-3">快递名称：</label>
                <div class="formControls col-xs-8">
                  <input type="text" class="input-text" name="express_name" placeholder="快递名称" name="username" id="username">
                </div>
              </div>
          </form>
      </div>
      <div class="modal-footer">
        <button class="btn btn-primary" onclick="sure_add('modal-demo')">确定</button>
        <button class="btn" data-dismiss="modal" aria-hidden="true">关闭</button>
      </div>
    </div>
  </div>
</div>


<style type="text/css">
  .row_title{font-weight: bold; text-align: center;}
  .goods_frame{     max-height: 300px;
    overflow: auto;
    padding: 5px;
    margin: auto;
    width: 600px;}
  .goods_header li{text-align: center; font-weight: bold;}
  .goods_header ul,.goods_list ul{display: block; clear: both;}
  .goods_header li,.goods_list li{float: left;
    list-style: none;
    line-height: 30px;
    padding: 5px;
    text-align: center;
    border-bottom: 1px solid #09feef;
    width: 20%;
}
</style>
@stop