...
<script type="text/javascript">
    ...
    function edit_func(id){
        ...
        sendData('{{url('admin/district/edit')}}','id='+id,function(obj){
    ...
    function sure_add(id){
        ...
        send_request('{{url('admin/district/add')}}',get_form_data(id),function(msg){
            if(msg != 1){
    ...
    function sure_edit(id){
        send_request('{{url('admin/district/update')}}',get_form_data(id),function(msg){
            if(msg != 1){
                show_message.alert(msg);
            }else{
                location.reload();
    ...
    function del_sub(){
        ...
        show_message.confirm('您确定要删除吗？',function(){
            send_request('{{url('admin/district/del')}}',query.join('&'),function(msg){
                if(msg != 1){
                    show_message.alert(msg);
                }else{
                    location.reload();
...
<section >
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a class="maincolor" href="/">参数配置</a><span class="c-999 en">&gt;</span>行政区管理<span class="c-999 en">&gt;</span><span class="c-666">列表</span></nav>
    <div class="text-c mt-20">
        <form>

            <input type="text" name="id" style="width:126px" class="input-text" value="{{isset($search['id']) ? $search['id'] :'' }}" placeholder="行政区编号" />
    ...
    <form name="delform" action="{{ url('admin/district/del')}}" method="post">
    <div class="panel-body">
      <table class="table table-border table-bordered table-striped mt-20">
        ...
        <tbody>
            ...
            <?php
            $index = 1;
            foreach($district as $line){ $index ++; ?>
            <tr>
    ...
    <div class="modal-content radius">
      ...
      <div class="modal-body">
          ...
          <form action="" method="post" class="form form-horizontal responsive" id="demoform" novalidate="novalidate">
            ...
            <div class="row cl">
                          <label class="form-label col-xs-3">行政名称：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="name" placeholder="行政名称" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                          <label class="form-label col-xs-3">新政级别划分：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="grade" placeholder="新政级别划分" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                          <label class="form-label col-xs-3">父级编号：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="parent_id" placeholder="父级编号" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                          <label class="form-label col-xs-3">首字母：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="initial" placeholder="首字母" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                          <label class="form-label col-xs-3">经度：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="lng" placeholder="经度" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                          <label class="form-label col-xs-3">纬度：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="lat" placeholder="纬度" name="username" id="username">
                          </div>
                        </div>
          </form>
      </div>
