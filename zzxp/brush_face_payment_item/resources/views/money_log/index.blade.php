...
<script type="text/javascript">
    ...
    function edit_func(id){
        ...
        sendData('{{url('admin/money-log/edit')}}','id='+id,function(obj){
    ...
    function sure_add(id){
        ...
        send_request('{{url('admin/money-log/add')}}',get_form_data(id),function(msg){
            if(msg != 1){
    ...
    function sure_edit(id){
        send_request('{{url('admin/money-log/update')}}',get_form_data(id),function(msg){
            if(msg != 1){
                show_message.alert(msg);
            }else{
                location.reload();
    ...
    function del_sub(){
        ...
        show_message.confirm('您确定要删除吗？',function(){
            send_request('{{url('admin/money-log/del')}}',query.join('&'),function(msg){
                if(msg != 1){
                    show_message.alert(msg);
                }else{
                    location.reload();
...
<section >
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a class="maincolor" href="/">代理商管理</a><span class="c-999 en">&gt;</span>资金流水管理<span class="c-999 en">&gt;</span><span class="c-666">列表</span></nav>
    <div class="text-c mt-20">
        <form>

            <input type="text" name="id" style="width:126px" class="input-text" value="{{isset($search['id']) ? $search['id'] :'' }}" placeholder="资金流水编号" />
    ...
    <form name="delform" action="{{ url('admin/money_log/del')}}" method="post">
    <div class="panel-body">
      <table class="table table-border table-bordered table-striped mt-20">
        ...
        <tbody>
            ...
            <?php
            $index = 1;
            foreach($money_log as $line){ $index ++; ?>
            <tr>
                ...
                <td>
                  ...
                  电话:{{$line['phone']}}<br />
					...
					<td>{{$line['money']}}</td>
					<td>{{$line['less_money']}}</td>
					<td class="col1"><?php $filed=[0=>'进账',1=>'',2=>'进账-1出账)']; ?>{{isset($filed[$line['type']]) ? $filed[$line['type']] : $line['type']}}</td>
  ...
  <div class="modal-dialog">
    <div class="modal-content radius">
      <div class="modal-header">
        <h3 class="modal-title">资金流水修改</h3>
      ...
      <div class="modal-body">
		...
		<div class="row cl">
                          <label class="form-label col-xs-3">影响后金额：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="less_money" placeholder="影响后金额" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                      <label class="form-label col-xs-3">状态：</label>
                      <div class="formControls col-xs-8">
                        <select name="type"><option value="0">进账</option>
					<option value="1"></option>
					<option value="2">进账-1出账)</option></select>
                      </div>
                    </div>
		<div class="row cl">
