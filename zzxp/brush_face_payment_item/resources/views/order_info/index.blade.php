...
<script type="text/javascript">
    ...
    function edit_func(id){
        ...
        sendData('{{url('admin/order_info/edit')}}','id='+id,function(obj){
    ...
    function sure_add(id){
        ...
        send_request('{{url('admin/order_info/add')}}',get_form_data(id),function(msg){
            if(msg != 1){
    ...
    function sure_edit(id){
        send_request('{{url('admin/order_info/update')}}',get_form_data(id),function(msg){
            if(msg != 1){
                show_message.alert(msg);
            }else{
                location.reload();
    ...
    function del_sub(){
        ...
        show_message.confirm('您确定要删除吗？',function(){
            send_request('{{url('admin/order_info/del')}}',query.join('&'),function(msg){
                if(msg != 1){
                    show_message.alert(msg);
                }else{
                    location.reload();
...
<section >
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a class="maincolor" href="/">订单明细</a><span class="c-999 en">&gt;</span>订单明细管理<span class="c-999 en">&gt;</span><span class="c-666">列表</span></nav>
    ...
    <form name="delform" action="{{ url('admin/order_info/del')}}" method="post">
    <div class="panel-body">
      <table class="table table-border table-bordered table-striped mt-20">
        ...
        <tbody>
            ...
            <?php
            $index = 1;
            foreach($order_info as $line){ $index ++; ?>
            <tr>
          ...
          <form action="" method="post" class="form form-horizontal responsive" id="demoform" novalidate="novalidate">
            ...
            <div class="row cl">
                          ...
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="order_id" placeholder="商品编号" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
                          <label class="form-label col-xs-3">货品编号：</label>
                          <div class="formControls col-xs-8">
                            <input type="text" class="input-text" name="goods_id" placeholder="货品编号" name="username" id="username">
                          </div>
                        </div>
		<div class="row cl">
