...
class User extends CI_Controller {
	...
	function editxinzhi() {
		...
		if (null !== $this->input->post ( 'submit' )) {
			...
			$desrc =htmlspecialchars($_POST ['content']);  // $this->input->post ( 'content' );
			$outimgurl = $this->input->post ( 'outimgurl' );
			...
			if ($touser) {
	         if($_COOKIE['findwpdbyemail']){
	         	...
	         	if ($touser) {
	         		...
	         		$this->db->where(array('email'=>$email))->update('user',$data);
	...
	*/
	function getphonepass(){
		...
		if (null !== $this->input->post ( 'submit' )) {
			...
			if ($touser) {
				$data=array('password'=>md5($password));
				$this->db->where(array('phone'=>$userphone))->update('user',$data);
				$this->message ( "密码修改成功，请重新登录!","user/default" );
			}
	...
	function sendcheckmail() {
		if ($this->user ['uid'] > 0) {
			...
			if (isset ( $this->user ['email'] ) && $this->user ['email'] != "") {
				...
				$activecode = md5 ( rand ( 000000000, 999999999 ) );
				$url = SITE_URL . 'index.php?user/checkemail/' . $this->user ['uid'] . '/' . $activecode;
				$message = "这是一封来自【" . $sitename . "】邮箱验证，<a target='_blank' href='$url'>请点击此处验证邮箱邮箱账号</a>";
				$v = md5 ( "yanzhengask2email" );
				$v1 = md5 ( "yanzhengask2time" );
				...
				exit ( "邮箱验证发送成功，10分钟之内请进行邮箱验证，在您没激活邮件之前你不能发布问题和文章等操作！" );
