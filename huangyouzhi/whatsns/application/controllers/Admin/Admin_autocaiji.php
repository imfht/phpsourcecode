  ...
  PRIMARY KEY (`id`)
	...
	function getarticle() {
		...
		try {
			...
			$result ['neirong'] =$desc;// $this->fillter ( $desc );
...
// 			}
			...
			if ($ckabox == 'true' || $ckabox == 'on') {
				...
				$atest =filter_outer($atest);// preg_replace ( "#<a[^>]*>(.*?)</a>#is", "$1", $atest );
			}
			if ($imgckabox == 'true' || $imgckabox == 'on') {
			...
			foreach ( $type_fill as $r ) {
				...
				if ($ckabox == 'true' || $ckabox == 'on') {
					...
					$caijilist [$count1] ['title'] =filter_outer($caijilist [$count1] ['title'] );// preg_replace ( "#<a[^>]*>(.*?)</a>#is", "$1", $caijilist [$count1] ['title'] );
				}
				if ($imgckabox == 'true' || $imgckabox == 'on') {
...
$hduid = $this->user_model->caijiadd ( $wendausername, '123456', rand ( 1111111, 99999999 ) . "@qq.com" );
				...
				if ($ckabox == '1' || $ckabox == 'on') {
					...
					$answer_content =filter_outer($answer_content);// preg_replace ( "#<a[^>]*>(.*?)</a>#is", "$1", $answer_content );
				}
	...
	function posteditguize() {
		...
		$source = $_POST['source'];
		//插入数据库
        $data=array('caiji_url'=>$caiji_url,'atitle'=>$atitle,'tiwenshijian'=>$tiwenshijian,'huidashijian'=>$huidashijian,'caiji_prefix'=>$caiji_prefix,'category1'=>$category1,'category2'=>$category2,'category3'=>$category3,'cid'=>$cid,'ckabox'=>$ckabox,'caijitype'=>$caijitype,'imgckabox'=>$imgckabox,'bianma'=>$bianma,'guize'=>$guize,'daanyuming'=>$daanyuming,'daandesc'=>$daandesc,'caiji_best'=>$caiji_best,'caiji_hdusername'=>$caiji_hdusername,'caiji_hdusertx'=>$caiji_hdusertx,'source'=>$source);
		$this->db->set($data)->where(array('id'=>$id))->update('autocaiji');
	...
	function addquestion($title, $description, $zuijiadaan, $cid, $qtime, $views, $uid, $username, $cid1 = 0, $cid2 = 0, $cid3 = 0, $status = 1, $shangjin = 0, $askfromuid = 0) {
		...
		$dataquestion=array('views'=>$views,'cid'=>$cid,'cid1'=>$cid1,'cid2'=>cid2,'cid3'=>$cid3,'askuid'=>$askfromuid,'authorid'=>$uid,'shangjin'=>$shangjin,'author'=>$username,'title'=>$title,'description'=>$description,'price'=>$price,'time'=>$creattime,'endtime'=>$endtime,'hidden'=>$hidanswer,'status'=>$status,'ip'=>getip());
