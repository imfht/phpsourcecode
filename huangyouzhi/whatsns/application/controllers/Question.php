...
class Question extends CI_Controller {
	...
	function ajaxquickadd() {
		...
		if ($askfromuid) {
			...
			if ($needpay > 0) {
				// 判断用户余额是否足够支付
				$useryue = $this->user ['jine'] / 100;
				if ($useryue >= $needpay) {
				} else {
					// 余额不足
					$message ['message'] = "您账户余额不足，无法付费咨询";
					echo json_encode ( $message );
					exit ();
				}
			}
	...
	function ajaxadd() {
		...
		if ($qid > 0) {
			...
			$neemayzhuanjia = $touser ['mypay'] * 100;
			if ($askfromuid) {
				$id = $this->depositmoney_model->add ( $this->user ['uid'], $touser ['mypay'], 'eqid', $qid, $touser ['uid'] );
				if ($id > 0) {
					$this->db->query ( "UPDATE " . $this->db->dbprefix . "user SET  `jine`=jine-'$neemayzhuanjia' WHERE `uid`=$uid" );
					$xsmoney = $touser ['mypay'];
					$time = time ();
					$fromuid = $touser ['uid']; // 专家uid
					$this->db->query ( "INSERT INTO " . $this->db->dbprefix . "paylog SET type='fufeitiwen',typeid=$qid,money=$xsmoney,openid='',fromuid=$fromuid,touid=$uid,`time`=$time" );
					
				}
			}
			if ($shangjin > 0) {
				$this->depositmoney_model->add ( $this->user ['uid'], $shangjin, 'qid', $qid );
				$tpshangjin = $shangjin * 100;
				$this->db->query ( "UPDATE " . $this->db->dbprefix . "user SET  `jine`=jine-'$tpshangjin' WHERE `uid`=$uid" );
				
				$time = time ();
				$this->db->query ( "INSERT INTO " . $this->db->dbprefix . "paylog SET type='wtxuanshang',typeid=$qid,money=$shangjin,openid='',fromuid=0,touid=$uid,`time`=$time" );
			}
		}
		if ($hidanswer == 0) {
			$this->user_model->follow ( $qid, $this->user ['uid'], $this->user ['username'] );
	...
	function answer() {
		...
		$aid = intval ( $this->uri->rsegments [4] ); // 接收qid参数
		...
		if (empty ( $useranswer )) {
			...
			echo "font-family: cursive;font-size: 20px;text-shadow: 0 1px 0 #fff;  letter-spacing: 1px;line-height: 2em;margin-top: -50px;}";
			echo ".nullpage p a{margin-left:10px;font-size:20px;}";
			echo '</style></head><body> <div class="nullpage"><p><span>4</span><span>0</span><span>4</span></p><p>回答已经被删除！⊂((δ⊥δ))⊃<a href="/">返回主页</a></p></div></body></html>';
			exit ();
		}
		...
		// 定义熊掌号推送的url数组
		$tuiurls = array ();
		$_url = url ( "question/view/$qid" ); // 此问题的url
		array_push ( $tuiurls, $_url );
		
		$_url = url ( "question/answer/$qid/" . $useranswer ['id'] ); // 此回答的url
		array_push ( $tuiurls, $_url );
	...
	function view() {
		...
		// 如果用户打开悬赏问题，并且采纳超时，系统设置最佳答案
		if ($question ['shangjin'] > 0) { // 赏金大于0
			$outhour = $this->setting ['question_outtime'];
			
			if (! $outhour) {
				$outhour = 24;
			}
			$outtime = strtotime ( "+$outhour hour", $question ['time'] ) - time (); // 获取悬赏采纳过期时间戳
			
			if ($outtime <= 0 && $bestanswer ['adopttime'] <= 0 && $question ['status'] != 9) { // 如果过期了并且没有最佳答案就自动采纳
			                                                                                    // runlog('bug', $bestanswer['adopttime']);
			                                                                                    // 自动采纳代码
				$this->question_model->setbestanswer ( $question );
				$bestanswer = $this->answer_model->get_best ( $qid );
				// runlog('ceshichaoshi', 'meiyou');
				...
				if (count ( $answerlist ) > 0) {
					...
					$time = time ();
					// runlog('xuanshang', "UPDATE " . $this->db->dbprefix . "user SET `jine`=jine+'$cash_fee' WHERE `uid`=$touid");
					// 将悬赏金额给被采纳者
					...
					if ($adoptmoeny > 0) {
						...
						$sitemoney = $adoptmoeny * $weixin_fenceng_zuijia;
					...
					if ($model != null) {
						if ($model ['touid'] != $touid) {
							// 先判断行家是否回答问题
							$answer = $this->answer_model->getanswerbyuid ( $quid, $question ['id'] );
							if (! $answer) {
			...
			// runlog('ceshichaoshi', 'haole'.$bestanswer['content']);
			
			$bestanswer ['adopttime'] <= 0 ? $showtime = 1 : $showtime = 0;
		}
		...
		if ($this->setting ['seo_question_keywords']) {
			...
			$seo_keywords = str_replace ( "{wtbq}", trim ( $tagkeystr, ',' ), $seo_keywords );
	...
	function close() {
		...
		if ($question ['answers'] > 0 && $question ['shangjin'] > 0) {
			$this->message ( "悬赏问题已经有回答，不能关闭！" );
		}
		$this->question_model->update_status ( $qid, 9 );
		$viewurl = urlmap ( 'question/view/' . $qid, 2 );
