...
<div id="comment-list" class="comment-list bb" style="margin:0px;margin-bottom:20px;{if $question['answers']==0}display:none;{/if}">
    ...
    <source src="{SITE_URL}data/weixinrecord/{$useranswer['mediafile']}" type="audio/mpeg" />
             ...
             <div class="tool-group">
                <a class="button_agree dianzan" id='{$useranswer['id']}'><i class="fa fa-eject"></i> <span>{$useranswer['supports']}人赞</span></a>
             <a class="icon_discuss jsslideshowcomment showcommentid" data-id="{$useranswer['id']}" onclick="show_comment('{$useranswer['id']}');"><i class="fa fa-cloud"></i> <span>  添加讨论({$useranswer['comments']})</span></a>
            
                  <!--{if  1==$user['grouptype'] ||$user['uid']==$useranswer['authorid']}-->

                       <a href="{url answer/append/$question['id']/$useranswer['id']}" data-original-title="继续回答问题"  data-placement="bottom" title="" data-toggle="tooltip" ><i class="fa fa-edit"></i> <span>继续回答</span></a>

               <a href="{url question/editanswer/$useranswer['id']}" data-original-title="修改自己答案"  data-placement="bottom" title="" data-toggle="tooltip" ><i class="fa fa-edit"></i> <span>编辑</span></a>
                 <!--{/if}-->
                <!--{if $user['uid']==$question['authorid']}-->

           <a data-placement="bottom" title="" data-toggle="tooltip" data-original-title="继续回答问题" href="{url answer/append/$question['id']/$useranswer['id']}"><i class="fa fa-file-powerpoint-o"></i> <span>追问</span></a>


               <!--{/if}-->
             {if $setting['openwxpay']==1}
        <a class="shangke " data-placement="bottom" title="" data-toggle="tooltip" {if $useranswer['total']==0} data-original-title="任意打赏Ta" {else} data-original-title="已获得网友$useranswer['total']元赏金" {/if} onclick="wxpay('aid',{$useranswer['id']},{$useranswer['authorid']} );"><i class="fa fa-hongbao color-white"></i></a>
                {/if}

               <a class="report" onclick="openinform({$question['id']},'{$question['title']}',{$useranswer['id']})"><span>举报</span></a>
                <!---->
                </div>
                </div>
