...
class Page
{
    ...
    {
        $returnUrl = $this->setUrl($this->customUrl); //配置url地址
        /**
         * 数型返回url地址
         * b(before)返回url地址前部分
         * a(after)返回url地址后部分
    ...
    protected function setUrl($customUrl)
    {
        if (!is_null(self::$staticUrl)) {
            ...
            switch ($url_type) {
                case 1:
                    $url = __WEB__.'/'. MODULE .'/'. CONTROLLER .'/'. ACTION . '/';
                    foreach ($get as $k => $v) {
                        $url .= $k . '/' . $v . '/';
                    }
                ...
                default:
                    $array = array(
                        C('VAR_MODULE')=>MODULE,
                        C('VAR_CONTROLLER')=>CONTROLLER,
                        C('VAR_ACTION')=>ACTION,
                    );
                    $url = __WEB__ . '?' . http_build_query($array) . '&';
                    foreach ($get as $k => $v) {
                        $url .= $k . "=" . $v . '&';
                    }
    ...
    protected function pre()
    {
        ...
        return "<span class='disabled'>{$this->desc['pre']}</span>";
    }
