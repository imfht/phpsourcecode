...
abstract class Hook
{
    ...
    {
        Log::record("[ $hook ] --START--", Log::INFO);
        if (!isset(self::$hook[$hook])) return false;
        foreach (self::$hook[$hook] as $name) {
            if (false == self::exec($name, $hook, $param)) return;
        }
    }
     ...
     * @return boolean
     */
    static public function exec($name, $action, &$param = null)
    {
        if (substr($name, -4) == 'Hook') { //钓子
            $action = 'run';
        } else { //插件
            require_cache(APP_ADDON_PATH . $name . '/' . $name . 'Addon'.EXT);
            $name = $name . 'Addon';
