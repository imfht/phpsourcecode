 ...
 */
namespace Took\Cache\Driver;
class Redis extends \Took\Cache
{
    ...
    private function connectRedis() {
        ...
        foreach ($hostArr as $h) {
            ...
            } catch (\Took\Exception $e) {
                if (C("DEBUG")) {
                    throw_exception("Redis{$_host} connection fails");
                }
                \Took\Log::write($e->getMessage());
            }
        }
    }
    ...
    public function set($name, $value, $expire = null) {
        ...
        if ($this->options['save'] === true) {
            if ($data === false) {
                N("took_cache_set_misses", 1);
            } else {
                N("took_cache_set_hits", 1);
            }
        }
        return $data;
    }
