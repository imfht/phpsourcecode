...
class Upload {
    ...
    private $config = array(
        ...
        'allowEmptyFile'  =>  false, // 是否允许上传文件为空
    );
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            $image = in_array($ext, array('gif','jpg','jpeg','bmp','png'));
            ...
            /* 保存文件 并记录保存成功的文件 */
            $driver = strtolower(basename(get_class($this->uploader)));
            if ($this->uploader->save($file,$this->replace)) {
                unset($file['error'], $file['tmp_name']);
                $file['uptime'] = time();
                $file['image'] = $image ? intval($image) : (strpos($files[$key]['type'],'image/')===false ? 0 : 1);
                $file['driver'] = $driver;
                if($driver=='local') {
                    $filepath = realpath($this->rootPath.$file['savepath'].$file['savename']);
                    $file['filepath'] = __ROOT__ . '/' . str_replace('\\','/',substr(realpath(realpath($filepath)),strlen(realpath(ROOT_PATH))+1));
                    $file['realpath'] = str_replace('\\','/',$filepath);
                }
                $info[$key] = $file;
            } else {
                $this->error = $this->uploader->getError();
    ...
    private function setDriver($driver = null, $config = null){
        ...
        $class = strpos($driver,'\\')? $driver : 'Took\\Upload\\Driver\\'.ucfirst(strtolower($driver));
    ...
    private function check($file) {
        /* 文件上传失败，捕获错误代码 */
        if ($file['error']) {
            if($this->config['allowEmptyFile'] && $file['error']==4) { //补充参数
                return false;
            }
    ...
    private function error($errorNo) {
        switch ($errorNo) {
            ...
            case UPLOAD_ERR_FORM_SIZE: //value: 2
                $this->error = '上传文件的大小超过了 HTML 表单中 MAX_FILE_SIZE 选项指定的值！'; //文件超过表单限制大小
                break;
