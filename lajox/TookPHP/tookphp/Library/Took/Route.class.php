...
final class Route
{
    ...
    static public function parseUrl()
    {
        ...
        //以下划线分隔的模块名称改为pascal命名如tookphp_user=>PhpUser
    ...
    static public function setConst()
    {
        ...
        //框架目录相关URL
        defined('__TOOK__') or define("__TOOK__", __HOST__ . '/' . trim(str_ireplace(str_replace('\\', '/', $_SERVER['DOCUMENT_ROOT']), "", TOOK_PATH), '/'));
        defined('__TOOK_DATA__') or define("__TOOK_DATA__", __TOOK__ . '/Data');
        //应用
     ...
     * @return string
     */
    static public function getUrl($path, $args = array(), $type = 0)
    {
        if (preg_match('/^https?:\/\//', $path)) {
            return $path;
        }
        $type = $type ? intval($type) : C("URL_TYPE");
        // 开启伪静态时去除入口文件
        ...
        if (preg_match('@[^\w/]@i', $path)) {
            /**
             * 参数如: m=Home&c=Index&a=index&cid=1&page=2
             * 这样形式时直接进行路由解析
             * 比如在分页类时使用
             */
