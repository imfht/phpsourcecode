...
/*    	
        ...
        $mode=1;
        $page=new \Lib\Page();
        $page->pageSuffix='.html';//后缀
        $page->pageWrap="div";//分页容器，默认为''
        $cur_page=$page->getCurPage();//当前页码
        $limit_start=($cur_page-1)*$listRows;//开始条数
        $listRows=10;//每页显示条数
        $limit=$limit_start.','.$listRows;//sql的limit
...
class Page 
 {		
	...
	public $pageWrap='';//分页字符串容器
	public $pageWrapClass='pagewrap';//分页字符串容器class
	public $pageWrapId='';//分页字符串容器的id
	...
	public function show($url="",$total=0,$perPage=10,$pageBarNum=10,$mode=1)
	{
		...
		switch ($mode)
		{
			case 1:
				...
				break;
			case 2:
				$pageString = $this->firstPage().$this->prePage().'[第'.$this->nowIndex.'页]'.$this->nextPage().$this->lastPage().'第'.$this->select().'页';
				break;
			case 3:
		...
		if($this->pageWrap){
		    ...
		    $pageWrapClass = $this->pageWrapClass ? " class=\"{$this->pageWrapClass}\"" : '';
		    $pageWrapId = $this->pageWrapId ? " id=\"{$this->pageWrapId}\"" : '';
		    $pageString = "<{$pageWrap}{$pageWrapClass}{$pageWrapId}>".$pageString."</{$this->pageWrap}>";
		}
		//输出完整分页字符串
		return $pageString;		
 }
...
private function _requestUri()
{
    ...
    //url中省略action时防止翻页链接地址出错
	if(Route::$status['actionIsDefault']){
		$uri = Config::get('GROUP_DEFAULT') ? url(CP_GROUP.'/'.CP_MODULE.'/'.CP_ACTION) : url(CP_MODULE.'/'.CP_ACTION);
		if(!empty($_SERVER['QUERY_STRING'])) $uri = $uri.'?'.$_SERVER['QUERY_STRING'];//加入其它参数
	}
    return $uri;
}
 ...
 {
 	//原生url处理
 	if (Config::get('URL_REWRITE_ON')<2) {
 		$this->_gmaUrl($url);
 		return ;
 	}
	...
	{
		 $this->requestUri= $url ? $url : $this->_requestUri(); //设置地址时覆盖自动获取的值	
 ...
 private function _gmaUrl($url=""){
 	if(!empty($url)) return $url;
 	$urlarray = $_GET;
 	unset($urlarray[$this->pageName]);
    $str = http_build_query($urlarray);
    $str = empty($str) ? '' : $str."&";
    $uri = $_SERVER['PHP_SELF'] .'?'. $str.$this->pageName."={page}";    
