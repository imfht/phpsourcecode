...
class Image {
     ...
     * @return void
     */
    static public function water($source, $water, $savename=null, $alpha=80,$waterPos=9) {
        //检查文件是否存在
	 ...
	 * 裁剪功能方法
	 *
	 * @param unknown_type $thumb_image_name 目标文件
	 * @param unknown_type $image 源文件
	 * @param unknown_type $width 原图要载入的宽度
	 * @param unknown_type $height 原图要载入的高度
	 * @param unknown_type $start_width 原图要载入的区域x开始坐标
	 * @param unknown_type $start_height 原图要载入的区域y开始坐标
	 * @param unknown_type $scale 缩放比例，如需规定尺寸则 $scale=260/$width 这样生成的尺寸就是260
	 * @return unknown
	 * 
	 */
	static function cropImage($thumb_image_name, $image, $width, $height, $start_width, $start_height, $scale){
    ...
    // 中文验证码 如果看不到图片，有可能是$fontface字体路径错误
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='msyhbd.ttf', $verifyName='verify') {       
        $code = random($length, 4);
