...
class UploadFile {//类定义开始
    ...
    private $config =   array(
        ...
        'formName'          =>  false,    // 使用表单name作为数组key ture是 false否
        'thumb'             =>  false,    // 使用对上传图片进行缩略图处理		
        'thumbMaxWidth'     =>  '150',// 缩略图最大宽度
        'thumbMaxHeight'    =>  '150',// 缩略图最大高度
    ...
    private function dealFiles($files) {
       ...
       foreach ($files as $form => $file){
           if(is_array($file['name'])) {
               $keys = array_keys($file);              
               foreach ($file['name'] as $formkey=>$val){
               	   foreach ($keys as $key){
                       $fileArray[$n][$key] = $file[$key][$formkey];                                        
                   }
                   $fileArray[$n]['form'] = $form."[{$formkey}]"; 
                   $n++;
               }               
           }else{
               $fileArray[$n] = $file;
               $fileArray[$n]['form'] = $form;
               $n++;
           }
       }
       return $fileArray;
