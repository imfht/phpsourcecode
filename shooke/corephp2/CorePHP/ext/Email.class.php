 ...
 * 邮件发送类,基于PHPMailer类
        ...
        $config['SMTP_USERNAME']='82523829@qq.com';//smtp服务器帐号，如：你的qq邮箱
        $config['SMTP_PASSWORD']='123456';//smtp服务器帐号密码，如你的qq邮箱密码
        $config['SMTP_AUTH']=true;//启用SMTP验证功能，一般需要开启
        $config['SMTP_CHARSET']='utf-8';//发送的邮件内容编码
        $config['SMTP_FROM_TO']='82523829@qq.com';//发件人邮件地址
        $config['SMTP_FROM_NAME']='CorePHP官方';//发件人姓名
        $config['SMTP_DEBUG']=false;//是否显示调试信息
        
        \Ext\Email::init($config);//初始化配置
        
        //参数1：接收人的邮箱地址；参数2：邮件标题；参数3：邮件内容
        \Ext\Email::send('82523829@qq.com','邮件标题','邮件内容');//发送邮件
 *
 */
class Email
{
    ...
    static public function init($config = array()) 
    {
		...
		self::$config['SMTP_USERNAME']=isset($config['SMTP_USERNAME'])?$config['SMTP_USERNAME']:'82523829@qq.com';//smtp服务器帐号，如：你的qq邮箱
		...
		self::$config['SMTP_FROM_TO']=isset($config['SMTP_FROM_TO'])?$config['SMTP_FROM_TO']:'82523829@qq.com';//发件人邮件地址
		self::$config['SMTP_FROM_NAME']=isset($config['SMTP_FROM_NAME'])?$config['SMTP_FROM_NAME']:'shooke';//发件人姓名
