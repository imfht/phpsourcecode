...
(function(){
	...
	ABP.bind = function (playerUnit, mobile, state,vpath,durl) {
		var ABPInst = {
			btnPlay:null,
			barTime:null,
			barLoad:null,
		...
		ABPInst.swapVideo = function(video){
			video.addEventListener("timeupdate", function(){
				ABPInst.Time.innerHTML=gTimeFormat(video.currentTime)+"/"+gTimeFormat(video.duration);
				if(!dragging)
			...
			if(ABPInst.cmManager){
				...
				video.addEventListener("ratechange", function(){
					if(ABPInst.cmManager.options.scroll.scale != null){
						if(video.playbackRate !== 0){
							ABPInst.cmManager.options.scroll.scale = (1 / video.playbackRate);
		...
		/** Bind the TimeProgress **/
		var tbar = playerUnit.getElementsByClassName("ABP-Time");
		if(tbar.length <= 0)return ;
		ABPInst.Time =tbar[0];
		/** Bind the soundControl button **/
		var sbar = playerUnit.getElementsByClassName("progress");
		if(sbar.length <= 0)return ;
		ABPInst.soundControl = sbar[0];
		/** Bind the soundOpen/Off Control **/
		var sbar = playerUnit.getElementsByClassName("glyphicon");
		if(sbar.length <= 0)return ;
		ABPInst.soundIcon = sbar[0];
		...
		if(video.isBound !== true){
			...
			ABPInst.btnFull.addEventListener("click", function(){
				...
				if(!ABPInst.state.fullscreen){
					...
					this.setAttribute("title","关闭全屏");
				}else{
					removeClass(playerUnit, "ABP-FullScreen");
					this.setAttribute("title","全屏");
				}
				...
				if(ABPInst.state.fullscreen){
					if(ABPInst.defaults.w >0){
						ABPInst.cmManager.options.scroll.scale = playerUnit.offsetWidth / ABPInst.defaults.w;
					}
				}else{
					ABPInst.cmManager.options.scroll.scale = 1;
				}
			});
			ABPInst.btnDm.addEventListener("click", function(){
				if(ABPInst.cmManager.display == false){
					...
					ABPInst.cmManager.clear();
					ABPInst.cmManager.stopTimer();
					this.className = "button ABP-CommentShow ABP-Close";
					this.setAttribute("title","打开弹幕");
				}   
			}); 
			ABPInst.barTime.style.width = "0%";
			var dragging = false;
			...
			ABPInst.soundControl.addEventListener("mouseup",function(e){
				dragging = false;
				var vol =((e.offsetX) * 100 / this.offsetWidth) ;
				this.firstElementChild.style.width=vol + "%";
				this.firstElementChild.innerHTML= Math.floor(vol) + "%"
				ABPInst.video.volume=vol/100;
				if(ABPInst.video.volume==0){
					ABPInst.soundIcon.className="button glyphicon glyphicon-volume-off";
				}else{
					ABPInst.soundIcon.className="button glyphicon glyphicon-volume-up";
				}
			});

			ABPInst.soundControl.addEventListener("mousedown",function(e){
				dragging = true;
			});

			ABPInst.soundControl.addEventListener("mousemove",function(e){
				if(dragging){
					var vol =((e.offsetX) * 100 / this.offsetWidth) ;
					this.firstElementChild.style.width=vol + "%";
					this.firstElementChild.innerHTML= Math.floor(vol) + "%"
			...
			ABPInst.soundIcon.addEventListener("click",function(){
				if(ABPInst.soundIcon.className=="button glyphicon glyphicon-volume-off"){
					ABPInst.soundIcon.className="button glyphicon glyphicon-volume-up";
					ABPInst.soundControl.firstElementChild.style.width=Math.floor(sound*100)+"%";
					ABPInst.soundControl.firstElementChild.innerHTML= Math.floor(sound*100)+"%";
					ABPInst.video.volume=sound;
				}else{
					sound = ABPInst.video.volume;
					ABPInst.soundIcon.className="button glyphicon glyphicon-volume-off";
					ABPInst.video.volume = 0;
					ABPInst.soundControl.firstElementChild.style.width=0+"%";
					ABPInst.soundControl.firstElementChild.innerHTML = 0+"%";
				}
			});
			ABPInst.barHitArea.addEventListener("mousemove", function(e){
				...
				ABPInst.barHitArea.setAttribute("data-original-title",gTimeFormat(((e.layerX) / this.offsetWidth) * ABPInst.video.duration));
				document.getElementsByClassName("tooltip-inner")[0].innerHTML=gTimeFormat(((e.layerX) / this.offsetWidth) * ABPInst.video.duration);
			});
			...
			ABPInst.txtText.addEventListener("keyup", function(k){
				...
				if(k != null && k.keyCode === 13){
					...
					if(/^!/.test(this.value)){
						...
						CommentSender(vpath,ABPInst.cmManager,ABPInst.video,ABPInst.txtText.value,durl);//这里url最好用绝对定位
						this.value="";
					}
					...
					/*
						这里做了修改，滚动字幕与全局字幕透明度一起设定
					*/
						if(ABPInst.cmManager !== null){
							var opa = Math.min(Math.round(ABPInst.cmManager.options.global.opacity * 100) + 5,100);
							ABPInst.cmManager.options.global.opacity = opa / 100;
							ABPInst.cmManager.options.scroll.opacity = opa / 100;
							ABPInst.removePopup();
