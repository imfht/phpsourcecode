...
define([], function(){
  ...
  var shapeStrategyFactory = (function(){
    return {
      createStrategy: function(shapeType){
        ...
        }else if(shapeType === 'biggerNode'){
          strategy = new shapeStrategy(new biggerNodeShape());
        }
        else if(!shapeType){
   ...
   * 策略类，根据选择实现不同的shape
   */
  function shapeStrategy(strategy) {
  ...
  function normalShape(){
    this._nodePadding = 38;
  }
  normalShape.prototype.doShape = function(node){
  ...
  rootShape.prototype.doShape = function(node, options){
    ...
    if(options.mindmapType == 'desktop'){
      rect.attr({ fill: '#428bca'});
    }
    else if(options.mindmapType == 'taskMore'){
      rect.attr({
        //fill: '#cb983e'
        //fill: '#676bfb'
        fill: '#64d4a5'
   ...
   *
   */
  function biggerNodeShape(){
    this._nodePadding = 48;
  }
  biggerNodeShape.prototype.doShape = function(node){
    var shape = node.shape;
    var label = shape[0];
    var rect = shape[1];
  ...
  selectedShape.prototype.doShape = function(node){
    var shape = node.shape;
    shape[1].attr({
      stroke: '#ff0033',
      'stroke-width': 3.5
  ...
  unSelectedShape.prototype.doShape = function(node){
    var shape = node.shape;
    shape[1].attr({
      stroke: '#000',
      'stroke-width': 2.5
