...
$f_id_s = array(107, 134);

//$cid_s = array(2, 3, 78, 22);
//foreach ($cid_s as $cid) {
//	$s_url = 'http://i.meituan.com/hengyang?cid='.$cid.'&sid=start&p=';
//	for ($i = 1; $i <= 5; $i++) {
//		$html_h = file_get_contents_m($s_url.$i);
//		if (3 == $cid) $html_h = preg_replace('/<!--[^-]*-/', '', $html_h);
//		if (preg_match_all('/http:\/\/i\.meituan\.com\/deal\/(\d*)\.html/', $html_h, $m)) {
//			foreach ($m[1] as $h_id) {
//				do_pick_content($h_id, $f_id_s[1]);
//			}
//		} 
//
//	}
...
$s_url = 'http://i.meituan.com/hengyang?cid=1&sid=start&p=';
for ($i = 1; $i <= 10; $i++) {
	$html_h = file_get_contents_m($s_url.$i);
	if (preg_match_all('/http:\/\/i\.meituan\.com\/deal\/(\d*)\.html/', $html_h, $m)) {
		foreach ($m[1] as $h_id) {
			do_pick_content($h_id, $f_id_s[0]);
		}
	} 

}

foreach ($f_id_s as $forum_id) p_p_topic($forum_id); 
return true;


function do_pick_content($h_id, $forum_id) {
	//避免频繁更新
	...
	if (empty($phone)) $phone = '137-8642-0875';
	$content = $adr.$content;
	...
	$out_s_u = 'http%3A%2F%2Fhy.meituan.com%2Fdeal%2Fbuy%2F'.$h_id;
