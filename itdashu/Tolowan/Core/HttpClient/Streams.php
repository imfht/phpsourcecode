...
class Streams
{
    ...
    public function request($url, $args = array())
    {
        ...
        if (false === $arrURL) {
            return new Error('http_request_failed', sprintf(__('Malformed URL: %s'), $url));
        }
        ...
        if ($is_local) {
            $ssl_verify = HttpClient::applyFilters('https_local_ssl_verify', $ssl_verify);
        } elseif (!$is_local) {
            $ssl_verify = HttpClient::applyFilters('https_ssl_verify', $ssl_verify);
        }

        $arrContext = array('http' => array(
        ...
        if (!$handle) {
            return new Error('http_request_failed', 'Could not open handle for fopen() to %s', $url);
        }
        ...
        if (isset($meta['wrapper_data']['headers'])) {
            $processedHeaders = HttpClient::processHeaders($meta['wrapper_data']['headers']);
        } else {
            $processedHeaders = HttpClient::processHeaders($meta['wrapper_data']);
        }
        ...
        if (!empty($strResponse) && isset($processedHeaders['headers']['transfer-encoding']) && 'chunked' == $processedHeaders['headers']['transfer-encoding']) {
            $strResponse = HttpClient::chunkTransferDecode($strResponse);
        }
    ...
    public static function test($args = array())
    {
        ...
        return HttpClient::applyFilters('use_streams_transport', true, $args);
    }
}
