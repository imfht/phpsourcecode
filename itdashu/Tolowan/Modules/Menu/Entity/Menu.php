...
use Modules\Config\Entity\ConfigList;
use Phalcon\Exception;

/**
 */
class Menu extends ConfigList
{
    ...
    public static function findFirst($query)
    {
        ...
        if (isset($query['block']) && isset($blockList[$query['block']])) {
            $block = new Block();
            foreach ($blockList[$query['block']] as $key => $value) {
                $block->{$key} = $value;
            }
            return $block;
        } elseif (isset($query['block'])) {
            return false;
        }
    }
