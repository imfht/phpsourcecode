...
class UploadFile {//类定义开始
    ...
    public function upload($savePath ='') {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                if(function_exists($this->hashType) && file_exists($file['savepath'].$file['savename'])) {
                    $file['hash']   =  call_user_func_array(
                        $this->hashType,
                        array($this->autoCharset($file['savepath'].$file['savename'],'utf-8','gbk'))
                    );
                }
                //上传成功后保存文件信息，供其他地方调用
    ...
    private function getSaveName($filename) {
        ...
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
            ...
            if(is_callable($this->saveRule)) {
                //使用函数生成一个唯一文件标识号
                $saveName = call_user_func($this->saveRule).".".$filename['extension'];
            } else {
                //使用给定的文件名作为标识号
