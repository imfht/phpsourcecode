...
class Article extends Admin
{
    ...
    /* 保存允许访问的公共方法 */
    static protected $allow = ['draftbox','mydocument'];
    ...
    protected function checkDynamic()
    {
        ...
        switch(strtolower($this->request->action())){
            ...
            case 'permit':    //回收站
                ...
                $cate_id =  \think\Db::name('Document')->where(array('id'=>array('in',$doc_id)))->getField('category_id',true);
...
//     	empty($cate_id) && $this->error('分类参数不能为空');
        if(!empty($cate_id)){
            //当前分类支持的文档模型
        	$modelList =   \think\Db::name('Category')->getFieldById($cate_id,'model');  
        }else{
        	$modelList = 1;
        } 
