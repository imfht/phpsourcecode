...
//     );
    ...
    public function lists($category, $order = '`id` DESC', $status = 1, $field = true){
        ...
        return \think\Db::name($this->name)->field($field)->where($map)->order($order)->select(); 
    }
    ...
    public function detail($id){
        /* 获取基础数据 */
        $info = db($this->name)->field(true)->find($id); 
        if ( !$info ) {
            $this->error = '文档不存在';
            return false;
    ...
    public function prev($info){  
        ...
        /* 返回前一条数据 */
        return  \think\Db::name($this->name)->field(true)->where($map)->order('id desc')->find();
    }
