...
<title>了了社区 &rsaquo; 调整配置文件</title>
<script>
function check(){
...
<?php
	if(isset($_POST['submit'])&&$_GET['step']==2){
		...
		$sql=file_get_contents("./liaoliao.sql");
		$sql_arr=explode(";",$sql);
		try{
    ...
    $db->exec("SET sql_mode=''");
		...
		$db->exec("insert into liaoliao_admin(username,password) values('".$_POST['auser']."','".cc_encrypt($_POST['apass'],$key)."')");
		$db->exec("update liaoliao_setting set siteurl='".$_POST['root']."' where id=1");
		$db->exec("insert into liaoliao_user(username,password,email,avatar,status,time) values('【系统通知】','".cc_encrypt('123123',$key)."','system@liaoliao.youyax.com','static/images/system.jpg','-1',now())");
