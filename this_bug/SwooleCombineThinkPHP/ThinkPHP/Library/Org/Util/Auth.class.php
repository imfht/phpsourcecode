...
class Auth{
    ...
    protected $_config = array(
        ...
        'AUTH_GROUP' => 'hengtu_authgroup', //用户组数据表名
        'AUTH_GROUP_ACCESS' => 'hengtu_authaccess', //用户组明细表
//        'AUTH_RULE' => 'hengtu_authrule', //权限规则表
        'AUTH_RULE' => 'hengtu_menu', //权限规则表,与菜单表合并
        'AUTH_USER' => 'hengtu_admin'//用户信息表
    );
...
//          if (in_array($val, $name))
            if (in_array(strtolower($val), $name))//数据表中的name规则忽略大小写，全部转成小写处理 @lxh
                $list[] = $val;
        } 
