...
function insertNewRow() {
    ...
    var insertStr = "<tr id=" + rowId + ">"
                  + "<td style='width: 30%'>" + firstCell + "</td>"
                  + "<td style='width: 50%'>" + secondCell + "</td>"
                  + "<td style='width: 10%'>" + thirdCell + "</td>"
    ...
    //这里的行数减1，是因为要减去顶部的一行，剩下的为开始要插入行的索引
     ...
     $("#" + rowId + " td:eq(1)").children().eq(0).attr("id", "remarks" + flag);
     $("#" + rowId + " td:eq(2)").children().eq(0).attr("id", "sort" + flag);
     $("#" + rowId + " td:eq(2)").children().eq(0).attr("value", flag*5);
		 ...
		 if (i >= 1) {
			 ...
			 $(this).children().each(function(j) {
				 ...
				 if(j==0 && value==""){
					 ...
					 $("#name"+xh).after('<span class="error">请输入类别名称！</span>');
					 ifalg=0;
				 }else if(j==0 && value!=""){
					 $("#name"+xh).next("span").empty();
				 }
				 ...
				 if(j==1 && value==""){
					 $("#remarks"+xh).next("span").empty();
					 $("#remarks"+xh).after('<span class="error">请输入类别描述！</span>');
					 ifalg=0;
				 }else if(j==1 && value!=""){
					 $("#remarks"+xh).next("span").empty();
				 }
				 ...
				 if(j==2 && value==""){
					 ...
					 $("#sort"+xh).after('<span class="error">请输序号！</span>');
					 ifalg=0;
				 }else if(j==2 && value!=""){
					 var re =  /^[1-9]+[0-9]*]*$/;
					 if (!re.test(value)) {
