...
class NewsController extends NewsBaseController
{
    ...
    public function detailAction(\GeorgRinger\News\Domain\Model\News $news = null, $currentPage = 1)
    {
        ...
        /**add hit 2019-7-22 17:24:32**/
        GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('tx_news_domain_model_news')
        ->update(
            'tx_news_domain_model_news',
            [ 'hit' => $news->getHit()+1 ], // set
            [ 'uid' => $news->getUid()] // where
        );
        ...
        if($news->getKeywords()){
            $metaTagManager = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\MetaTag\MetaTagManagerRegistry::class)->getManagerForProperty('keywords');
            $metaTagManager->addProperty('keywords', $news->getKeywords());
        }
        // <meta name="description" content="这是个介绍">
        if($news->getTeaser()){
        //if($news->getDescription()){
            $metaTagManager = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\MetaTag\MetaTagManagerRegistry::class)->getManagerForProperty('description');
