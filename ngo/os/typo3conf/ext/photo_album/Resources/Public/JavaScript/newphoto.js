...
function insertNewRow() {
	...
	var insertStr = "<tr id=" + rowId + ">"
	              ...
	              + "<td style='width: 30%'>" + secondCell + "</td>"
	              + "<td style='width: 30%'>" + thirdCell + "</td>"
	              + "<td class='text-center'><input type='button' class='btn btn-danger' name='delete' value='删除' onclick='deleteSelectedRow(\"" + rowId + "\")' />";
	              +"</tr>";
	//这里的行数减1，是因为要减去顶部的一行，剩下的为开始要插入行的索引
	
	 $("#orderTable tr:eq(" + (rowLength - 1) + ")").after(insertStr); //将新拼接的一行插入到当前行的下面
	 //为新添加的行里面的控件添加新的id属性。
	 $("#" + rowId + " td:eq(0)").children().eq(0).attr("id", "photo" + flag);
	 $("#" + rowId + " td:eq(1)").children().eq(0).attr("id", "title" + flag);
	 $("#" + rowId + " td:eq(2)").children().eq(0).attr("id", "description" + flag);
	...
	$("#orderTable tr").each(function(i) {
		if (i >= 1) {
			...
			$(this).children().each(function(j) {
				 ...
				 if(j==0 && value==""){
					 $("#photo"+xh).next("span").empty();
					 $("#photo"+xh).after('<span class="error">请选择上传文件！</span>');
					 ifalg=0;
				 }else if(j==0 && value!=""){
					 $("#photo"+xh).next("span").empty();
				 }
